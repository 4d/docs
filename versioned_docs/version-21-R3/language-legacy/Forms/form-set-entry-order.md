...
If an invalid object is detected at runtime, it is simply ignored and 4D will try to use the next valid object in the *objectNames* array. You can know the actual entry order of the current page (based upon valid objects) using the [FORM GET ENTRY ORDER](../commands/form-get-entry-order) command with the \* parameter. 
...
* This command does not define the first focused object in the form at runtime. If you want to set a first object in the entry order, you need to use the [GOTO OBJECT](../commands/goto-object) command in the On Load event of the form. If you used the [OBJECT DUPLICATE](../commands/object-duplicate) command, you can set the duplicated object as the first one by passing the Object First in entry order constant in the *boundTo* parameter.
