...
* If [COPY NAMED SELECTION](../commands/copy-named-selection) was used to create *name*, the named selection *name* is copied to the current selection of the table to which *name* belongs. The named selection *name* exists in memory until it is cleared. Use the [CLEAR NAMED SELECTION](../commands/clear-named-selection) command to clear the named selection and free the memory used by *name*.
* If [CUT NAMED SELECTION](../commands/cut-named-selection) was used to create *name*, the current selection is set to *name* and *name* no longer exists in memory.
