...
The **INVOKE ACTION** command does not generate an error, for example, if the requested action is not available in the current context. You must validate the expected action using the [Action info](../commands/action-info) command. 
