...
<!--REF #_command_.ENCRYPT BLOB.Summary-->The ENCRYPT BLOB command encrypts the content of the *toEncrypt* BLOB with the sender’s private key *sendPrivKey*, as well as optionally the recipient’s public key *recipPubKey*.<!-- END REF--> These keys should be generated by the command [GENERATE ENCRYPTION KEYPAIR](../commands/generate-encryption-keypair) (within the “Secured Protocol” theme).
...
Once the command has been run, the *toEncrypt* BLOB contains the encrypted data that will be decrypted only with the [DECRYPT BLOB](../commands/decrypt-blob) command, with the sender’s public key passed as parameter.   
...
1) The company generates a pair of keys with the command [GENERATE ENCRYPTION KEYPAIR](../commands/generate-encryption-keypair):
