...
|`--user-param`|Custom user string|A string that will be available within the application through the [`Get database parameter`](../commands-legacy/get-database-parameter.md) command (the string must not start with a "-" character, which is reserved). |
