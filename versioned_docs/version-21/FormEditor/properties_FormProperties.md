...
This property defines the color scheme for the form. By default when the property is not set, the value for a color scheme is **inherited** (the form uses the scheme defined at the [application level](./commands/set-application-color-scheme)). This can be changed for the form to one of the following two options:
