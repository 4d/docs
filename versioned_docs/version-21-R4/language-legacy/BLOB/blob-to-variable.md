...
The BLOB data must be consistent with the destination variable. Typically, you will use BLOBs that you previously filled out using the command [VARIABLE TO BLOB](../commands/variable-to-blob).
...
**Note:** **BLOB TO VARIABLE** supports object and collection variables. For more information, refer to the [VARIABLE TO BLOB](../commands/variable-to-blob) command.
...
**Note regarding Platform Independence:** BLOB TO VARIABLE and [VARIABLE TO BLOB](../commands/variable-to-blob) use a 4D internal format for handling variables stored in BLOBs. As a benefit, you do not need to worry about byte swapping between platforms while using these two commands. In other words, a BLOB created on Windows using either of these commands can be reused on Macintosh, and vice-versa.

## Example 

See the examples for the [VARIABLE TO BLOB](../commands/variable-to-blob) command.
