...
**QUERY BY FORMULA** and [QUERY SELECTION BY FORMULA](../commands/query-selection-by-formula) work exactly the same way, except that **QUERY BY FORMULA** queries every record in the entire table and [QUERY SELECTION BY FORMULA](../commands/query-selection-by-formula) queries only the records in the current selection.
...
> **Compatibility note** **:** For compatibility reasons with databases converted from versions older than v11, it is possible to disable the joins mechanism using a selector of the [SET DATABASE PARAMETER](../commands/set-database-parameter) command.
...
This example finds the records for all invoices that were entered in December of any year. It does this by applying the [Month of](../commands/month-of) function to each record. This query could not be performed any other way without creating a separate field for the month:
