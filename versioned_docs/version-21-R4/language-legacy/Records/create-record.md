...
<!--REF #_command_.CREATE RECORD.Summary-->**CREATE RECORD** creates a new empty record for *aTable*, but does not display the new record.<!-- END REF--> Use [ADD RECORD](../commands/add-record) to create a new record and display it for data entry. 

**CREATE RECORD** is used instead of [ADD RECORD](../commands/add-record) when data for the record is assigned with the language. The new record becomes the current record but the current selection is left untouched.

The record exists in memory only until a [SAVE RECORD](../commands/save-record) command is executed for the table. If the current record is changed (for example, by a query) before the record is saved, the new record is lost.
