...
function getCollator(locale) {
  if (!collators[locale]) {
    collators[locale] = new Intl.Collator(locale || 'en');
  }
  return collators[locale];
}

function labelForSort(item) {
  return item.label || item.title || item.id || '';
}

function sortList(items, locale) {
  const collator = getCollator(locale);
  return [...items].sort((a, b) => collator.compare(labelForSort(a), labelForSort(b)));
}

function transformTree(items, locale, parentLabel = null) {
  // 1) Renommer les catégories "commands" pour éviter les collisions
  for (const it of items) {
    if (it.type === 'category') {
      const normalized = String(it.label || "").toLowerCase();
      const isCommands = normalized === "commands" || normalized === "commands-legacy";
      if (isCommands && parentLabel) {
        // ex: "Backup — commands"
        it.label = `${parentLabel} — ${it.label}`;
      }
      if (Array.isArray(it.items)) {
        it.items = transformTree(it.items, locale, it.label);
      }
    }
  }
  // 2) Trier à ce niveau
  return sortList(items, locale);
}
      ...
      '@docusaurus/preset-classic',
      {
        docs: {
          ...
          sidebarItemsGenerator: async function (args) {
            ...
            const { item, locale } = args;

            // Cible UNIQUEMENT l'arbre généré depuis docs/language/
            if (item?.type === 'autogenerated' && item.dirName === 'language') {
              return transformTree(items, locale, /* parentLabel */ 'Commands by theme');
            }
