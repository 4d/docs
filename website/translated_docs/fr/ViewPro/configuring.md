...
|           | kind   | Texte       | Name of standard paper size (e.g., A2, A4, legal, etc.) returned by `Get Print Option`. Default value = "letter" |
...
| fitPagesTall   | entier long | The number of vertical pages (portrait orientation) to check when optimizing printing.<p> Default value = -1    |
| fitPagesWide   | entier long | The number of horizontal pages (landscape orientation) to check when optimizing printing. <p>Default value = -1 |
...
|           | \[ ].page     | Texte                | The page(s) where the watermark is printed.<p>For all pages: "all". For specific pages: page numbers or page ranges separated by commas. Ex.: "1,3,5-12" |
