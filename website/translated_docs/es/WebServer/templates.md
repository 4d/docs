...
El análisis del contenido de las páginas plantillas enviadas por el servidor web de 4D tiene lugar cuando se llaman los comandos `WEB SEND FILE` (.htm, .html, .shtm, .shtml), `WEB SEND BLOB` (text/html type BLOB) o `WEB SEND TEXT`, así como cuando se envían páginas llamadas mediante URLs. In this last case, for reasons of optimization, pages that are suffixed with “.htm” and “.html” are NOT parsed. Para "forzar" el análisis de las páginas HTML en este caso, debe añadir el sufijo ".shtm" o ".shtml" (por ejemplo, `http://www.server.com/dir/page.shtm`). Un ejemplo del uso de este tipo de página se da en la descripción del comando `WEB GET STATISTICS`. Las páginas XML (.xml, .xsl) también son compatibles y siempre son analizadas por 4D.
