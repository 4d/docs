...
- **Integrar el último archivo de historial si la base está incompleta**: cuando se marca esta opción, el programa integra automáticamente el archivo de historial al abrir o restaurar la base de datos.
    - Durante la apertura de la base, el archivo de historial actual se integra automáticamente si 4D detecta que hay operaciones almacenadas en el archivo de historial que no están presentes en los datos. This situation arises, for example, if a power outage occurs when there are operations in the data cache that have not yet been written to the disk.
    - Al restaurar la base, si el archivo de historial actual o un archivo de copia de seguridad del historial que tiene el mismo número que el archivo de copia de seguridad se almacena en la misma carpeta, 4D examina su contenido. If it contains operations not found in the data file, the program automatically integrates it.
