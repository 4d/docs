...
<!--REF file.create().Note --> **No disponible para archivos ZIP**<!-- END REF -->
...
Creación de un archivo de preferencias en la carpeta de la base:
...
La función `.createAlias()` <!-- REF #FileClass.createAlias().Summary --> crea un alias (macOS) o un acceso directo (Windows)<!-- END REF --> para el archivo con el nombre *aliasName* especificado en la carpeta designada por el objeto *destinationFolder*.
...
Quiere crear un alias para un archivo en su carpeta de base:
...
La función `.moveTo()` <!-- REF #FileClass.moveTo().Summary --> mueve o renombra el objeto `File` en la *destinationFolder*<!-- END REF -->.
...
Quiere cambiar el nombre de "ReadMe.txt" a "ReadMe_new.txt":
...
| Parámetros | Tipo   |    | Descripción                                                                |
| ---------- | ------ | -- | -------------------------------------------------------------------------- |
| info       | Objeto | -> | Propiedades a escribir en el archivo .plist o el recurso versión .exe/.dll |
...
La función `.setAppInfo()` <!-- REF #FileClass.setAppInfo().Summary --> escribe las propiedades *info* como contenido informativo de un archivo **.exe**, **.dll** o **.plist**.
...
**Parámetro *info* con un un archivo .exe o .dll**

> Escribir la información de archivos .exe o .dll sólo es posible en Windows.

Cada propiedad válida definida en el parámetro objeto *info* se escribe en el recurso de versión del archivo .exe o .dll. Las propiedades disponibles son (cualquier otra propiedad será ignorada):
