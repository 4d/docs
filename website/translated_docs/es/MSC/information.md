...
- **Encriptable**: si se marca, se selecciona el atributo **Encriptable** para la tabla al nivel de la estructura (ver el párrafo Encriptable en el Manual de Diseño).
- **Encrypted**: If checked, the records of the table are encrypted in the data file. ***Nota:** toda incoherencia entre las opciones Cifrable y Cifrado requiere que se compruebe el estado de cifrado del archivo de datos en la **página Cifrar** de la base. *
