...
> Compacting is only available in maintenance mode. Si intenta realizar esta operación en modo estándar, una caja de diálogo de alerta le informará de que la base se cerrará y se reiniciará en modo de mantenimiento. Puede compactar un archivo de datos que no esté abierto por la base de datos (ver [Compactar los registros y los índices](#compact-records-and-indexes) abajo).
...
When the operation is completed, the compacted files automatically replace the original files. La base de datos es inmediatamente operacional sin ninguna otra manipulación.
...
Una vez finalizada la compactación, 4D genera un archivo de historial en la carpeta Logs de la base. This file allows you to view all the operations carried out. Se crea en formato XML y se denomina  *DatabaseName**_Compact_Log_yyyy-mm-dd hh-mm-ss.xml*" donde:
...
> - Using this option makes the address table, and thus the database, incompatible with the current journal file (if there is one). Se guardará automáticamente y habrá que crear un nuevo archivo de historial la próxima vez que se inicie la base.
