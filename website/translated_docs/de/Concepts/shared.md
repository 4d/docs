...
- Der Aufruf von `Use` in shared object/collection innerhalb einer Gruppe sperrt die Eigenschaften/Elemente aller shared objects/collections, die zur gleichen Gruppe gehören und erhöht ihren Zähler (locking counter). Aufrufen von `End use` verringert der Zähler (locking counter) der Gruppe. Ist dann der Zähler bei 0, werden alle verknüpften shared objects/collections entsperrt.
