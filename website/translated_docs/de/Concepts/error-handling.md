...
Mit dem Befehl `Method called on error` erfahren Sie den Namen der Methode, die über den Befehl `ON ERR CALL` für den aktuellen Prozess installiert wurde. It is particularly useful in the context of components because it enables you to temporarily change and then restore the host application error-catching method:
