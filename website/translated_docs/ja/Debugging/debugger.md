...
メソッドは中断されます。 メソッドエディターウィンドウが開いて、編集ボタンがクリックされた時点で実行していたメソッドを表示します。
...
| Subtheme     | 説明                                                           | Can the values be modified? |
| ------------ | ------------------------------------------------------------ | --------------------------- |
| Interprocess | List of interprocess variables being used at this point      | ◯                           |
| Process      | List of process variables used by the current process        | ◯                           |
| Local        | List of local variables used by the method being traced      | ◯                           |
| 引数           | List of parameters received by the method                    | ◯                           |
| Self         | Pointer to the current object, when tracing an Object Method | ×                           |
