...
The `.name` property contains the name of the `4D.Class` object. クラス名の大文字・小文字は区別されます。
...
The `.new()` function creates and returns a `cs.className` object which is a new instance of the class on which it is called. この関数は、[`cs` クラスストア](Concepts/classes.md#cs) に属する全クラスで自動的に利用可能です。
...
The `.superclass` property returns the parent class of the class. スーパークラスは、`4D.Class` オブジェクト、あるいは `cs.className` オブジェクトのいずれかです。 親クラスが存在しない場合は、このプロパティは **null** を返します。
