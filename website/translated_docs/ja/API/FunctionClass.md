...
`.apply()` 関数は、 <!-- REF #FunctionClass.apply().Summary -->対象の `Formula` オブジェクトを実行し、その結果の値を返します<!-- END REF -->。 The formula object can be created using the `Formula` or `Formula from string` commands.
...
`.call()` 関数は、 <!-- REF #FunctionClass.call().Summary -->対象の `Formula` オブジェクトを実行し、その結果の値を返します<!-- END REF -->。 The formula object can be created using the `Formula` or `Formula from string` commands.
