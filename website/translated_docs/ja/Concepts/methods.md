...
> 4D ランゲージは **クラスメソッド** もサポートしています。クラスメソッドは、オブジェクトインスタンスのコンテキストにおいて呼び出されます。 Class functions can be built-in (*e.g.* `collection.orderBy()` or `entity.save()`), or [created by the 4D developer](classes.md#class-function).
