...
> **Tip:** アンダースコア ("_") 文字で関数名を開始すると、その関数は自動補完機能から除外されます。 たとえば、MyClase に `Function _myPrivateFunction` を宣言した場合、コードエディターにおいて `"cs.MyClass "` とタイプしても、この関数は候補として提示されません。
...
- "Function" クラスメンバーメソッド 
    - `apply()`
    - `call()`

> **スレッドセーフに関する警告:** クラス関数がスレッドセーフではなく、"プリエンプティブプロセスで実行可能" なメソッドから呼び出された場合:  
