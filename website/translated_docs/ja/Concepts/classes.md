...
> **Tip:** アンダースコア ("_") 文字でメンバーメソッド名を開始すると、そのメンバーメソッドは自動補完機能から除外されます。 たとえば、MyClase に `Function _myPrivateFunction` を宣言した場合、コードエディターにおいて `"cs.MyClass "` とタイプしても、このメンバーメソッドは候補として提示されません。
