...
引数は、ボディに定義されたコレクションに格納します。 たとえば、DataClass クラス関数 `getCities()` はがテキスト引数を受け取る場合: `/rest/City/getCities`
...
| __DATACLASS | 文字列               | 必須 - エンティティのデータクラスを指定します             |
...
| ---------- | --- | ------------------------------------------ |
| エンティティの属性  | 混合  | 任意 - 変更する値                                 |
| __DATASET  | 文字列 | 必須 - エンティティセレクションのエンティティセットID (UUID)       |
