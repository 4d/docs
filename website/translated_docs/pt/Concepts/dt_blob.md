...
A BLOB (Binary Large OBject) field, variable or expression is a contiguous series of bytes which can be treated as one whole object or whose bytes can be addressed individually. Um BLOB pode estar vazio (longitude nula) ou pode conter até 2147483647 bytes (2 GB).
...
You can address each byte of a blob individually using the curly brackets symbols {...}. Dentro de um BLOB, os bytes são numerados de 0 a N-1, onde N é o tamanho do BLOB. Example:
```4d
  ` Declare a variable of type BLOB
 C_BLOB(vBlob)
