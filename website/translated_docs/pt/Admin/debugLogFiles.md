...
Aplicações 4D podem gerar vários arquivos de histórico ou log que são úteis para depuração e otimizar sua execução. Históricos são geralmente começados ou parados usando seletores dos comandos [SET DATABASE PARAMETER](https://doc.4d.com/4dv19/help/command/en/page642.html) ou [WEB SET OPTION](https://doc.4d.com/4dv19/help/command/en/page1210.html) e são armazenados na pasta [Logs](Project/architecture.md#logs-folder) do banco de dados.
...
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| sequence_number                            | Número de operação único e sequêncial na sessão de histórico                                                                                                                                                 |
...
| process\_info_                           | index   Corresponde ao campo "index" no histórico 4DRequestsLog_ProcessInfo.txt e permite conectar uma petição a um processo.                                                                                |
...
| server\_duration &#124; exec\_duration | Depende de onde o histórico for gerado:<p><li>*server\_duration* se for gerado no cliente - tempo que levou em microssegundos para o servidor processar a petição e retornar a resposta. B a F na imagem abaixo OU</li><li>*exec\_duration* se gerado no servidor - tempo que levou em microssegundos para o servidor processar a petição. B a E na imagem abaixo.</li>                                                                                          |
| write\_duration                          | Tempo que levou em microssegundos para enviar:<p><li>a petição (quando rodar no cliente). A a B na imagem abaixo.</li><li>Resposta (quando rodar no servidor). E a F na imagem abaixo.</li>                                                                                   |
| task_kind                                  | Preemptivo ou cooperativo (respectivamente 'p' ou 'c')                                                                                                                                                       |
| rtt                                        | Tempo estimado em microssegundos para o cliente enviar a petição e o servidor para reconhecê-la. A a D e E a H na imagem abaixo.<p><li>Só medido quando usar a capa de rede ServerNet, retorna 0 se usado com capa de rede legado.</li><li>Para versões de Windows anteriores a Windows 10 ou Windows Server 2016, a chamada retorna 0.</li> |
...
*   Identificador de sessão de histórico
*   Hostname do servidor que hospeda a aplicação
*   Nome de login de usuário: login do SO do usuário que roda a aplicação 4D no servidor.
...
| Nome de campo                     | Descrição                                                               |
| --------------------------------- | ----------------------------------------------------------------------- |
| sequence_number                   | Número de operação único e sequêncial da sessão de log                  |
...
| SequenceNumber | Número de operação único e sequencial na sessão de histórico |
...
| Column # | Descrição                                                                                                       |
| -------- | --------------------------------------------------------------------------------------------------------------- |
| 1        | Número único e sequencial de operação na sessão de histórico                                                    |
...
//formato tabulado estendido, todos os processos
...
| Column # | Campo nome                      | Descrição                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| -------- | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1        | sequence_number                 | Número de operação único e sequencial na sessão de histórico                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 2        | time                            | Data e hora em formato ISO 8601 (YYYY-MM-DDThh:mm:ss.mmm)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 3        | ProcessID                       | Process ID                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 4        | unique_processID                | ID de processo                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 5        | stack_level                     | Nível stack                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
*   uma versão comum:
    ...
    *   para depurações comuns
    ...
    Para iniciar esse registro:

    ```4d
    SET DATABASE PARAMETER(SMTP Log;1) //começa SMTP log 
    SET DATABASE PARAMETER(POP3 Log;1) //começa POP3 log 
    SET DATABASE PARAMETER(IMAP Log;1) //começa IMAP log
    ```
...
ds.startRequestLog(File("/PACKAGE/Logs/ordaLog.txt"))  
//também pode ser enviado à memória
...
| Nome de campo  | Descrição                                                    | Exemplo                                                 |
| -------------- | ------------------------------------------------------------ | ------------------------------------------------------- |
