...
* Digite uma lista de valores padrão nas propriedades objeto ao selecionar "\<Static List>" in the [Data Source](properties_DataSource.md) theme of the Property List. The default values are loaded into an array automatically. You can refer to the array using the name of the variable associated with the object.
...
```4d
  ...
  $aCities{6}:="Green Bay"
```
Neste caso, o nome da variável associada ao objeto de formulário deve ser *$aCities*. This code could be placed in the form method and be executed when the `On Load` form event runs.
...
```
 ...
 Neste caso também o nome da variável asociada al objeto del formulario debe ser *$aCities*. This code would be run in place of the assignment statements shown above.
