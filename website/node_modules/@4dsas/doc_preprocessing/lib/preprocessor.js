            ...
            switch(keywords[0]) {
                case TYPE.REF:
                    if(keywords.length > 1) {
                        ...
                        item.ID = this._convertID2Args(keywords)
        ...
        while ((match = re.exec(inContent)) != null) {
            ...
            switch(keywords[0]) {
                case TYPE.INCLUDE:
                    if(keywords.length > 1)
                    {
                        let keyword = this._convertID2Args(keywords)

                        const value = this._refManager.getContentFromID(keyword)
                        if(value.error === true) {
                            ...
                            log.show(log.MESSAGE.WARNING, "The include \'" + keyword + "\' in path " + inPath + " has an invalid reference")
                        }
                        isCommand = true;
                    }
                    break;
