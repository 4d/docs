...
for(v of versions)
{
    ...
    listVersions.push(new Promise((resolve) => {let processor = new preprocess.Preprocessing(info); processor.collect()
...
Promise.all(listVersions).then((localData)=> {
    ...
    for(l of localData)
    {
        ...
        function cleanPath(inPath) {
            return inPath.replace(/\\/g, '/').replace("versioned_docs/version-", "").replace("docs", "").replace("-","").replace(cleanVersion, "").replace(".md","").slice(1)
        }
        ...
        if(Object.keys(data).length !== 0) {
            Object.keys(data).forEach(function(key, index) {
                ...
                if(split[0] === "_command_") {
                    commands[split[1].toLowerCase()] = cleanPath(data[key]);
                }
            });
        }
