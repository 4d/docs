...
| --- | --- | --- | --- |
| tableTitles | String array | &rarr; | Table names as they must appear in dialog boxes |
| tableNumbers | Longint array | &rarr; | Actual table numbers |
...
<!--REF #_command_.SET TABLE TITLES.Summary-->**SET TABLE TITLES** lets you mask, rename, and reorder the tables of your database that appear in standard 4D dialog boxes within the Application environment (i.<!-- END REF-->e. when the startup mode is **Application**, or after you select **Test application** in the **Run** menu). For example, this command can modify the display of tables in the Query editor in Application mode.
