...
| --- | --- | --- | --- |
| variable | Variable | &srarr; | Variable to store in the BLOB |
| blob | Blob | &srarr; | BLOB to receive the variable |
...
* if you pass an object ([C\_OBJECT](c-object.md)) or a collection ([C\_COLLECTION](c-collection.md)) in the *variable* parameter, the command places a copy (and not a reference) of it in the BLOB as JSON in UTF-8\. If the object or collection contains pointers, their dereferenced values are stored in the BLOB, not the pointers themselves.
