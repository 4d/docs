...
| --- | --- | --- | --- |
| Function result | Object | &larr; | Information about the active application activity |
...
| volumeShadowCopyStatus      | string     | Returns the status of the Volume Shadow Copy Service. Windows only. For more information, see *Using Volume Shadow Copy Service on Windows*.  Possible values: Constant Value Comment vss available available The service is available. vss error error The service experienced an error. vss not available notAvailable The service is unavailable. vss update required updateRequired The service needs to be updated. | "available"                                     |
