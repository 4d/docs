...
* Do not call any interface elements in the **On Web Authentication database method** ([ALERT](alert.md), [DIALOG](../commands/dialog), etc.) because otherwise its execution will be interrupted and the connection refused. The same thing will happen if an error occurs during its processing.
