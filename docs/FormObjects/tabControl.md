...
The screens can represent pages in a multi-page form or an object that changes when the user clicks a tab. If the tab control is used as a page navigation tool, then the [`FORM GOTO PAGE`](../commands/form-goto-page) command or the `gotoPage` standard action would be used when a user clicks a tab. 
...
>You can also store the names of the pages in a hierarchical list and use the [LIST TO ARRAY](../commands/list-to-array) command to load the values into the array.
...
You can use the [`FORM GOTO PAGE`](../commands/form-goto-page) command in the tab control’s method:
