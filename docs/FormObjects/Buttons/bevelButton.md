...
The Bevel button style combines the appearance of the [Regular Button](regularButton.md) style (*i.e.*, a rectangle with a descriptive label) with the [Toolbar Button](toolbarButton.md) button style's pop-up menu property option. 

By default, the Bevel button style has a light gray background with a label in the center: 

![alt-text](assets/en/regularButton.en.png)

The appearance of the Bevel button style can be different when the cursor hovers over it depending on the OS:
...
The Bevel button's style property can be set via:

* **Property List:** Select *Bevel* from the **Button Style** drop-down list in the **Text and Picture** section.  <br>![alt-text](assets/en/bevelButton_propertyList.en.png)


* **Code:** Define the property using JSON: <br>Example: ```{"style":"bevel"} ```<br>
...
A button's appearance, location, and behavior can be altered by modifying its properties. The Bevel button style supports the following properties:  
