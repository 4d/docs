...
The OS X Textured button style is nearly identical to the [Bevel Button](bevelButton.md) style except, depending on the OS, it may have a different appearance. As with the Bevel button style, the OS X Textured button style combines the appearance of the [Regular Button](regularButton.md) style (*i.e.*, a rectangle with a descriptive label) with the [Toolbar Button](toolbarButton.md) style's pop-up menu property option. 

By default, the OS X Textured button style appears as:

 - *Windows* - a standard system button with a light gray background with a label in the center. It has the special feature of being transparent in Vista.<br>![alt-text](assets/en/regularButton.en.png)
 
 - *macOS* - a standard system button displaying a color change from light to dark gray. Its height is predefined: it is not possible to enlarge or reduce it.<br>**ADD PICTURE**
...
The OS X Textured button's style property can be set via:

* **Property List:** Select *OS X Textured* from the **Button Style** drop-down list in the **Text and Picture** section.  <br>![alt-text](assets/en/osTexturedButton_propertyList.en.png)


* **Code:** Define the property using JSON: <br>Example: ```{"style":"texturedBevel"} ```<br>
...
A button's appearance, location, and behavior can be altered by modifying its properties. The OS X Textured button style supports the following properties:  
