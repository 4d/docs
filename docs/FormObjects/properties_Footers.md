...
This property is used to display or hide column footers. No other options are available in each theme unless this property is selected. There is one footer per column; each header is configured separately.
...
This property is used to set the row height for a list box footer in **pixels** or **text lines**. Both types of units can be used in the same list box:
