...
|Name|Data Type|Possible Values|
|---|---|---|
|inheritedForm |string |<li>Name (string) of table or project form</li><li>a POSIX path (string) to a .json file describing the form</li><li>an object describing the form</li> |
...
The platform interface property sets the appearance of a form according to the context of its execution. This property can also be set individually for each form object.
...
*	**System**: This property allows adapting the appearance of the form or object automatically depending on the current platform on which 4D is running:
	*	When the form is displayed in macOS, the form or object has a macOS look and feel,
	*	When the form is displayed in Windows, the form or object has the look and feel based on the current “Appearance” setting of the control panel.<p>
...
This form property activates the textured look for the form when it is displayed using the `Open form window` command on macOS. This look is found throughout the macOS interface:

![](assets/en/FormObjects/propertytextured.png)

If the form is displayed in a window that was not created by `Open form window` (for example, in the Design mode), the property will not be taken into account. In macOS, the metal look is previewed in the Form editor when the **Textured** option is checked and when the form limits are displayed. When the form is executed in Windows, this option has no effect.
...
You can control access to a form by setting Access and Owner privileges for groups of users. A single group can be assigned for each privilege using the Access and Owner drop-down lists. For information about creating a password access system with users and groups, see the chapter *Users and groups* in the *4D Design Reference*.
...
2.	Use the **Access** and **Owner** drop-down lists to make the desired access privilege assignments.<br>
The names of existing groups are displayed in each drop-down list.
![](assets/en/FormObjects/propertyFormAccess.png)
<br>The **[...]** button can be used to access the Groups editor directly in the Tool Box of 4D.
...
The **Edit...** button lets you access the Form method. This is covered in *Editing methods* in the *4D Design Reference*.
