...
<!--REF #_command_.WP Selection range.Syntax-->**WP Selection range** ( {* ;} *wpArea* : Text ) : Object<!-- END REF-->
...
If an anchored image (picture at a fixed location) is selected in *wpArea*, **WP Selection range** returns a picture reference object and not a range object (for more information, please refer to the [WP Add picture](../commands/wp-add-picture) command description). Otherwise, **WP Selection range** returns a new range object. For more information on range objects, please refer to *Range handling commands*.
