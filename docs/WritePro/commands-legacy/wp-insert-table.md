...
You can pass in *nbCols* and *nbRows* the number of columns and rows to define in the table. By default, if *nbCols* and *nbRows* are omitted, a new table with 0 column and 0 row is created. You will then be able to add these elements using the [WP Table append row](../commands/wp-table-append-row) command.

The command returns the new table as a table object, that you can pass thereafter to the other WP table commands. The table is created with default font, border, and color attributes that you can customize using the [WP SET ATTRIBUTES](../commands/wp-set-attributes) command.
