...
Request and Debug logs: This area indicates the log files recording duration (when log files are activated). Log files include:
the request log file: stores information concerning the requests received by the server (excluding Web requests): time, process number, user, request size, processing time, etc. that can be used to analyze the server operation. This file is named 4DRequestsLog_X (X being the sequential number of the file).
the debugging events file: stores each execution of a method, 4D command or plug-in command in a file named “4DDebugLog_X.txt”. Each event is systematically recorded in the file before its execution, which ensures its presence in the file even if the application quits unexpectedly. Note that this file is erased and rewritten each time you launch the application. You can configure this file using the SET DATABASE PARAMETER command.
the 4D SMTP log file: stores each exchange between 4D Server and the SMTP Mail server, when a SMTP transporter object is processed on 4D Server. It is named 4DSMTPLog_X.txt. You can configure this file using the SET DATABASE PARAMETER command.
the 4D POP3 log file: stores each exchange between 4D Server and the POP3 Mail server, when a POP3 transporter object is processed on 4D Server. It is named 4DPOP3Log_X.txt. You can configure this file using the SET DATABASE PARAMETER command.

Note: Log files are stored in the Logs folder of the database, next to the structure file.
...
Note: This button only logs operations that are executed on the server. To log operations executed on a remote 4D, you need to use the SET DATABASE PARAMETER command.
Once the logs have been activated, the button title changes to Stop Request and Debug Logs, so that you can stop recording requests at any time. Keep in mind that restarting the log after stopping it "erases" the previous file.
