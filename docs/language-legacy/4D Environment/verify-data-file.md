...
*dataPath* designates a 4D data file (.4DD). It must correspond to the structure file defined by the *structurePath* parameter. Be careful, you can designate the current structure file but the data file must not be the current (open) file. To verify the currently open data file, use the [VERIFY CURRENT DATA FILE](../commands/verify-current-data-file) command. If you attempt to verify the current data file with the VERIFY DATA FILE command, an error is generated.
