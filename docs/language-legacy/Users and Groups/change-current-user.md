...
* If an alias was defined for the current user with the [SET USER ALIAS](../commands/set-user-alias) command, it is reset by the **CHANGE CURRENT USER** command call.
...
In the *user* parameter, pass the name or unique user ID (*userRef*) of the account to be used. The user names and IDs can be obtained using the [GET USER LIST](../commands/get-user-list) command.

If the user account does not exist or was deleted, error -9979 is returned. You can intercept this error with the error-handling method installed by the [ON ERR CALL](../commands/on-err-call) command. Otherwise, you can call the function [Is user deleted](../commands/is-user-deleted) to test the user account before calling this command.
...
\- You can display the list of database users, as in the standard access dialog box of 4D, using the [GET USER LIST](../commands/get-user-list) command.   
...
\- If you want the characters of passwords being entered to be masked on screen, you can use the [FILTER KEYSTROKE](../commands/filter-keystroke) command with the special *%password* font.   
