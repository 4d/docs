...
The position of items is always expressed using the current expanded/collapsed state of the list and its sublists. You pass a position value between 1 and the value returned by [Count list items](../commands/count-list-items). If you pass a value outside this range, no item is selected.
