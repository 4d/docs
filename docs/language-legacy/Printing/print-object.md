...
* The [LISTBOX GET PRINT INFORMATION](../commands/listbox-get-print-information) command lets you check the status of the printing during the operation.

The **Print object** command can only be used in the context of a print job opened beforehand with the [OPEN PRINTING JOB](../commands/open-printing-job) command. If it is not called in this context, the command does nothing. Several Print object commands can be called in the same print job. 
