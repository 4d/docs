...
<!--REF #_command_.BLOB to print settings.Summary-->The **BLOB to print settings** command replaces the current 4D print settings with the parameters stored in the *printSettings* BLOB.<!-- END REF--> This BLOB must have been generated by the [Print settings to BLOB](../commands/print-settings-to-blob) command.
...
The parameters are applied to the [current 4D printing settings](../../settings/compatibility.md) as long as no command such as [SET PRINT OPTION](../commands/set-print-option) or [PRINT SELECTION](../commands/print-selection) without the > parameter modifies them. The parameters set are used more particularly by the [PRINT SELECTION](../commands/print-selection), [PRINT LABEL](../commands/print-label), [PRINT RECORD](../commands/print-record), [Print form](../commands/print-form) and [QR REPORT](../commands/qr-report) commands, as well as by the menu commands of 4D, including those of the Design environment.

The [PRINT SELECTION](../commands/print-selection), [PRINT LABEL](../commands/print-label), and [PRINT RECORD](../commands/print-record) commands must be called with the *\>* parameter (if applicable) in order for the settings defined by **BLOB to print settings** to be kept.
