...
<!--REF #_command_.HTTP GET OPTION.Summary-->The **HTTP GET OPTION** command returns the current value of the HTTP options (options used by client for next request triggered by the [HTTP Get](../commands/http-get) or [HTTP Request](../commands/http-request) commands).<!-- END REF--> The current value of an option can be the default value or it can have been modified using the [HTTP SET OPTION](../commands/http-set-option) command.
