...
**Note:** Starting with 4D 16 R4, **JSON PARSE ARRAY** can usually be replaced by a call to [JSON Parse](../commands/json-parse) that returns a **collection**. Collections are based on JSON arrays and allow to store data of mixed types, which provides more flexibility than arrays. 
