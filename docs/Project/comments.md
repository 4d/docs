...
Your comments are written in markdown syntax (.md files) using any editor. They are stored as independant files within your project folder. 
...
1. If a file named "<MethodName>.md" from "<package>/documentation" folder exists, the code editor displays, by priority:
...
2. Otherwise, the code editor displays [the block comment at the top of the method code](https://doc.4d.com/4Dv18R2/4D/18-R2/Writing-a-method.300-4824019.en.html#4618226).
For example:
```4d
// WP SwitchToolbar ("WParea" ; "Toolbar")
// WP SwitchToolbar (string ; string)
```
...
- The code tag is supported with 4D highlight: 
Example:
``` 4d
C_TEXT($txt)
$txt:="Hello world!" 
...
- The link tag shall be supported:
Example:
```
// Case 1
...
- Explorer view:
![](assets/en/Project/comments-explo6.png)
