...
|connectionTimeout|Real|**Read-only** Maximum time (in seconds) to establish the connection. If exceeded, the connection attempt is aborted and may trigger errors such as "DNS server unreachable or unknown domain name ", "DNS connection timeout" and "Connection timeout". | 30 |
