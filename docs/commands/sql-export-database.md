...
* a text file named "Export.sql" is created in the subfolder. This file is encoded in UTF-8 with a BOM (byte-order mark). It contains SQL [INSERT](/4Dv20R6/4D/20-R6/INSERT.300-7183999.en.html) orders corresponding to the exported data. The field values are separated by colons. There may be fewer values than there are fields in the table. In this case, the remaining fields will be considered NULL.
...
Note that binary field data (BLOB and Picture) that are embedded into the export file are written in hexadecimal format, in the form of X'0f20' (standard SQL hexadecimal notation, see [literal](/4Dv20R6/4D/20-R6/literal.300-7183955.en.html)). This format is automatically supported by the 4D SQL engine.   
