...
The *errorAction* parameter is used to configure the functioning of the command when an error occurs during script execution. You can pass one of the three following constants, placed in the [SQL](/4Dv20R6/4D/20-R6/SQL.302-6958478.en.html) theme:
...
The *attribName* and *attribValue* parameters must be passed by pairs. These parameters are intended to be used to specify specific attributes for the script execution. In the current version of 4D, a single attribute can be passed in *attribName*, available via the following constant, placed in the [SQL](/4Dv20R6/4D/20-R6/SQL.302-6958478.en.html) theme:
...
**Note:** If you use this command to execute memory-consuming actions such as massive data imports, you can consider calling the [ALTER DATABASE](/4Dv20R6/4D/20-R6/ALTER-DATABASE.300-7183984.en.html) SQL command in order to temporarily disable the SQL options.
