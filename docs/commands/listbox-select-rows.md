...
| action | Longint | -> | lk replace selection (default if omitted), lk add to selection, lk remove from selection |
...
The optional *action* parameter, if passed, is used to define the selection action to execute when a selection of rows already exists in the list box. You can pass a value or one of the following constants (located in the “[List Box](/4Dv20R6/4D/20-R6/List-Box.302-6958504.en.html)” theme):
