...
**4D Server:** In Client/Server mode, sets are "visible" depending on their type (interprocess, process and local) and where they were created (server or client). **INTERSECTION** requires all three sets to be visible on the same machine. See the [4D Server, Sets and Named Selections](/4Dv20R6/4D/20-R6/4D-Server-Sets-and-Named-Selections.300-7182867.en.html) section in the 4D Server Reference manual for more information.
