...
In *folder*, you can pass one of the following constants, which are located in the "[4D Environment](/4Dv20R6/4D/20-R6/4D-Environment.302-6958442.en.html)" theme (you will find below a description of each folder):
...
**Note: O**n Windows, in merged client projects, the location of this folder is modified if the [ShareLocalResourcesOnWindowsClient](/4Dv20R6/4D/20-R6/ShareLocalResourcesOnWindowsClient.300-7189565.en.html) BuildApp key is used. 
...
| buildApplicationName               | Text     | Optional - Name of the application (same as the [BuildApplicationName](/4Dv20R6/4D/20-R6/BuildApplicationName.300-7189525.en.html) BuildApp xml key value). Default is name of the current application.           |
| structureName                      | Text     | Mandatory except if the *clientServerSystemFolderName* attribute is defined - Name of the structure.                                                                                                              |
| ipAddress                          | Text     | Mandatory except if the *clientServerSystemFolderName* attribute is defined - IP address of the server (same as the [IPAddress](/4Dv20R6/4D/20-R6/IPAddress.300-7189556.en.html) BuildApp xml key value).         |
| portNumber                         | Number   | Mandatory except if the *clientServerSystemFolderName* attribute is defined - Publication port of the server (same as the [PortNumber](/4Dv20R6/4D/20-R6/PortNumber.300-7189557.en.html) BuildApp xml key value). |
| clientServerSystemFolderName       | Text     | Optional - Custom name of the local resources final folder (same as the [ClientServerSystemFolderName](/4Dv20R6/4D/20-R6/ClientServerSystemFolderName.300-7189549.en.html) BuildApp xml key value).               |
| shareLocalResourcesOnWindowsClient | Boolean  | Optional - Shared local resources folder (same as the [ShareLocalResourcesOnWindowsClient](/4Dv20R6/4D/20-R6/ShareLocalResourcesOnWindowsClient.300-7189565.en.html) BuildApp xml key value).                     |
...
* On Windows, in merged client projects, the location of this folder is modified if the [ShareLocalResourcesOnWindowsClient](/4Dv20R6/4D/20-R6/ShareLocalResourcesOnWindowsClient.300-7189565.en.html) BuildApp key is used.
...
**Note:** On Windows, in merged client projects, the location of this folder is modified if the [ShareLocalResourcesOnWindowsClient](/4Dv20R6/4D/20-R6/ShareLocalResourcesOnWindowsClient.300-7189565.en.html) BuildApp key is used. 
