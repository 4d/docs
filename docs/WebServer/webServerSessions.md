...
- Permanently, using the **Scalable sessions** option on the "Options (I)" page of the Settings: 
...
- Using the `Web scalable session` selector of the `WEB SET OPTION` command. In this case, this setting is applied only to the current server session and "overrides" the setting defined in the settings.
...
> A **Legacy sessions** option is available for compatibility in projects created with a 4D version prior to 4D v18 R6. It is recommended to enable **Scalable sessions** as soon as possible. 


## Session implementation

When sessions are enabled, automatic mechanisms are implemented: on each web client request, 4D checks for the presence and the value of the private "4DSID" cookie. This cookie references the web session.  
...
Each `Session` object provides a **storage** property which is a [shared object](Concepts/shared). This property allows you to share information between several processes handled by the session. 
...
This timeout can be set using the [timeout](API/sessionClass.md#timeout) property of the `Session` object (the timeout cannot be less than 60 minutes). 

When a scalable web session is closed, if the [`Session`](API/sessionClass.md#session) command is called subsquently:
...
|sessionCookieName|text|Name of the cookie used for storing the session ID. <p><p>Default = "4DSID"|
