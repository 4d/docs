...
|CORSEnabled|boolean|CORS (Cross-origin resource sharing) service status for the Web server. For security reasons, "cross-domain" requests are forbidden at the browser level by default. When enabled (True), any XHR calls (e.g. REST requests) from Web pages outside the domain will be allowed in your application (you need to define the list of allowed addresses in the CORS domain list, see `CORSSettings` below). When disabled (False, default), all cross site requests sent with CORS are rejected with a "403 - forbidden" error response.<p><p>Default: False (disabled)<p>For more information about CORS, please refer to the [Cross-origin resource sharing page](http://en.wikipedia.org/wiki/Cross-origin_resource_sharing) on Wikipedia.|
