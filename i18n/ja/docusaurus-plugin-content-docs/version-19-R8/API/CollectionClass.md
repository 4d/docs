...
`.combine()` 関数は、 <!-- REF #collection.combine().Summary -->コレクションインスタンスの最後、あるいは *index* で指定した位置に *col2* の要素を挿入し、変更された元のコレクションを返します<!-- END REF -->。 `.fill()` 関数は、
> このコマンドは、元のコレクションを変更します。
...
`.fill()` 関数は、 <!-- REF #collection.copy().Summary --> コレクションインスタンスのディープ・コピーを返します<!-- END REF -->。 この関数は ***シャロウ・コピー*** を返します。 つまり、元のコレクションにオブジェクト要素やコレクション要素が含まれていた場合、それらの参照は戻り値のコレクションで共有されます。
> このコマンドは、元のコレクションを変更しません。
...
<!-- REF #collection.extract().Syntax -->オブジェクトのコレクションを、*pathObjects* コレクションを使用して並べ替えます:<!-- END REF -->
...
`.query()` 関数は、 <!-- REF #collection.filter().Summary -->検索条件に合致するオブジェクトコレクションの要素をすべて返します<!-- END REF -->。 この関数は ***シャロウ・コピー*** を返します。 つまり、元のコレクションにオブジェクト要素やコレクション要素が含まれていた場合、それらの参照は戻り値のコレクションで共有されます。 また、元のコレクションが共有コレクションであった場合、返されるコレクションもまた共有コレクションになります。
> このコマンドは、元のコレクションを変更しません。
...
`.shift()` 関数は、 <!-- REF #collection.push().Summary -->コレクションの先頭要素を取り除き、それを戻り値として返します<!-- END REF -->。
> このコマンドは、元のコレクションを変更します。
...
`.reverse()` 関数は、 <!-- REF #collection.query().Summary -->全要素が逆順になった、コレクションのディープ・コピーを返します <!-- END REF -->検索条件は、*queryString* および、任意の *value* や *querySettings* パラメーターによって定義されます。 また、元のコレクションが共有コレクションであった場合、返されるコレクションもまた共有コレクションになります。
> このコマンドは、元のコレクションを変更しません。
...
`.copy()` 関数は、 <!-- REF #collection.reverse().Summary -->コレクションインスタンスのディープ・コピーを返します<!-- END REF -->。 また、元のコレクションが共有コレクションであった場合、返されるコレクションもまた共有コレクションになります。
> このコマンドは、元のコレクションを変更しません。
...
`.equal()` 関数は、 <!-- REF #collection.shift().Summary -->コレクションを collection2 とディープ比較し、同一の場合には **true** を返します<!-- END REF -->。
> このコマンドは、元のコレクションを変更します。
