...
- `kind` プロパティで用紙サイズを設定する場合、以下のいずれかを使用することができます:
  ...
  - [`PRINT OPTION VALUES`](../commands-legacy/print-option-values.md) コマンドによって返されるフォーマットのひとつ。
    ...
    この場合、[`VP Get print info`](./commands/vp-get-print-info.md) は対応するフォーマットを高さと幅とともに返します。この場合、[`VP Get print info`](./commands/vp-get-print-info.md) は対応するフォーマットを高さと幅とともに返します。
