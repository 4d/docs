...
> **注記:** [三項演算子](operators.md#三項演算子) を使うことで、条件式を 1行で書くことができ、If...Else 文を置き換えることもできます。
...
一般に、`While...End while` ループに入る手前で、ブール式で判定する値を初期化しておきます。 値を初期化するとは適切な値に設定するということで、通常はブール式が true になるように設定してからループに入ります。
