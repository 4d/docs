...
| storeCertificateName   | Text    | undefined                                                                                                                        | (Windows のみ) 証明書フォルダに保存されているものの代わりに使用する、証明書ストアに保存されている証明書の名前。 証明書が見つからなかった場合、エラーが返されます。 より詳細な情報に関しては、 [こちらの blog 記事](https://blog.4d.com/https-requests-now-support-windows-certificate-store) を参照してください。 |
