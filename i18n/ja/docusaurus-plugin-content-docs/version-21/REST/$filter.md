...
| [].metric    | Text                  | 任意。 クエリに使用する[ベクトル計算](../API/VectorClass.md#ことなるベクトル計算を理解する)。 Supported values:<li>"cosine" (default if omitted): calculates the cosine similarity between vectors.</li><li>"dot": ベクトルのドット類似度を計算します。</li><li>"euclidean": ベクトル間のユークリッド距離を計算します。 |
