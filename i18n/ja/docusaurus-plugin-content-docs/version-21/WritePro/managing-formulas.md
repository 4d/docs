...
| [This](../commands/this.md).pageNumber (*) | Number | 定義されているページ番号:<ul><li>ドキュメントの開始からのページ番号(デフォルト) あるいは</li><li>セクションの開始ページが定義されている場合にはセクション開始からのページ番号。</li></ul>このフォーミュラは常にダイナミックであり、[**WP FREEZE FORMULAS**](commands-legacy/wp-freeze-formulas.md) コマンドの影響を受けません。 |
