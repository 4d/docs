...
`4D.WebForm` オブジェクトは、 **プロキシオブジェクト** であり、Web フォームオブジェクト自身への直接的な参照ではないことに留意してください。 その結果として例えば、`4D.WebForm` オブジェクトはすべてのページプロパティをデバッガ上に表示するわけではありません。 その結果として例えば、`4D.WebForm` オブジェクトはすべてのページプロパティをデバッガ上に表示するわけではありません。
