...
- **macOSにて改行コードとしてLFを使用する:** 4D v19 R2 以降 (XMLファイルについては 4D v19 R3 以降) の新規プロジェクトにおいて、4D は macOS でデフォルトの改行コード (EOL) として CR (xml SAX では CRLF) ではなくラインフィード (LF) をテキストファイルに書き込みます。 以前の 4D のバージョンから変換されたデータベースにおいてこの新しい振る舞いを利用したい場合には、このオプションをチェックしてください。 [`TEXT TO DOCUMENT`](../commands/text-to-document)、[`Document to text`](../commands/document-to-text)、および [XML SET OPTIONS](../commands/xml-set-options) コマンドの詳細を参照してください。 以前の 4D のバージョンから変換されたデータベースにおいてこの新しい振る舞いを利用したい場合には、このオプションをチェックしてください。 [`TEXT TO DOCUMENT`](../commands-legacy/text-to-document.md)、[`Document to text`](../commands-legacy/document-to-text.md)、および [XML SET OPTIONS](../commands-legacy/xml-set-options.md) コマンドの詳細を参照してください。
