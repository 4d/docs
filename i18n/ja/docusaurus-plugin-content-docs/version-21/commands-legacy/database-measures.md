...
<!--REF #_command_.Database measures.Summary-->**Database measures** コマンドは、4Dデータベースエンジンイベントについての詳細な情報を取得します。<!-- END REF-->返される情報には、ディスクやメモリーキャッシュへの(もしくはからの)読み出し/書き込みアクセスに加え、データベースのインデックス、クエリ、並び替えの使用も含まれます。
