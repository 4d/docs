...
| defaultPath | Text, Integer | &#8594;  | デフォルトのパス名、 または、デフォルトのユーザフォルダーを表示する空の文字列 (Windowsでは"My documents"、 Mac OSでは"Documents")、 または記憶されたパス名の番号 |
| options | Integer | &#8594;  | Mac OS上での選択オプション |
| 戻り値 | Text | &#8592; | 選択されたフォルダーへのアクセスパス |
</div>
<!-- END REF-->
...
<!--REF #_command_.Select folder.Summary-->**Select folder** コマンドは、フォルダーを選択するダイアログボックスを表示し、フォルダーへの完全なアクセスパスを検索します。<!-- END REF-->オプションの引数*defaultPath*を使用して、フォルダーの場所を指定します。そのフォルダーは最初にフォルダー選択ダイアログボックスに表示されます。 

**Note:** このコマンドは、4Dのカレントフォルダーを変更しません。

**Select folder** コマンドはワークステーションのボリュームおよびフォルダー内をナビゲートするための標準のダイアログボックスを表示します。  
...
![](../assets/en/commands/pict3881985.en.png)
 ...
 引数*defaultPath*を使用して、フォルダー選択ダイアログボックスにあるデフォルトフォルダーの場所を表示します。3タイプの値をこの引数に渡します:  
* カレントプラットフォームのシンタックスを使用している有効なフォルダーのパス名。
* システムのデフォルトユーザフォルダーを表示する空の文字列(“”) (デフォルトユーザフォルダーは、Windowsでは"My documents” 、 macOSでは“Documents”になります) 。
* 記憶されたパス名の番号 (1から32,000) 。それに結び付いているフォルダーを表示します。これは、選択ボタンをクリックすると開いているフォルダーのパス名が記憶されることを意味します。つまり、 ユーザによって選択されたフォルダーを指します。このコマンドの最初の呼び出し時に任意の数字 (例えば5) を渡すと、コマンドはシステムのデフォルトユーザフォルダーを表示します(空の文字列を渡すことに対応する) 。ユーザはハードディスク上の複数のフォルダーをブラウズし、選択ボタンをクリックすると、パス名が番号5に結び付けられます。次に数値の5を渡してこのコ マンドを呼び出すと、記録されたパス名がデフォルトで使用されます。新しい場所が選択されると、5に対応するパスが更新されます。  
...
| 定数               | 型    | 値  | コメント                                                                                                                                                                                                                  |
| ---------------- | ---- | -- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Package open     | 倍長整数 | 2  | (Mac OSのみ): パッケージをフォルダーとして開きその内容を閲覧できます。この定数が使用されないと、コマンドはパッケージの開封を許可しません。                                                                                                                                             |
...
ユーザがフォルダーを選択して**フォルダーを選択**ボタン(Windows)または開くボタン(macOS)をクリックすると、フォルダーへのアクセスパスが返されます。
...
**Note:** macOSでは、ダイアログボックス内でフォルダーの名前を選択しているかどうかで、返ってくるパスは違うものになる場合があります。
...
次の例ではフォルダーを選択し、そこにピクチャライブラリ内のピクチャを保存する場合を考えます:
