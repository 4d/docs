...
ABORT コマンドはエラー処理プロジェクトメソッド内でのみ使用されるようになっていますが、4Dコミュニティには実行を中断するために他のプロジェクトメソッド で使用しているメンバーも存在します。これが動作するというのは単に副次的な効果に過ぎません。このコマンドをエラー処理メソッド以外のメソッド内で使用することはお勧めできません。
