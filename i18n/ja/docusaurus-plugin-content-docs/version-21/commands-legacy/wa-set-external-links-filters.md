...
* *allowDenyArr*配列のそれぞれ対応する要素には、URL をWeb エリアで表示する ([True](../commands/true)) かWeb ブラウザーで表示する ([False](../commands/false)) かを示すブール値が含まれます。
...
**重要**: [WA SET URL FILTERS](wa-set-url-filters.md "WA SET URL FILTERS")コマンドで設定されたフィルタが、WA SET EXTERNAL LINKS FILTERSコマンドよりも前に評価されます。つまりURLが[WA SET URL FILTERS](wa-set-url-filters.md "WA SET URL FILTERS")コマンドフィルタの設定により拒否されると、WA SET EXTERNAL LINKS FILTERSコマンドで受け入れていても、そのURL をブラウザーで開くことはできません (例2 参照)。

## 例題 1 

この例はサイトを外部ブラウザーで開きます: 
...
 APPEND TO ARRAY($filters;"*www.google.*") //Select "google"
 APPEND TO ARRAY($AllowDeny;False)
  //False: このリンクは外部ブラウザーで開かれるべきだが、この設定は
