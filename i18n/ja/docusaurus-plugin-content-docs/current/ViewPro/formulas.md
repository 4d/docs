...
4D View Pro allows you to reference 4D fields using the virtual structure of the database, i.e. declared through the [`SET TABLE TITLES`](../commands-legacy/set-table-titles) and/or [`SET FIELD TITLES`](../commands-legacy/set-field-titles) commands with the \* parameter. この代替方法は、アプリケーションがすでに仮想ストラクチャーに依存している場合に便利です (そうでない場合は、[`VP SET CUSTOM FUNCTIONS` の使用](#4dファンクション) が推奨されます)。
...
- The field must belong to the virtual structure of the database, i.e. it must be declared through the [`SET TABLE TITLES`](../commands-legacy/set-table-titles) and/or [`SET FIELD TITLES`](../commands-legacy/set-field-titles) commands with the \* parameter (see example),
