...
独自の 4Dコンポーネントを [開発](../Extensions/develop-components.md) し、[ビルド](../Desktop/building.md) することもできますし、4D あるいは4Dコミュニティによって公開されているパブリックなコンポーネントを [**4D Corner プラットフォーム**](https://corner.4d.com/) からダウンロードすることもできます。
...
[**4D Corner プラットフォーム**](https://corner.4d.com/) は、4D 社または4D コミュニティによって公開されたすぐに使える4D コンポーネントの包括的なリストを掲載しています。リストの中を閲覧または検索して、ご要望にあうコンポーネントをお探しください。

![4d-corner](../assets/en/Project/4d-corner.png)

依存関係マネージャーからを開くこともできます。[依存関係を追加ウィンドウ](#githubまたはgitlab依存関係を追加する) のトップから、あるいは依存関係マネージャーウィンドウの下部のメニューから**4D Corner でさらにコンポーネントを探す...** を選択することで開けます。

![4d-corner-access](../assets/en/Project/4d-corner-access.png)

4D Corner プラットフォームからコンポーネントをインストールするには、以下のようにします:

1. コンポーネントファイルの右上隅の"リンクをコピー"ボタンをクリック:

![copy button](../assets/en/Project/4d-corner-copy.png)

2. 依存関係マネージャーを開き、**[+]** ボタンをクリックします。

依存関係のリンクがアドレスエリアに自動的に追加されます:

![add dependency](../assets/en/Project/4d-corner-add.png)

3. [依存関係参照を設定](#githubまたはgitlab依存関係を追加する) し、**追加** をクリックします。
