...
コンパイルには[適切なライセンス](../Admin/licenses.md) が必要となります。ライセンスがない場合、コンパイルを実行することはできません (ボタンが無効になります)。その場合でも、シンタックスチェックと変数定義メソッドの生成はおこなうことができます。
...
アプリケーションの開発中に、ホストプロジェクトが使用しているコンポーネントを個別に開くことなく、ホストプロジェクトから直接コンパイルすることができます。ホストプロジェクトに一つまたはそれ以上の*対象となり得る* コンポーネント([要件](#要件) の項目を参照) が含まれている場合、コンパイラーウィンドウに追加のメニューが表示されます。このメニューを使用することでどのプロジェクトをコンパイルしたいかを選ぶことができます:

- ホストプロジェクトのみ(デフォルト)
- ホストプロジェクトの対象となり得る全てのコンポーネント
- 対象となり得るすべてのコンポーネントの一覧から一つのコンポーネントを対象として選択。

![](../assets/en/Project/compile-component.png)

:::note

エクスプローラーの**メソッド>コンポーネントメソッド** のセクションを選択してコンパイラーウィンドウを開くことができます: そこで対象となり得るコンポーネントを右クリックし、コンテキストメニューから**コンパイラー...** を選択します。

:::

コンパイル中のコンポーネントの一覧がコンパイラウィンドウ内に表示されます。コンポーネント内でエラーが検知された場合、そのコンパイルは停止されますがその他のコンポーネントに対しての処理は継続されます。

- コンポーネントに少なくとも一つのエラーがある場合には**太字**で表示され、エラーリストを展開して表示することができます。
- コンポーネントに少なくとも一つの警告がある場合には、警告のリストを展開して表示することができます。
- エラーのないコンポーネントは展開することができません。

#### 要件

ホストプロジェクトからのコンパイルの対象になり得るためには、コンポーネントは以下の要件に合致している必要があります:

- コンポーネントは[プロジェクトアーキテクチャー](../Project/architecture.md) を使用している
- コンポーネントの[インタープリタコード](../Concepts/interpreted.md) が利用可能であり、コンポーネントが[コンパイルモードで実行中](#run-compiled) ではない
- コンポーネントが[直接型指定](#直接型指定の有効化) 宣言を使用している。

:::note

追加のメニューは、対象となり得るコンポーネントがプロジェクトに含まれていない場合には表示されません。

:::

#### コンパイラー機能と設定

以下の、コンパイラーウィンドウで利用可能な機能はすべて、選択されたコンポーネントに対して適用されます: **コンパイル**、**シンタックスチェック**、**コンパイルコードを削除**。

コンポーネントが選択されている場合、[コンパイラー設定](../settings/) を編集することはできません(ボタンはグレーアウトされています)。

コンパイルに関してはそれぞれのコンポーネントの[コンパイラー設定](../settings/) が適用されますが、[エラーファイルとSymbol ファイルの生成オプション](#コンパイラー設定) は例外です: これに関してはホストプロジェクトの設定がコンポーネントのオプションを上書きします。これらのオプションが有効化されている場合、それぞれのコンポーネントの[error and warning file](#エラーファイルを生成) および[Symbol ファイル](#symbolファイルを生成) は[ホストプロジェクトの**Logs** フォルダ](../Project/architecture.md#logs) 内の専用のフォルダに保存されます(コンポーネント名がフォルダ名になります)。

コンパイルまたはシンタックスチェックを開始すると、処理されているすべてのプロジェクトがコンパイラーウィンドウに表示されます。エラーまたは警告が検知された場合、これらは対応するプロジェクトの下に階層リストとして表示されます。エラーが含まれるプロジェクトは**太字** で表示されます:
...
このボタンは、変換されたプロジェクトで、[コンパイルパスオプション](#直接型指定の有効化) に **自動変数定義は行わない (直接型指定)** が選択されていない場合にのみ表示されます。このボタンに関する情報については、[以前の 4Dリリースのドキュメント](../Project/compiler.md#型宣言を生成する) を参照ください。
...
これらのオプションは、従来のコードとの互換性のためにのみ維持されています。詳細については、[以前の 4Dリリースのドキュメント](../Project/compiler.md#コンパイラー設定) を参照ください。
