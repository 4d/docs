...
> データフォルダーが読み取り専用モードの場合やメンテナンスログファイルの保存には、システムのユーザー設定フォルダー (Active 4D Folder のこと、詳しくは [Get 4D folder](../commands-legacy/get-4d-folder) コマンド参照) 内にある追加の Logs フォルダーが利用されます。
