...
- マシンのアクティブ4Dフォルダー。 この場合、マクロはすべてのデータベースで共有されます。
  **注:** アクティブ4Dフォルダーの位置は OS により異なります。 詳細については、*4Dランゲージリファレンス* マニュアルの [Get 4D folder](../commands-legacy/get-4d-folder) コマンドの説明を参照ください。
...
| `<method_path/>`     | カレントのメソッドのパスシンタックス([`METHOD Get path`](../commands-legacy/method-get-path) から返されるもの)で置き換えられたタグ。                                                                                                                                                                                                                                                                                                                                                                       |
...
テキスト選択は、[GET MACRO PARAMETER](../commands-legacy/get-macro-parameter) および [SET MACRO PARAMETER](../commands-legacy/set-macro-parameter) コマンドを使用して管理することが推奨されています。 これらのコマンドを使用すると、ホストプロジェクトとコンポーネント間でテキストを相互に交換することができ、マクロの管理専用のコンポーネントを作成することができます。 マクロでこのモードを有効にするには、macro 要素内で version 属性に値 2 を指定しなければなりません。 この場合、4D は旧式の _textSel, _textReplace 等の定義済み変数を管理しなくなり、[GET MACRO PARAMETER](../commands-legacy/get-macro-parameter) と [SET MACRO PARAMETER](../commands-legacy/set-macro-parameter) が使用されます。 属性は以下のように定義します:
