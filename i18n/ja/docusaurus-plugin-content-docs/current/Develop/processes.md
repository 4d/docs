...
- [`New process`](../commands-legacy/new-process) コマンドを使用する。 [`New process`](../commands/new-process) コマンドの引数として渡されたメソッドがプロセスメソッドです。
- サーバー上ですトアドプロシージャーを作成するためには、[`Execute on server`](../commands-legacy/execute-on-server) コマンドを使用します。 コマンドの引数として渡されたメソッドがプロセスメソッドです。
- [`CALL WORKER`](../commands-legacy/call-worker) コマンドを使用する。 ワーカープロセスが既に存在していない場合、新たに作成されます。
...
- [`KILL WORKER`](../commands-legacy/kill-worker) コマンドを呼び出した場合(ただしこれで削除できるのはワーカープロセスのみです)。
...
あらゆるプロセスは [`CALL WORKER`](../commands-legacy/call-worker) コマンドを使用することでワーカープロセスを "雇用" することができ、ワーカーのコンテキストにおいて任意のプロジェクトメソッドを指定のパラメーターで実行させることができます。つまり、呼び出し元のプロセスとワーカーの間で情報の共有が可能です。

:::info

デスクトップアプリケーションにおいては、[`CALL FORM`](../commands-legacy/call-form) コマンドを使うことで、あらゆるフォームのコンテキストにおいて任意のプロジェクトメソッドを指定のパラメーターで実行させることができます。
...
ワーカープロセスを閉じるには [`KILL WORKER`](../commands-legacy/kill-worker) コマンドをコールします。これによってワーカーのメッセージボックスが空にされ、関連プロセスはメッセージの処理を停止し、現在のタスク完了後に実行を終了します。
