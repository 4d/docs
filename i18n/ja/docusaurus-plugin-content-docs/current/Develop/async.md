...
- [`CALL WORKER`](../commands-legacy/call-worker) executes the code for which it has been called, then returns to a listening status from where it can be called afterwards.
- [`CALL FORM`](../commands-legacy/call-form) opens a form and makes it listen for incoming messages from the event queue.
