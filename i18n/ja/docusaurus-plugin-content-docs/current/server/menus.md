...
このコマンドは[Maintenance and Security Center](../MSC/overview.md) (MSC)を開きます。ここにはデータベースの検証、解析、メンテナンス、バックアップ、圧縮を行うのに必要なすべてのツールが集められています。このコマンドは4D Server がプロジェクトを開いていない時でも使用できます。この場合、このコマンドを"メンテナンスモード"でプロジェクトを開くために使用できます (選択すると標準のファイルを開くダイアログが表示され、開くデータベースを選択できます)。メンテナンスモードは圧縮や破損したデータベースを開く際に使用されます。
