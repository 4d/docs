...
`On After Edit` イベント内において、入力テキストは [`Get edited text`](../commands-legacy/get-edited-text) コマンドによって返されます。
