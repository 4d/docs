...
| placeholder | オブジェクト |    | プレースホルダー行に適用されるデフォルトスタイルを格納したオブジェクト。                            |
