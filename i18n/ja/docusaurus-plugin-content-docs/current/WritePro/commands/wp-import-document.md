...
- 旧式の4D Write ドキュメント(.4w7 あるいは .4wt)。 旧式の4D Write ドキュメント(.4w7 あるいは .4wt)。 旧式の4D Write ドキュメント(.4w7 あるいは .4wt)。 4D Write Pro オブジェクトでもサポートされる4D Write 機能の詳細な一覧については、[4D Write ドキュメントの読み込み](../user-legacy/importing-4d-write-documents.md) の章を参照して下さい。
...
- .docx フォーマットのドキュメント。 詳細な情報に関しては、[.docx フォーマットでの読み込みと書き出し](../user-legacy/importing-and-exporting-in-docx-format.md) を参照して下さい。
