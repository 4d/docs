...
| 定数                   | 型    | 値 | コメント               |
| -------------------- | ---- | - | ------------------ |
| Delete only if empty | 倍長整数 | 0 | フォルダーの中身が空の場合のみ削除する |
| Delete with contents | 倍長整数 | 1 | フォルダーを中身ごと削除する      |

* Delete only if empty (0) を受け渡した場合、または *deleteOption* を省略した場合:  
   * *folder* パラメーターに指定したフォルダーは、空の場合に限り削除されます。空でない場合には削除処理は行わず、エラー -47 (ファイルが既に開かれている、あるいはフォルダーが空ではありません。) が発生します。  
