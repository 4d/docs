...
<!--REF #_command_.HTTP SET OPTION.Summary-->**HTTP SET OPTION**コマンドは ([HTTP Get](../commands/http-get)や[HTTP Request](../commands/http-request)コマンドで実行される次のリクエストでクライアントにより使用される) HTTPオプションの値を設定します。<!-- END REF-->設定するオプションの数だけこのコマンドを呼び出します。
