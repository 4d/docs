...
| 引数 | 型 |  | 説明 |
| --- | --- | --- | --- |
| destFolderPath | Text | &#8594; | 保存されているファイルの保存先フォルダーへのパス |
...
デフォルトで、*logPath* 引数が省略されている場合、コマンドはカレントログファイルを保存します。特定のログファイルを書き出したい場合、そのパスを *logPath* 引数に渡します。ログファイルは ".journal" 拡張子を持つファイルでなければなりません。アーカイブされたログファイル(.4bl)を書き出したい場合、[RESTORE](../commands/restore) コマンドを使用して事前に変換する必要があります。空の文字列("")を渡す事で標準のファイルを開くダイアログボックスを表示する事ができ、書き出すログファイルをユーザーに選択させることができます。選択されたログファイルのパスは**Document**システム変数に返されます。
