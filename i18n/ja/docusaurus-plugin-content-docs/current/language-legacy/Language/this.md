...
> コンストラクター内で [Super](../commands/super) キーワードを使ってスーパークラスのコンストラクターを呼び出す場合、必ず `This` より先にスーパークラスのコンストラクター を呼ぶ必要があることに留意してください。順番を違えるとエラーが生成されます。 こちらの [例題](super.md#例題-1) を参照ください。
...
[Formula](../commands/formula) あるいは [Formula from string](../commands/formula-from-string) コマンドで作成されたフォーミュラオブジェクトの実行コンテキストにおいては、`This` はフォーミュラによって現在処理されているオブジェクトへの参照を返します。
