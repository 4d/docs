...
**4D Server:** 4D v13より、このコマンドを4D Serverで実行することができるようになりました。この場合コマンドは、指定されたファイルを作成する前に、まずサーバー上で[QUIT 4D](../commands/quit-4d)を呼び出します (結果各リモートマシン上にサーバーが終了する旨通知するダイアログボックスが表示されます)。 
