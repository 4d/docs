...
* [On Exitデータベースメソッド](../commands/on-exit-database-method)がある場合、4Dはローカルプロセスを新しく作成し、その内でこのデータベースメソッドの実行を開始します。例えばこのデータベースメソッドを使用して、開発者は他のプロセスに、プロセス間通信を駆使し、 データ入力や[On Startupデータベースメソッド](../commands/on-startup-database-method)で開始された処理 (他のデータサーバーへの接続等) を停止するよう、通知できます。4Dは、いずれ終了してしまうことを覚えておいてください。[On Exitデータベースメソッド](../commands/on-exit-database-method)は、必要なすべてのクリーンアップ操作や終了を実行しますが、終了を拒否することはできないため、ある時点でデータベースは終了します。
* [On Exitデータベースメソッド](../commands/on-exit-database-method)がない場合、4Dは実行プロセスそれぞれを区別せずに1つずつアボートします。
...
* この処理を、[On Exitデータベースメソッド](../commands/on-exit-database-method)内で行う。
