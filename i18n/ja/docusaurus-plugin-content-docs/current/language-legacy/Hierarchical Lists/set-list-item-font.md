...
<!--REF #_command_.SET LIST ITEM FONT.Syntax-->**SET LIST ITEM FONT** ( * ; *list* : Text ; *itemRef* : 倍長整数, 演算子 ; *font* : Text, Integer )<br/>**SET LIST ITEM FONT** ( *list* : Integer ; *itemRef* : 倍長整数, 演算子 ; *font* : Text, Integer )<!-- END REF-->
...
*font* 引数には、使用するフォントの名前または番号を渡します。デフォルトフォントを設定するにはこの引数に空の文字列を渡します。

## 例題 

リストのカレント項目にTimes フォントを適用します:
