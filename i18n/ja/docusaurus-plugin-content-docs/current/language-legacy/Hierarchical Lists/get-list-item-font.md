...
<!--REF #_command_.Get list item font.Syntax-->**Get list item font** ( * ; *list* : Text ; *itemRef* : 倍長整数, 演算子 ) : Text<br/>**Get list item font** ( *list* : 倍長整数 ; *itemRef* : 倍長整数, 演算子 ) : Text<!-- END REF-->
...
**注意:** オブジェクト名に@文字を使用し、フォーム上にこれに合致するリストオブジェクトが複数ある場合、**Get list item font** コマンドは最初に見つけたリストオブジェクトを検索の対象とします。
