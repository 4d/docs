...
- セッション/情報:
  ...
  - Webプロセス - URLパス
- プロセス名
- プロセスの番号 (例えば [`New process`](../commands/new-process) コマンドなどから返される番号)。 プロセス番号はサーバー上で割り当てられる番号です。 グローバルプロセスの場合、この番号はクライアントマシン上で割り当てられた番号と異なる場合があります。
- プロセスの現在の状況
- 作成されてからのプロセスの実行時間 (秒)
...
> 確認ダイアログなしに選択したプロセスをアボートするには、**Alt**\*キーを押しながらこのボタンをクリックするか、[`ABORT PROCESS BY ID`](../commands/abort-process-by-id) コマンドを使用します。

- **プロセスを一時停止**: 選択したプロセスを一時停止します。
- **プロセスをアクティベート**: 選択したプロセスの実行を再開します。 The processes must have been paused previously (using the button above or by programming); otherwise, this button has no effect.
