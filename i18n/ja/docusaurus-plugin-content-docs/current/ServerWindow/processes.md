...
- セッション/情報:
  ...
  - Webプロセス - URLパス
- プロセス名
- プロセスの番号 (例えば [`New process`](../commands-legacy/new-process.md) コマンドなどから返される番号)。 プロセス番号はサーバー上で割り当てられる番号です。 グローバルプロセスの場合、この番号はクライアントマシン上で割り当てられた番号と異なる場合があります。
- プロセスの現在の状況
- 作成されてからのプロセスの実行時間 (秒)
...
> 確認ダイアログなしに選択したプロセスをアボートするには、**Alt**\*キーを押しながらこのボタンをクリックするか、[`ABORT PROCESS BY ID`](../commands-legacy/abort-process-by-id.md) コマンドを使用します。
