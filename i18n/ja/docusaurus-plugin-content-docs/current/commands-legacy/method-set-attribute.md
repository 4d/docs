...
| 引数 | 型 |  | 説明 |
| --- | --- | --- | --- |
| path | Text | &srarr; | プロジェクトメソッドのパス |
| attribType | Integer | &srarr; | 属性タイプ |
| attribValue | Boolean, Text | &srarr; | True: 属性を選択False: 属性の選択解除 |
| * | 演算子 | &srarr; | 指定時 = コンポーネントで実行されたとき、コマンドをホストデータベースに適用する (コンポーネントのコンテキスト以外ではこの引数は無視されます) |
