...
| 引数 | 型 |  | 説明 |
| --- | --- | --- | --- |
| tablePtr&#124; tableNum | ポインター, 倍長整数 | &rarr; | テーブルポインタ、またはテーブル番号 |
| invisible | Boolean | &larr; | True = 非表示、False = 表示 |
| trigSaveNew | Boolean | &larr; | True = トリガ“新規レコード保存時”が有効、 False = それ以外 |
| trigSaveRec | Boolean | &larr; | True = トリガ“既存レコード保存時”が有効、 False = それ以外 |
| trigDelRec | Boolean | &larr; | True = トリガ“レコード削除時”が有効、 False = それ以外 |
| trigLoadRec | Boolean | &larr; | ***使用しない (廃止) *** |
