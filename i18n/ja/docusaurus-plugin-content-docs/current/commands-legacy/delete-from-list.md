...
| list | Integer, Text | &srarr; | リスト参照番号 (* が省略された場合), または リストタイプオブジェクト名 (* を渡した場合) |
| itemRef &#124; * | 倍長整数, 演算子 | &srarr; | 項目参照番号, または 0 はリストに最後に追加された項目 または * 現在選択されているリスト項目 |
| * | Operator |  &srarr; | 指定した場合, サブリストがあればそれもメモリから消去 省略した場合, サブリストがあってもそれを消去しない |
