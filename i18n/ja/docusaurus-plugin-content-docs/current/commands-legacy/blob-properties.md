...
| 引数 | 型 |  | 説明 |
| --- | --- | --- | --- |
| Blob | Blob | &srarr; | 情報を取得するBLOB |
| compressed | Integer | &harr; | 0 = BLOBは圧縮されていない 1 = BLOBは圧縮率優先で圧縮されている 2 = BLOBは速度優先で圧縮されている |
| expandedSize | Integer | &harr; | 非圧縮時のBLOBのサイズ (バイト単位) |
| currentSize | Integer | &harr; | BLOBの現在のサイズ (バイト単位) |
