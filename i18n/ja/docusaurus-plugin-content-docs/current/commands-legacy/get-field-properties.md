...
| 引数 | 型 |  | 説明 |
| --- | --- | --- | --- |
| fieldPtr &#124; tableNum | ポインター, 倍長整数 | &rarr; | テーブル番号、またはフィールドポインタ |
| fieldNum | Integer | &rarr; | テーブル番号を渡した場合は、フィールド番号 |
| fieldType | Integer | &larr; | フィールドのタイプ |
| fieldLength | Integer | &larr; | 文字フィールドの場合、長さ |
| indexed | Boolean | &larr; | True = インデックス付き、False = インデックスなし |
| unique | Boolean | &larr; | True = 重複不可、 False = 重複あり |
