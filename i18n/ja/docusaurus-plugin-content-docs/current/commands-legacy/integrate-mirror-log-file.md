...
| 引数 | 型 |  | 説明 |
| --- | --- | --- | --- |
| pathName | Text | &srarr; | 統合されるログファイルの名前もしくはパス名 |
| operationNum | Real | &srarr; | 統合が開始されるオペレーションの番号 |
| &harr; | 最後に統合されたオペレーションの番号 |
| mode | Integer | &srarr; | 0=厳格な統合モード(デフォルトモード)、1=自動修復モード |
| errObject | Object | &harr; | 失われたオペレーション |
