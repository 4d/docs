...
| 引数 | 型 |  | 説明 |
| --- | --- | --- | --- |
| aTable | Table | &rarr; | フィールドタイトルを設定するテーブル |
| fieldTitles | Text array | &rarr; | ダイアログボックスに表示するフィールドの名前 |
| fieldNumbers | Integer array | &rarr; | 実際のフィールド番号 |
| * | Operator |  &rarr; | カスタマイズした名前をフォーミュラエディタで使用する |
