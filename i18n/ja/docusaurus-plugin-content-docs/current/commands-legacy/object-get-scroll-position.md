...
| object | any | &rarr; | オブジェクト名 (* 指定時) または <br/>変数、フィールドまたはテーブル (* 省略時) |
| vPosition | Integer | &larr; | 表示されている最初の行数、または ピクセル単位の縦スクロール (ピクチャ) |
| hPosition | Integer | &larr; | 表示されている最初の列数、または<br/>ピクセル単位の横スクロール (ピクチャ) |
