...
| 引数 | 型 |  | 説明 |
| --- | --- | --- | --- |
| objectField | Field | &rarr; | 重複しない属性値の一覧を取得したいオブジェクトフィールド |
| path | Text | &rarr; | 重複しない値を取得したい属性へのパス |
| valuesArray | Text array, Integer array, Boolean array, Date array, Time array | &larr; | 属性パス内の重複しない値 |
