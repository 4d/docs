...
| 引数 | 型 |  | 説明 |
| --- | --- | --- | --- |
| scriptPath | Text | &rarr; | 実行するSQLスクリプトが書かれたファイルの完全パス名 |
| errorAction | Integer | &rarr; | スクリプト実行中にエラーが発生した場合のアクション |
| attribName | Text | &rarr; | 使用する属性の名前 |
| attribValue | Text | &rarr; | 属性の値 |
