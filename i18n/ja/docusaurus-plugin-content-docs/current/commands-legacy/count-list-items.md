...
| 引数 | 型 |  | 説明 |
| --- | --- | --- | --- |
| * | 演算子 | &rarr; | 渡された場合, listはオブジェクト名 (文字列) 省略すると, listはリスト参照番号 |
| list | Integer, Text | &rarr; | リスト参照番号 (* が省略された場合), または リストタイプオブジェクト名 (* が渡された場合) |
| * | 演算子 | &rarr; | 省略すると (デフォルト): 表示されたリスト項目を返す (展開) 指定すると: すべてのリスト項目を返す |
