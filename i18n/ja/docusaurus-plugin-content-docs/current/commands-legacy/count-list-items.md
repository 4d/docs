...
| 引数 | 型 |  | 説明 |
| --- | --- | --- | --- |
| * | 演算子 | &srarr; | 渡された場合, listはオブジェクト名 (文字列) 省略すると, listはリスト参照番号 |
| list | Integer, Text | &srarr; | リスト参照番号 (* が省略された場合), または リストタイプオブジェクト名 (* が渡された場合) |
| * | 演算子 | &srarr; | 省略すると (デフォルト): 表示されたリスト項目を返す (展開) 指定すると: すべてのリスト項目を返す |
| 戻り値 | Integer | &larr; | (展開されて) 表示中のリスト項目数 (2番目の * が省略された場合) またはリスト項目の総数 (2番目の * が指定された場合) |
