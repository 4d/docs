...
| 引数 | 型 |  | 説明 |
| --- | --- | --- | --- |
| * | 演算子 | &srarr; | 指定時、objectはオブジェクト名 (文字列)。省略時、オブジェクトは変数またはフィールド |
| object | any | &srarr; | オブジェクト名 (*指定時)、または変数/フィールド (*省略時) |
| newText | Text | &srarr; | 挿入するテキスト |
| startSel | Integer | &srarr; | 選択の開始位置 |
| endSel | Integer | &srarr; | 選択の終了位置 |
