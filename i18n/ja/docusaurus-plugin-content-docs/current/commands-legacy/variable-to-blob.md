...
* オブジェクト ([C\_OBJECT](c-object.md)) またはコレクション ([C\_COLLECTION](c-collection.md)) を *variable* 引数に渡した場合、コマンドはその (参照ではなく) コピーをUTF-8のJSON形式でBLOBの中に保存します。オブジェクトまたはコレクションがポインターを含んでいた場合、ポインター自身ではなくそれが参照していた値が保存されます。
