...
| 引数 | 型 |  | 説明 |
| --- | --- | --- | --- |
| message | Text | &rarr; | ウィンドウのタイトル |
| defaultPath | Text, Integer | &rarr; | デフォルトのパス名、 または、デフォルトのユーザフォルダを表示する空の文字列 (Windowsでは"My documents"、 Mac OSでは"Documents")、 または記憶されたパス名の番号 |
| options | Integer | &rarr; | Mac OS上での選択オプション |
