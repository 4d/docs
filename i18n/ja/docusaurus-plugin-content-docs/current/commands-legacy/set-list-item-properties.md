...
| enterable | Boolean | &srarr; | TRUE = 入力可, FALSE = 入力不可 |
| styles | Integer | &srarr; | 項目のフォントスタイル |
| icon | Text, Integer | &srarr; | ピクチャー名または番号 (アイコンを使用しない場合は "" または 0) |
| color | Integer | &srarr; | RGBカラー値 または -1 = 元のカラーにリセット |
