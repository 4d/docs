...
| 引数 | 型 |  | 説明 |
| --- | --- | --- | --- |
| $1 | 倍長整数 | &harr; | ユーザーを識別するために4D Serverが内部的に使用するユーザーID |
| $2 | 倍長整数 | &harr; | 接続を識別するために4D Serverが内部的に使用する接続ID |
| $3 | 倍長整数 | &harr; | 廃止: 常に0が返されますが、宣言はしなくてはなりません。 |
| $0 | 倍長整数 | &larr; | 0または省略時 = 接続を受け入れる、0以外 = 接続を拒否する |
