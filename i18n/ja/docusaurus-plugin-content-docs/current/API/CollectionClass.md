...
| options      | 整数     |  -> | `ck diacritical`, `ck count values` |
...
| 引数    | 型      |     | 説明                                      |
| ----- | ------ | :-: | --------------------------------------- |
| depth | 整数     |  -> | ネストされたコレクションの階層をどの範囲まで平坦化するか。 デフォルト = 1 |
| 戻り値   | コレクション |  <- | 平坦化されたコレクション                            |
...
| startFrom | 整数  |  -> | 検索を開始するインデックス                  |
| 戻り値       | ブール |  <- | *toSearch* がコレクションにある場合は true。 |
...
| ---------- | --------------------------- | :-: | ------------------------------------------------------------------------------------------------------------------------------------------- |
| formula    | 4D.Function |  -> | フォーミュラオブジェクト                                                                                                                                |
| colsToSort | コレクション                      |  -> | コレクションのコレクション、または {`collection`:*colToSort*;`order`:`ck ascending` / `ck descending`} プロパティを持つオブジェクトのコレクション |
