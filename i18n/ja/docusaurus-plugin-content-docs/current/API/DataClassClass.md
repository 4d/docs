...
*attributePath* が[**ベクトルオブジェクト**](../API/VectorClass.md) を格納している属性を指定している場合([4D フィールドに4D.Vector クラスオブジェクトを保存するように設定する](../Develop/field-properties.md#class) 参照) 、キーワードではなく**埋め込み**に基づいてエンティティを探すクエリをビルドすることができます。 This technology is designed for Artificial Intelligence (AI) workloads and allows you to query data based on semantics, rather than keywords.
