...
| 2   | **ブール**    |                                                                           | 論理順: false がtrue の**前**になります                                                       |
| 3   | **文字列**    |                                                                           | 辞書式順序 (例: "a" *の次に* "ab"、 *その次に* "b")                           |
| 4   | **数値**     | 時間 (`Time inside objects` データベース設定に応じて、ミリ秒または秒に変換されます) | 標準的な代数的順序 (数値比較)                                                                |
| 5   | **オブジェクト** | Blob、ピクチャー、ヌルでないポインター(コレクション)                          | 内部的な順序(コレクション関数と同じ、以下参照)                                                        |
| 6   | **コレクション** |                                                                           | 内部的な順序(コレクション関数と同じ、以下参照)                                                        |
