...
1. Welcome ウィザードにて **ローカルアプリケーションプロジェクトを開く** を選択します。<br/> または<br/> **ファイル** メニューより **開く ＞ ローカルプロジェクト...** を選択するか、**開く** ツールバーボタンより同様に選択します。 The standard Open dialog appears.

2. プロジェクトの `.4dproject` ファイルを選択し、**開く** をクリックします。 By default, the project is opened with its current data file. Other file types are suggested:
...
Since the editors use files on the disk, potential conflicts could happen if the same file is modified or even deleted from different locations. たとえば、1つのメソッドをメソッドエディターで編集しつつ、標準のテキストエディターでも開いて変更した場合に競合が起こりえます。
