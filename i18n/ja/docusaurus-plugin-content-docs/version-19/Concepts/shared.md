...
共有オブジェクトや共有コレクションを変更するすべての [コレクション関数](../API/CollectionClass.md) は利便性のため、内部的に `Use...End use` 構文を使用します。 例:
...
同じコレクションに対して複数の変更を実行する必要がある場合、すべての修正を 1つの `Use...End use` で保護することができます。
