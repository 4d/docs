...
タブコントロールの[データソース](properties_Object.md#変数あるいは式) として、[コレクション](Concepts/dt_collection.md) を格納した[オブジェクト](Concepts/dt_object.md) を割り当てることができます。 このオブジェクトには、次のプロパティが格納されていなくてはなりません:
