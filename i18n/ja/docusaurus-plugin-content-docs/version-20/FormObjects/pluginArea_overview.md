...
アプリケーションを開くと、そのアプリケーションに[インストールされているプラグイン](../Concepts/plug-ins.md#プラグインのインストール)のリストが内部的に作成されます。 プラグインエリアをフォームに挿入すると、そのプロパティリスト内の **タイプ** リスト ("オブジェクト"テーマ内) にて、プラグインを割り当てることができます:
