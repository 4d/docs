...
オブジェクト型フィールドに対して処理をおこなうには `QUERY BY ATTRIBUTE`、`QUERY SELECTION BY ATTRIBUTE` や `ORDER BY ATTRIBUTE` など、**クエリ** テーマの特定のコマンドも使用することができます。 オブジェクト型の変数・フィールド・式を操作するには標準の [オブジェクト記法](プロパティ) を用いるか、**オブジェクト (ランゲージ)** テーマが提供するコマンドを使用します。
...
いくつかの 4Dコマンドや関数はオブジェクトを返します。 たとえば、[`Get database measures`](https://doc.4d.com/4Dv19R7/4D/19-R7/Get-database-measures.301-5945423.en.html) や [`File`](../API/FileClass.md#file) などです。 この場合、オブジェクトを明示的にインスタンス化する必要はなく、4Dランゲージが代わりにおこなってくれます。
...
- 共有オブジェクトへのアクセスは `Use...End use` 構造によって管理されています。 [`New shared object`](https://doc.4d.com/4Dv20/4D/20/New-shared-object.301-6237617.ja.html) コマンドを使用して作成する共有オブジェクト。 共有オブジェクトはプロセス間 (プリエンティブ・スレッド含む) で共有可能なオブジェクトです。 詳細な情報については、[共有オブジェクトと共有コレクション](shared.md) を参照ください。
