...
| settings.4DSettings     | *ストラクチャー*データベース設定。 **警告**: コンパイル済みアプリケーションの場合、ストラクチャー設定は読み取り専用の .4dz ファイルに格納されます。 *[ユーザー設定](#settings-ユーザー)* または *[データファイル用のユーザー設定](#settings-ユーザーデータ)* が定義されている場合は、そちらの設定が優先されます ([設定の優先順位](../Desktop/user-settings.md#設定の優先順位) も参照ください)。 運用時にカスタム設定を定義するには、[ユーザー設定を有効化](../Desktop/user-settings.md#ユーザー設定の有効化) し、*ユーザー設定* または *データファイル用のユーザー設定* を使う必要があります。 | XML  |
