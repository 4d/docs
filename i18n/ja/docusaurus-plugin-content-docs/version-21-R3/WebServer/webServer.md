...
リモート4D においてWeb サーバーを使用する機能は4D 21 においてと **廃止予定** なっています。 この機能を使用することは今後推奨されません。 この機能を使用することは今後推奨されません。
...
- **公開するコンテンツの管理**: 明示的に公開した要素のみが、Web や RESTリクエストで直接利用できます。 次のものについて、宣言する必要があります: 次のものについて、宣言する必要があります:
...
4D Webサーバーの構成は、アプリケーションレベルの包括的な設定によって定義されます。この設定は、`webServer` オブジェクトのプロパティまたは [`WEB SET OPTION`](../commands/web-set-option) コマンドを使用して、セッション毎にカスタマイズすることもできます。
