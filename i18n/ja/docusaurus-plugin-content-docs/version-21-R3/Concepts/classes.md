...
`cs` コマンドは、<!-- REF #_command_.cs.Summary -->カレントプロジェクトまたはコンポーネントのユーザークラスストアを返します<!-- END REF -->。   開いているプロジェクトまたはコンポーネント内で[定義されている](../Project/code-overview.md#creating-classes) 全てのユーザークラスを返します。 デフォルトでは、 [ORDAクラス](ORDA/ordaClasses.md) のみ利用可能です。
