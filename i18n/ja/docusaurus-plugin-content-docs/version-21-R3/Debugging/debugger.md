...
- デバッガーウィンドウは、ブレークポイントがあったために表示されました。
...
現在のメソッド行 (プログラムカウンターと呼ばれる黄色い矢印で示されている行) が実行されます。その後、デバッガーは次の行に移動します。
...
デバッガーウィンドウの現在の状態がプロジェクトに自動的に保存されます。構成には次の内容が含まれます:

- ウィンドウのサイズと位置
- 分割線の位置
- [表示モード](#表示モード)
- カスタムのウォッチエリアに現在表示されている式デフォルトでは、カレントメソッドと関数に対しては式は保存されています。[**式を固定**](#pinning-an-expression) することで、全てのコンテキストにおいてその式を表示させることができます。
...
他の変数と同様に、配列はそのスコープによって、インタープロセス、プロセス、およびローカルサブテーマに表示されます。デバッガーは最初の 100要素を表示します。**値** 欄で配列要素の値を変更することは可能ですが、配列のサイズを修正することはできません。
...
- **アクティビティモニターを有効にする**: 動作のモニタリング (アプリケーション内部の詳細チェック) を有効にし、追加テーマ (**スケジューラー**、**Web**、**ネットワーク**) に情報を表示します。
...
カスタムウォッチペインの[コンテキストメニュー](#contextual-menu) から **表示** オプションを使用することで、すべてのデバッガーウィンドウで使用される表示モードを選択することができます。
...
評価したい任意の式を入力することができます。カスタムの式は [式を固定化](#pinning-an-expression) した場合を除き、カレントのデバッガーのウィンドウにしか表示されません。
...
- カスタムウォッチペインの[コンテキストメニュー](#コンテキストメニュー) 内の**新しい式...** を選ぶことで**フォーミュラエディター** を使用して式を追加することができます。値を返すフォーミュラであれば、なんでも追加できます。
...
ピン留めされた式はその後すべてのデバッガーウィンドウに表示されるようになります。
...
- **表示**: すべてのデバッガーウィンドウで使用する[表示モード](#表示モード) を選択します。
