...
**Note**: **On Exitデータベースメソッド**は、4D Serverでは起動されません。 このコンテキストにおいては[On Server Shutdownデータベースメソッド](../commands/on-server-shutdown-database-method) を使用してください。
...
* [QUIT 4D](../commands/quit-4d) コマンドへの呼び出しが実行された場合
* 4Dプラグインから[QUIT 4D](../commands/quit-4d) エントリポイントへの呼び出しが実行された場合
...
4D は生きているプロセスがある限り、プロセスをループし続けます。これは最大13秒間続きます。終了アクションに対応するためには、On Exitデータベースメソッド またはカスタムの終了メソッド内において(例えば[Storage](../commands/storage) オブジェクトへのエントリーなど)フラグを設定することが推奨されます。実行中のプロセスが終了アクションに対応し、プロセス自身を適切に終了させる時間を稼ぐためには、このメソッド内において(最大10秒間の)ループを行うのが理にかなっていると言えます。10秒以上待つことは推奨されておらず、この場合には4D はプロセスを強制終了させるか、あるいはOS がアプリケーション全体を強制終了させます。
...
**注:** クライアント/サーバー環境においては、ユーザーが手動で(**終了**メニューコマンドか[QUIT 4D](../commands/quit-4d) コマンドを呼び出すかで)終了するか、4D Server がシャットダウン(その結果全てのクライアントが強制終了)するかによって、振る舞いが異なります。  
...
以下の例は、[On Startupデータベースメソッド](../commands/on-startup-database-method) (あるいは[On Server Startupデータベースメソッド](../commands/on-server-startup-database-method)) 内において、無限に実行し続ける1つまたは複数のバックグラウンドプロセスを立ち上げるような場合の典型例を考えます。アプリケーション内の[Storage](../commands/storage) オブジェクト内にあるフラグを使用して、これらのプロセスに終了することを伝えると同時に、きちんと終了したかどうかをチェックするようになっています。

* [On Startupデータベースメソッド](../commands/on-startup-database-method) 内
...
[On Startupデータベースメソッド](../commands/on-startup-database-method)  
