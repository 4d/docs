...
* *langID* には [SPELL SET CURRENT DICTIONARY](../commands/spell-set-current-dictionary) コマンドによって自動的に生成され使用されるID番号が返されます。  
