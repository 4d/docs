...
| 引数 | 型 |  | 説明 |
| --- | --- | --- | --- |
| type | Integer | &#8594; | システムフォルダーのタイプ |
| 戻り値 | Text | &#8592; | システムフォルダーへのパス名 |
</div>
<!-- END REF-->
...
<!--REF #_command_.System folder.Summary-->**System folder**コマンドは、アクティブなWindows またはmacOS システムフォルダーにあるシステムフォルダーへのパス名、あるいはアクティブなWindows またはmacOS システムフォルダー自体へのパス名を返します。<!-- END REF-->

オプションの引数*type* には、システムフォルダーのタイプを示す値を渡します。  
...
| Fonts                         | 倍長整数 | 1  |                                                |
| Home folder                   | 倍長整数 | 18 | ユーザーのカレントのホームフォルダー(通常は "/Users/<username>/" です) |
...
* いくつかのシステム・フォルダーへのパス名はログインユーザ(現在のユーザ) のものを返します。 定数2～9を使用する場合、すべてのユーザで共有されるフォルダーへのパス名か、またはカレントユーザーのカスタマイズされたフォルダーへのパス名を使用するかを選択することが出来ます。

*type* 引数を省略すると、コマンドはアクティブなシステムフォルダー (定数= System )のパス名を返します。
