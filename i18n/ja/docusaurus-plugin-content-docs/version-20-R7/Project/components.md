...
コンポーネントが [GitHub のプライベートリポジトリ](#プライベートリポジトリ) に保存されている場合には、パーソナルアクセストークン (personal access token) を依存関係マネージャーに提供する必要があります。 これをおこなうには: これをおこなうには: これをおこなうには: これをおこなうには: これをおこなうには: これをおこなうには:
