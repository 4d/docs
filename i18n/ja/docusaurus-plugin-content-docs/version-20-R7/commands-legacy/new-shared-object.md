...
引数を何も渡さなかった場合、**New shared object** は空のオブジェクトを作成し、その参照を返します。この参照は必ず*C\_OBJECT* コマンドで宣言された4D 変数に割り当てる必要があります。

**注:** *C\_OBJECT* コマンドはObject 型の変数を宣言しますが、オブジェクトは作成しません。
