...
> - シングルユーザーの 4D では、テスト目的で RESTセッションを 3つまで開くことができます。 特定のセッションを継続利用するには [セッションcookie](authUsers.md#セッションcookie) を管理する必要があります。
...
> この設定を使用するには、`On REST Authentication` データベースメソッドを定義してはいけません。 これが定義されている場合は、データベース設定にて指定したアクセス設定は無視されます。
...
2. フィールドの **RESTリソースとして公開** オプションの選択を解除します: ![alt-text](../assets/en/REST/field.png) 公開設定を変更する各フィールドに対してこの手順を繰り返します。 ![alt-text](../assets/en/REST/field.png) 公開設定を変更する各フィールドに対してこの手順を繰り返します。
