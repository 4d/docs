...
* Delete with contents (1) を受け渡した場合:  
   ...
   * フォルダーや格納要素を削除できない場合、最初の削除不能な要素を検知した時点で処理は中断され、エラー**\*** が返されます。この場合、フォルダーは一部削除済みの可能性があります。処理が中断された場合には、[Last errors](../commands/last-errors.md)  を使って、問題となったファイルの名称とパスを取得できます。  
