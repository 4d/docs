...
* *standalone*: ツリーがスタンドアロン ([True](../commands/true)) か、それが他のファイルや外部リソースを処理のために必要とするか ([False](../commands/false)) を指定します。デフォルトで (コマンドが呼び出されないか引数が省略されると) 、ツリーはスタンドアロンではありません。
