...
<!--REF #_command_.WP New style sheet.Syntax-->**WP New style sheet** ( *wpDoc* : Object ; *styleSheetType* : Integer ; *styleSheetName* : Text ) -> 戻り値<!-- END REF-->
...
*styleSheetName* 引数にはスタイルシートの名前を渡します。このスタイルシート名はドキュメントとともに保存され、再利用や編集が容易にできます。またスタイルシート名は[WP Get style sheet](../WritePro/commands/wp-get-style-sheet) および [WP DELETE STYLE SHEET](../WritePro/commands/wp-delete-style-sheet) コマンドで使用することもできます。スタイルシートの名前は以下のルールに準拠している必要があります:
