...
このプロパティは、ユーザーがフォームの高さをサイズ変更したときに、当該オブジェクトが縦方向に移動するかリサイズするかを指定します。 また、[`OBJECT SET RESIZING OPTIONS`](../commands/object-set-resizing-options) コマンドによって動的に設定することもできます。 また、[`OBJECT SET RESIZING OPTIONS`](../commands-legacy/object-set-resizing-options.md) コマンドによって動的に設定することもできます。
