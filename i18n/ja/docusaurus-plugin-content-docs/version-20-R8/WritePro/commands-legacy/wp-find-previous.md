...
<!--REF #_command_.WP Find previous.Syntax-->**WP Find previous** ( *targetObj* ; *searchBefore* ; *searchValue* ; *searchCondition* {; *replaceValue*} ) -> 戻り値<!-- END REF-->
...
* レンジ
* 要素(表 / 行 / セル / 段落 / 本文 / ヘッダー / フッター / セクション / サブセクション )
* 4D Write Pro ドキュメント

*searchBefore* 引数にはレンジを渡します。検索は定義されたレンジの直前から開始されます。
...
 
  // (カレントのセレクションに基づいて)"alpha" 文字列について検索、または"前に"オカレンスをローンチする
