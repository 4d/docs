...
*dataPath*は、4 Dデータファイル(.4DD)を指定します。*structurePath*パラメータによって定義された構造ファイルに対応する必要があります。現在開いているデータファイルを確認するには、[VERIFY CURRENT DATA FILE](../commands/verify-current-data-file)コマンドを使用します。
