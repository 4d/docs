...
<!--REF #_command_.RESUME INDEXES.Summary-->**RESUME INDEXES** コマンドは、 [PAUSE INDEXES](../commands/pause-indexes) コマンドを使用して停止させた *aTable* のインデックスを全て再起動させます。<!-- END REF-->のインデックスが停止されていない場合、コマンドは何もしません。
...
**RESUME INDEXES** コマンドは4Dサーバーまたはローカルの4Dからのみ呼び出すことが出来ます。このコマンドがリモートの4Dマシンから呼び出されたときは、エラー -10513が生成されます。このエラーは [ON ERR CALL](../commands/on-err-call)コマンドによって実装されたメソッドを使用して割り込むことができます。
