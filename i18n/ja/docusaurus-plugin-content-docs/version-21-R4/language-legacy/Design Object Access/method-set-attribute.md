...
| Attribute folder name        | 倍長整数 | 1024 | メソッドのためのフォルダー名(「フォルダー」属性)。この定数を渡した場合、フォルダー名を *attribValue* に渡す必要があります。<br/>この名前が有効なフォルダーに対応する場合、メソッドはその親フォルダーに置かれます。フォルダーが存在しない場合、コマンドは親フォルダーの階層は何も変更しません。空の文字列を渡した場合、メソッドはルート階層に置かれます。 |
...
* Attribute folder name 定数を *attribType* に使用した場合は文字列(フォルダー名)を渡します。
