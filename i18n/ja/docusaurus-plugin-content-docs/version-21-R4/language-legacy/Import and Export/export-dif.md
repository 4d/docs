...
デフォルトでは、コマンドはデフォルトでUTF-8文字セットを使用します。DIFフォーマットドキュメントは一般的にIBM437文字セットを使用するため、適切な文字セットを指定するために[USE CHARACTER SET](../commands/use-character-set)コマンドを使用する必要があるでしょう。
