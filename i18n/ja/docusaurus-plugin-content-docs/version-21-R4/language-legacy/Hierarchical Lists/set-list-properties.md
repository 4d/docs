...
**注:** リスト内の各項目ごとのアイコンをカスタマイズしたい場合には、[SET LIST ITEM PROPERTIES](../commands/set-list-item-properties) コマンドを使用してください。
...
* プログラムを使用する場合、[SELECT LIST ITEMS BY POSITION](../commands/select-list-items-by-position) および[SELECT LIST ITEMS BY REFERENCE](../commands/select-list-items-by-reference) コマンドを使用します。  
