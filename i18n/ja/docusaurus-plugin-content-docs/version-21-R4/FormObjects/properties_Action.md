...
アクティブオブジェクトにより実行される典型的な処理 (例: レコードの入力・取り消し・削除、レコード間の移動、マルチページフォームでのページ間の移動、など)  は、4D より標準アクションとして提供されています。 They are described in detail in the [Standard actions](/Desktop/standard-actions) section of the *Design Reference*. また、[4D Write Pro エリア専用の標準アクション](../WritePro/user-legacy/standard-actions.md) もあります。
