...
サブフォームとして表示される出力フォーム、あるいは `DISPLAY SELECTION` や `MODIFY SELECTION` コマンドを使用して表示される出力フォームのヘッダーエリアにアクティブオブジェクトを配置して使用することもできます。 以下のようなアクティブオブジェクトを配置できます: 以下のようなアクティブオブジェクトを配置できます: 以下のようなアクティブオブジェクトを配置できます: 以下のようなアクティブオブジェクトを配置できます: 以下のようなアクティブオブジェクトを配置できます: 以下のようなアクティブオブジェクトを配置できます: 以下のようなアクティブオブジェクトを配置できます:
...
| 名称           | データタイプ                                                | とりうる値                                                                                                                 |
| ------------ | ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| markerHeader | integer &#x7c; integer collection | ヘッダーマーカーの位置 (ピクセル単位)、またはヘッダーマーカー位置のコレクションを指定します。 <br/>最小値: 0最小値: 0 |
