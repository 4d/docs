...
値がBLOB なのかオブジェクトなのかを判断するためには、[Value type](../commands/value-type) コマンドを使用します。オブジェクトがBlob オブジェクト(`4D.Blob`) であるかどうかをチェックするためには、[OB instance of](../commands/ob-instance-of) コマンドを使用します:
