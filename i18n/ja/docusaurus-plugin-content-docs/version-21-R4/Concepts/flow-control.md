...
*begin* 引数が負の値の場合、それは `begin:=begin+Expression のサイズ` として再計算されます (つまり、*Expression* の終端からのオフセットであるとみなされます)。再計算された値も負の値だった場合、*begin* は0に設定されます。**注:** *begin* が負の値だったとしても、繰り返しそのものは標準の順番で実行されます。
...
[`defer`](../commands/defer) コマンドを使用すると、カレントのメソッドまたは関数が **実行を終了したとき** に自動的に実行される一つまたはそれ以上の式をスタックすることができます。

管理しようとしているのがドキュメントの閉じ処理、インタープロセスのフラグのリセット、リソースの解放などであろうと、`defer` キーワードを使用すれば関数がどこで停止しようともこういったタスクが確実に実行されるようにすることができます。

```4d
   // 終了時に特定のコードが確実に実行される様にする
defer(myCleaningMethod)
   // 何かを行う...
```

より詳細な情報については[`defer`](../commands/defer) コマンドの詳細を参照してください。