...
表に対して [フォーミュラオブジェクト](../../API/FormulaClass.md) を **datasource** として割り当て、表内から特定の [`This` を使用した式](#表フォーミュラオブジェクト) を使用することで計算された値にアクセスすることができます。データソースのフォーミュラは、[フォーミュラが計算される](../managing-formulas.md#formula-evaluation) たびに(例: ドキュメントが開かれたとき、[WP COMPUTE FORMULAS](../commands/wp-compute-formulas) コマンドが呼ばれたとき、など)4D Write Pro によって処理されます。 

### データソースの割り当て

表にデータソースを割り当てる場合、[WP SET ATTRIBUTES](../commands/wp-set-attributes) コマンドに属性として `wk datasource` 定数を使用し、データソースオブジェクトを値として渡します。データソースオブジェクトは以下のいずれかです:

- [**4D フォーミュラ**](../../API/FormulaClass.md)。例えば、フランスに住む全ての人のデータを行に表示するには:
...
- **データコンテキスト**。[WP SET DATA CONTEXT](../commands/wp-set-data-context) コマンドを使用してドキュメント全体に定義されます。データコンテキストは `This.data` オブジェクトを通して表に割り当てることができます。例えば、注文から[エンティティセレクション](../../API/EntitySelectionClass.md)で表を埋めるには:
...
表内のフォーミュラ内で使用された場合、[`This`](../commands/this.md) キーワードはコンテキストに応じて追加の[式](../managing-formulas.md#formula-context-object)へのアクセスを提供します:
