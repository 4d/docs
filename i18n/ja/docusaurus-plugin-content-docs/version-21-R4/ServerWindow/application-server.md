...
- **状況**: 開始または停止
- **開始時刻**: アプリケーションサーバーの起動日と時刻。 これは、4D Server によってプロジェクトが開かれた日付です。 これは、4D Server によってプロジェクトが開かれた日付です。
...
> [`REJECT NEW REMOTE CONNECTIONS`](../commands/reject-new-remote-connections) コマンドを使用することで同じアクションを実行することもできます。
...
- **最高**: アプリケーションサーバーに許可された最大のクライアント同時接続数を表します。 この値は、サーバーマシンにインストールされているライセンスによります。 この値は、サーバーマシンにインストールされているライセンスによります。
