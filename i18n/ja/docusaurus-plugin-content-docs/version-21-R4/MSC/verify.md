...
要求された検査に関係なく、4D はアプリケーションの `Logs` フォルダーにログファイルを生成します。 このファイルには実行された検査の内容が記録され、エラーがあればそれも示されます。問題がない場合は [OK] が表示されます。 このファイルは XML形式で、ファイル名は *ApplicationName***Verify_Log***yyyy-mm-dd hh-mm-ss*.xml となり、それぞれ以下の要素が入ります: このファイルには実行された検査の内容が記録され、エラーがあればそれも示されます。 問題がない場合は [OK] が表示されます。 このファイルは XML形式で、ファイル名は *ApplicationName***Verify_Log***yyyy-mm-dd hh-mm-ss*.xml となり、それぞれ以下の要素が入ります:
