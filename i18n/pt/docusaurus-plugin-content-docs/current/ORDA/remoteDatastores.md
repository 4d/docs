...
- Classic 4D language commands ([`START TRANSACTION`](../commands-legacy/start-transaction), [`VALIDATE TRANSACTION`](../commands-legacy/validate-transaction), [`CANCEL TRANSACTION`](../commands-legacy/cancel-transaction)) only apply to the main datastore (returned by `ds`).
