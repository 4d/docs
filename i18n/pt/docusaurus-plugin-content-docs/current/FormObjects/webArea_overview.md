...
The [`4D embedded web rendering engine`](properties_WebArea.md#use-embedded-web-rendering-engine) provides a **JavaScript object named `$4d`** in the web area. Por padrão, '$4d' permite acesso a todos os métodos projeto 4D usando notação de ponto.
...
- **Menu contextual**: é possível usar o [menu contextual](properties_Entry.md#context-menu) padrão do sistema com a área web. A exibição do menu de contexto pode ser controlada com o comando [`WA SET PREFERENCE`](../commands/wa-set-preference).
...
Para obter mais informações, consulte a descrição do comando [`WA SET PREFERENCE`](../commands/wa-set-preference).
