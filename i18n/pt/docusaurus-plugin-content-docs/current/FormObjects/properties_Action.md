...
Atividades típicas a serem executadas por objetos ativos (\*e.g., permitindo que o usuário aceite, cancele ou exclua registros, mova entre registros ou de página para página em um formulário multi-páginas, etc.) foi predefinido por 4D como ações padrão. They are described in detail in the [Standard actions](/Desktop/standard-actions) section. There are also [standard actions specific to 4D Write Pro areas](../WritePro/user-legacy/standard-actions.md).
