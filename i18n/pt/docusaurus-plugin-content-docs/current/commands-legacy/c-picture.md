...
<!--REF #_command_.C_PICTURE.Syntax-->**C_PICTURE** ( {*metodo* ;} *variavel* {; *variavel2* ; ... ; *variavelN*} )<!-- END REF-->
...
<!--REF #_command_.C_PICTURE.Summary-->O comando **C\_PICTURE** atribui o tipo Imagem a cada variável especificada.<!-- END REF-->
...
A sintaxe **C\_PICTURE**(${...}) permite declarar um número variável de parâmetros do mesmo tipo, sob a condição que esses são os últimos parâmetros par ao método. Por exemplo, a declaração **C\_PICTURE**(${5}) diz a 4D e ao compilador que inicie com o quinto parâmetro, o método pode receber um número variável desse tipo. Para saber mais, veja a seção *Indireções sobre os parâmetros*.
