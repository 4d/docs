...
<!--REF #_command_.Drop position.Syntax-->**Drop position** {( colNum|posYimagem )} -> Resultado<!-- END REF-->
<!--REF #_command_.Drop position.Params-->
| Parâmetro | Tipo |  | Descrição |
| --- | --- | --- | --- |
| colNum&#124;posYimagem | Inteiro longo | &harr; | Número de coluna de list box (-1 se soltar antes da última coluna) ou  posição da coordenada Y da imagem |
| Resultado | Integer | &larr; | • Número (array/list box) ou • Posição (lista hierárquica) ou • Posição em string (texto/combo box) de item destino ou • -1 se soltar antes do último elemento do array ou item da lista • Posição da coordenada X da imagem |
