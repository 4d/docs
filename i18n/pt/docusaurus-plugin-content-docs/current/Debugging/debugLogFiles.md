...
Aplicações 4D podem gerar vários arquivos de histórico ou log úteis para depuração e otimizar sua execução. Os logs geralmente são iniciados ou parados usando seletores dos comandos [SET DATABASE PARAMETER](../commands-legacy/set-database-parameter), [WEB SET OPTION](../commands-legacy/web-set-option), ou [HTTP SET OPTION](../commands-legacy/http-set-option) e são armazenados na [pasta Logs](../Project/architecture.md#logs) do projeto.
...
| write\_duration                                                                  | Tempo em microssegundos para enviar a:<li>Solicitação (quando executado no cliente). A a B na imagem abaixo.</li><li>Resposta (quando executada no servidor). E a F na imagem abaixo.</li>                                                                                                                                                                    |
...
Esse arquivo registra vários eventos relacionados à operação de aplicação e sua leitura humana. Você pode incluir informações personalizadas nesse arquivo usando o comando [LOG EVENT](../commands-legacy/log-event).
...
- The "state" property values are described in the corresponding commands: `[`WEB SET OPTION`](../commands-legacy/web-set-option) (`Web log recording`), [`HTTP SET OPTION`](../commands-legacy/http-set-option) (`HTTP client log`), [`SET DATABASE PARAMETER`](../commands-legacy/set-database-parameter) (`Client Web log recording`, `IMAP Log\\\\\\\\\`,...).
- Para httpDebugLogs, a propriedade "level" corresponde às opções constantes do `wdl` descritas no comando [`WEB SET OPTION`](../commands-legacy/web-set-option).
- For diagnosticLogs, the "level" property corresponds to the `Diagnostic log level` constant values described in the [`SET DATABASE PARAMETER`](../commands-legacy/set-database-parameter) command.
