...
Quaalquer atributo de dataclass pode ser usado como um apropriedade de uma seleção de entidade a retornar <!-- REF EntitySelectionClass.attributeName.Summary -->Quaalquer atributo de dataclass pode ser usado como um apropriedade de uma seleção de entidade a retornar<!-- END REF -->. Também pode criar seleções de entidades em branco utilizando a função [`dataClass.newSelection()`](DataClassClass.md#newselection) ou o comando [`Create new selection`](#create-new-selection).
...
A função `.average()` <!-- REF #EntitySelectionClass.average().Summary -->retorna a média aritmética (média) de todos os valores não nulos de *attributePath* na seleção de entidade<!-- END REF -->.
...
A função `.first()` <!-- REF #EntitySelectionClass.first().Summary -->retorna uma referência para a entidade na primeira posição da seleção de entidade<!-- END REF -->.
...
A função `.isOrdered()` <!-- REF #EntitySelectionClass.isOrdered().Summary -->retorna True se a seleção de entidade for ordenada<!-- END REF -->e False se não for ordenada
...
A função `.orderByFormula()` <!-- REF #EntitySelectionClass.orderByFormula().Summary -->retorna uma nova entidade de seleção ordenada<!-- END REF --> **.orderByFormula**( *formulaString* : Text { ; *sortOrder* : Integer } { ; *settings* : Object} ) : 4D.
> Esta função não modifica a entity selection original.
...
A função `.query()` <!-- REF #EntitySelectionClass.query().Summary -->A função `.query()`<!-- END REF -->, e retorna um novo objeto de tipo `EntitySelection` contendo todas as entidades que foram encontradas. Se aplica carregamento diferido/lazy loading.
> Esta função não modifica a entity selection original.
...
A propriedade `.queryPlan` <!-- REF #EntitySelectionClass.queryPlan.Summary --> A propriedade `.queryPlan`<!-- END REF -->. Esta propriedade é disponível para objetos `EntitySelection` gerados através de pesquisas se a propriedade `"queryPlan":true` for passada no parâmetro *querySettings* da função [`.query()`](#query).
...
| Resultados       | Objeto              | <- | Intervalos da seleção de entidade da seleção de entidade<!-- END REF --> |
