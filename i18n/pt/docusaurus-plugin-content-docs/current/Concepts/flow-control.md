...
- **Secuencial**: una estructura secuencial es una estructura simple y lineal. Uma sequência é uma série de sentenças que 4D executa uma atrás da outra, da primera à última. Uma instrução de uma linha, utilizada frequentemente para os métodos dos objetos, é o caso mais simples de uma estrutura sequencial. Por ejemplo: `[People]lastName:=Uppercase([People]lastName)`
