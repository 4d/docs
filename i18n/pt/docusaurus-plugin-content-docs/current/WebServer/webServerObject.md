...
> Los [comandos WEB](../commands/theme/Web_Server) heredados del lenguaje 4D son soportados, pero no se puede seleccionar el servidor web al que se aplican (ver más abajo).
...
Para instanciar um objeto de servidor Web, chame o comando [`WEB Server`](../commands/web-server):
...
Para começar e parar um servidor web, basta chamar as funções [`start()`](API/WebServerClass.md#start) e [`stop()`](API/WebServerClass.md#stop) do objeto do servidor web:
...
El lenguaje 4D contiene [varios comandos](../commands/theme/Web_Server) permitiendo controlar el servido Web. No entanto, esses comandos são projetados para funcionar com um único servidor web (padrão). Ao usar esses comandos no contexto de objetos do servidor web, certifique-se de que seu escopo é apropriado.
