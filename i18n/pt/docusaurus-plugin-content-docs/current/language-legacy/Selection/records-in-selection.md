...
<!--REF #_command_.Records in selection.Summary-->Records in selection retorna o número de registros na seleção atual de *tabela*.<!-- END REF--> Por outro lado, [Records in table](../commands/records-in-table) retorna o número total de registros na tabela.  

## Exemplo 

O seguinte exemplo mostra uma técnica de loop comumente utilizada para se movimentar entre os registros da seleção atual. A mesma operação pode ser realizado com a ajuda do comando [APPLY TO SELECTION](../commands/apply-to-selection): 
