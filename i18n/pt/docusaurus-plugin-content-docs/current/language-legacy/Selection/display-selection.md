...
<!--REF #_command_.DISPLAY SELECTION.Syntax-->**DISPLAY SELECTION** ( {*tabela* : Table}{; *modoSelecao* : Integer}{; *entraLista* : Boolean}{; *}{; *})<!-- END REF-->
...
Para mostrar uma seleção e poder igualmente modificar um registro no formulário de entrada atual depois de fazer duplo clique nele (como é feito na janela do ambiente Desenho), utilize [OBJECT SET ENTERABLE](../commands/object-set-enterable) ao invés de DISPLAY SELECTION.  
  
A informação a continuação aplica a ambos comandos, exceto pela informação sobre a modificação de registros.  
  
Depois de executar DISPLAY SELECTION, não há registro atual. Utilize um comando tal como [FIRST RECORD](../commands/first-record) ou [LAST RECORD](../commands/last-record) para selecionar um.  
...
Lembre que com o comando DISPLAY SELECTION, este parâmetro apenas permite a seleção dos valores na lista e não sua modificação. De fato, o comando DISPLAY SELECTION carrega os registros da seleção atual em modo apenas leitura. Só o comando [MODIFY SELECTION](../commands/modify-selection) permite efetivamente a entrada de valores.   
  
**Nota**: O comando [OBJECT SET ENTERABLE](../commands/object-set-enterable) permite ativar ou desativar facilmente o modo Entrada em lista.  
...
Para reproduzir as funcionalidades oferecidas pelo menu Registros do ambiente Desenho quando utiliza DISPLAY SELECTION ou [MODIFY SELECTION](../commands/modify-selection) em modo Aplicação, aja da seguinte maneira:  
...
b. Associe esta barra de menus (utilizando o menu “Barra de menus associada” na caixa de diálogo de propriedades do formulário) com o formulário de saída utilizado com os comandos DISPLAY SELECTION ou [MODIFY SELECTION](../commands/modify-selection).  
...
Também pode utilizar outros comandos, tais como [PRINT SELECTION](../commands/print-selection), [QR REPORT](../commands/qr-report), etc. para oferecer todas as opções de menu padrão que desejar cada vez que visualize ou modifique uma seleção no modo Aplicação. Graças ao comando [Current form table](../commands/current-form-table) , estes métodos são genéricos, e a barra de menus as quais são compatíveis podem ser associadas a todo formulário de saída de qualquer tabela.
