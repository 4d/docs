...
<!--REF #_command_.SVG GET ATTRIBUTE.Syntax-->**SVG GET ATTRIBUTE** ( * ; *objetoPicture* : Text ; *id_Elemento* : Text ; *nomeAtrib* : Text ; *valorAtributo* : Text, Integer, Boolean )<br/>**SVG GET ATTRIBUTE** ( *objetoPicture* : Variable, Field ; *id_Elemento* : Text ; *nomeAtrib* : Text ; *valorAtributo* : Text, Integer, Boolean )<!-- END REF-->
...
Se passa o parâmetro opcional *\**, indica que o parâmetro *objetoImagem* é um nome de objeto (string). Neste caso, o comando retorna o valor do atributo da imagem renderizada anexa ao objeto. Este valor pode ter sido modificada por [SVG SET ATTRIBUTE](../commands/svg-set-attribute), por exemplo.  
...
**Nota**: este principio também se aplica ao comando [SVG Find element ID by coordinates](../commands/svg-find-element-id-by-coordinates).  
...
Para obter mais informação acerca dos atributos SVG, consulte a descripção do comando [SVG SET ATTRIBUTE](../commands/svg-set-attribute). Esta é uma lista de atributos 4D reservados e dedicados a animação:
...
| 4D-bringToFront                               | escrita         | Se 'true', mover o nó em frente dos nó irmãos. Apenas pode ser usada com o comando [SVG SET ATTRIBUTE](../commands/svg-set-attribute)                                                                                             |
