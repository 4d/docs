...
<!--REF #_command_.Subtotal.Summary-->Subtotal devolve o subtotal de valores para o nível de quebra atual ou anterior.<!-- END REF--> Subtotal só funciona quando uma seleção ordenada é impressa com [PRINT SELECTION](../commands/print-selection) ou utilizando Imprimir no ambiente Desenho. O parâmetro *valores* deve ser de tipo real, inteiro, ou inteiro longo. Você deve atribuir o resultado da função Subtotal a uma variável localizada na área de quebra do formulário.  

**Advertência**: deve executar os comandos [BREAK LEVEL](../commands/break-level) e [ACCUMULATE](../commands/accumulate) antes de cada relatório de formulário para o qual queira fazer processo de quebra e calcular subtotais. Ver a discussão ao final da descrição deste comando.  
...
Para poder gerar relatórios com quebras, deve ativar o tratamento de quebras chamando os comandos [BREAK LEVEL](../commands/break-level) e [ACCUMULATE](../commands/accumulate).  
...
Quando utilize [BREAK LEVEL](../commands/break-level) e [ACCUMULATE](../commands/accumulate), o processo de imprimir um relatório é geralmente como este:  
  
1\. Selecione os registros a imprimir.  
  
2\. Ordene os registros utilizando [ORDER BY](../commands/order-by). Ordene o mesmo número de níveis como quebras.  
  
3\. Executa [BREAK LEVEL](../commands/break-level) e [ACCUMULATE](../commands/accumulate).  
  
4\. Imprima o relatório utilizando[PRINT SELECTION](../commands/print-selection).  
