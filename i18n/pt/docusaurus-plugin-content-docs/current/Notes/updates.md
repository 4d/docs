...
- O objeto [`Session`](../API/SessionClass.md#session) agora está disponível em sessões de usuário remoto e procedimentos armazenados.
...
- Compatibilidade do [formato de criptografia `ECDSA` ](../Admin/tls.md#encryption) para certificados TLS.
- As conexões TLS do cliente/servidor e do servidor SQL agora são [configuradas dinamicamente](../Admin/tls.md#enabling-tls-with-the-other-servers) (não são necessários arquivos de certificado).
...
- Novo [Code Live Checker](../code-editor/write-class-method.md#warnings-and-errors) que melhora o controle de código durante a digitação de código, verificação de sintaxe e passos de compilação para evitar erros de execução.
- Os parâmetros do método declarados em protótipos `#DECLARE` não são mais necessários nos métodos "Compiler_".
- Suporte de [formatos de data e hora personalizados](../Project/date-time-formats.md)
- Novo [`Try(expressão)`](../Concepts/error-handling.md#tryexpression) para lidar com casos simples de erro.
...
- Novo modo [de edição](../Admin/dataExplorer.md#editing-data) no Explorador de Dados.
- Comandos da linguagem 4D: [página Novidades](https://doc.4d.com/4Dv20R4/4D/20-R4/What-s-new.901-6655756.en.html) em doc.4d.com.
- 4D Write Pro: [página Novidades](https://doc.4d.com/4Dv20R4/4D/20-R4/What-s-new.901-6683440.en.html) em doc.4d.com.
- [**Fixa lista de erros**](https://bugs.4d.fr/fixedbugslist?version=20_R4): lista de todos os bugs que foram corrigidos em 4D 20 R4.



#### Mudanças de comportamento

- Usando uma sintaxe legada para declarar parâmetros (por exemplo, `C_TEXT($1)` ou `var $1 : Texto`) está agora obsoleto e gera avisos na digitação de código, verificação de sintaxe e etapas de compilação.
- As seleções agora são mantidas consistentes após a exclusão de alguns registros e a criação de outros registros (veja [este post no blog](https://blog.4d.com/4d-keeps-your-selections-of-records-consistent-regarding-deletion-of-records/)).
...
Leia [**O que há de novo no 4D 20 R3**](https://blog.4d.com/en-whats-new-in-4d-v20-R3/), a postagem do blog que lista todos os novos recursos e aprimoramentos no 4D 20 R3.


#### Destaques

- Nova [`collection.multiSort`](../API/CollectionClass.md#multisort) função.
- Suporte do parâmetro *context* em [`Formula from string`](../API/FunctionClass.md#formula-from-string).
- Suporte da propriedade `headers` em *connectionHandler* parâmetro de [4D.WebSocket.new](../API/WebSocketClass.md#4dwebsocketnew).
- [Global modification stamp](../ORDA/global-stamp.md) para ajudar a implementar módulos de sincronização de dados. Novas funções: [`ds.getGlobalStamp`](../API/DataStoreClass.md#getglobalstamp) e [`ds.setGlobalStamp`](../API/DataStoreClass.md#setglobalstamp).
- Atribuindo arquivo de referências aos atributos imagem/blob é [suportado na ORDA](../ORDA/entities.md#d#assigning-files-to-picture-or-blob-attributes).
- Suporte para [inicializando o valor e tipo de dados da variável na linha de declaração](../Concepts/variables/#initializing-variables-in-the-declaration-line).
- As configurações do arquivo de log agora são [salvas com o arquivo de dados atual](../Backup/settings.md#log-file-management)
- Nova sintaxe para [declarando parâmetros variádicos](../Concepts/parameters.md#declaring-variadic-parameters)
- 4D View Pro: Suporte para a [importação](../ViewPro/commands/vp-import-from-blob) e [exportação](../ViewPro/commands/vp-export-to-blob) de documentos 4D View Pro no formato Blob.
...
- [**Lista de erros corrigidos**](https://bugs.4d.fr/fixedbugslist?version=20_R3): lista de todos os bugs que foram corrigidos em 4D 20 R3.



#### Mudanças de comportamento

- Alguns erros eram "capturáveis" pelo seu  [método de tratamento de erros](../Concepts/error-handling.md) somente no modo interpretado. Foi feita uma correção para que os erros abaixo, a partir de agora sejam detectados também no modo compilado: *Indice out of range*, *Type incompatible*, e *Dereferencing a Null pointer*. No entanto, para esses erros nos processadores interessados, o procedimento continua a ser interrompido como anteriormente. Enquanto que nos processadores do Apple Silicon o procedimento só é interrompido se você chamar o comando [`ABORT`](https://doc.4d.com/4dv20/help/command/en/page156.html).
...
- Nova configuração de interface da nova camada de rede QUIC [interface setting](../settings/client-server.md#network-layer).
- 4D View Pro: suporte ao formato de arquivo **.sjs** para [importação](../ViewPro/commands/vp-import-document) e [exportação](../ViewPro/commands/vp-export-document) de documentos.
- Comandos da linguagem 4D: [página Novidades](https://doc.4d.com/4Dv20R2/4D/20-R2/What-s-new.901-6398284.en.html) em doc.4d.com.
...
- [**Fixa lista de erros**](https://bugs.4d.fr/fixedbugslist?version=20_R2): lista de todos os bugs que foram corrigidos em 4D 20 R2.
...
Leia [**O que há de novo no 4D 20**](https://blog.4d.com/en-whats-new-in-4d-v20/), a postagem do blog que lista todos os novos recursos e aprimoramentos no 4D 20.
...
- (4D 20.1) Nova propriedade `plugins` no parâmetro *opções* para o comando [`Compilar projeto`](https://doc.4d.com/4dv20/help/command/en/page1760.html).
...
- Listas de bugs corrigidos: [4D 20](https://bugs.4d.fr/fixedbugslist?version=20) - [4D 20.1](https://bugs.4d.fr/fixedbugslist?version=20.1).
...
- :::
    - suporte a temas em tabelas: novos comandos [VP SET TABLE THEME](../ViewPro/commands/vp-set-table-theme) e [VP Get table theme](../ViewPro/commands/vp-get-table-theme), suporte a opções de tema no [VP CREATE TABLE](../ViewPro/commands/vp-create-table)
    - novo comando [VP Get table dirty rows](../ViewPro/commands/vp-get-table-dirty-rows)
...
- Células listbox suportam [horizontal](../FormObjects/properties_CoordinatesAndSizing.md#horizontal-padding) e [vertical](../FormObjects/properties_CoordinatesAndSizing.md#vertical-padding) preenchimento.
- Vista 4D Pro: novo [CRIAR ATUALIZAÇÃO](../ViewPro/commands/vp-create-table) e [VP REMOVER TABELA](../ViewPro/commands/vp-remove-table) comandos para manipular tabelas nas folhas.
...
- **4D View Pro**:
    - Os novos [comandos VP SET DATA CONTEXT](../ViewPro/commands/vp-set-data-context), [VP Get data context](../ViewPro/commands/vp-get-data-context), [VP SET BINDING PATH](../ViewPro/commands/vp-set-binding-path), [VP Get binding path](../ViewPro/commands/vp-get-binding-path) permitem criar contextos de dados e vincular seu conteúdo a células da planilha.
    - [VP EXPORT DOCUMENT](../ViewPro/commands/vp-get-binding-path) e [VP Export to object](../ViewPro/commands/vp-get-binding-path) agora aceitam a nova opção `includeBindingSource` que exporta o conteúdo de um contexto de dados como valores de célula.
...
- O [Code Editor](../code-editor/write-class-method.md) agora inclui uma ferramenta suspensa e suporta marcadores para melhorar a navegação do código.
...
- Capítulo [4D View Pro](../ViewPro/getting-started.md) adicionado com novos comandos: [VP Copiar para objeto](../ViewPro/commands/vp-copy-to-object), [VP MOVER CÉLULAS](../ViewPro/commands/vp-move-cells), [VP Colar de objeto](../ViewPro/commands/vp-paste-from-object).
...
- Por causa do suporte do operador [ternário](../Concepts/operators.md#ternary-operator), o dois pontos ":" não é mais permitido em nomes de variáveis, campo, constante, função, plugin e o método do projeto. Se o seu banco de dados/projeto contiver identificadores com dois, você deve substituí-los antes de convertê-lo para 4D 19 R4 ou superior, caso contrário podem ocorrer erros no seu código. Por exemplo, se você tiver uma variável denominada *a:b*, ela poderá ser interpretada como sintaxe de operador ternário:
...
- Recursos de depuração ampliados com o 4D Server: No modo interpretado, o 4D Server agora pode depurar todos os tipos de processos, incluindo sessões da Web escalonáveis. Isso está disponível quando o depurador está conectado ao servidor ou a um cliente remoto.   
  *Aviso: No modo interpretado, para disponibilizar a depuração estendida no computador do servidor, todos os processos do servidor agora são executados automaticamente no modo cooperativo quando o depurador está conectado ao servidor (configuração padrão). Isso pode ter um impacto significativo no desempenho de suas aplicações convertidas quando elas são executadas com o 4D Server 19 R3 e superior. Para restaurar a execução preemptiva no servidor nesse caso, tudo o que você precisa fazer é desconectar o depurador do servidor (e conectá-lo a um cliente remoto, se necessário).*
...
| OpenSSL    | 3.2.0        | 20 R4            | O nível de segurança TLS/SSL padrão foi atualizado. Consulte [Mudanças de comportamento](#behavior-changes) para liberar 20 R4 |
