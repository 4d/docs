...
-   **QUIC** (disponível somente no modo de projeto): Habilita a camada de rede QUIC no servidor. This network layer is available in **beta version**. Ainda não é recomendado que se use em produção, no entanto pode ser ativado em seus aplicativos cliente/servidor para fins de avaliação.
