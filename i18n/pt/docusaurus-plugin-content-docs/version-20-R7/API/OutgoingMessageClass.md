...
Normalmente, essa classe pode ser usada em funções declaradas com a palavra-chave [`onHttpGet`](../ORDA/ordaClasses.md#onhttpget-keyword) e projetadas para lidar com solicitações HTTP GET. Tais solicitações são usadas, por exemplo, para implementar recursos como arquivo de download, Gerar e baixar imagens, bem como receber qualquer tipo de conteúdo por um navegador.
