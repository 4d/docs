...
| Left Bit Shift         | O valor resultante é definido para o primeiro operando, depois os bits resultantes são deslocados para a esquerda pelo número de posições indicado pelo segundo operando. Os bits à esquerda são perdidos e os novos bits à direita são estabelecidos como 0. The bits on the left are lost and the new bits on the right are set to 0.                                                    |
