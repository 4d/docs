...
Os comandos da linguagem 4D oferecem acesso aos editores padrão de gestão de registros. For example, when you use the command, you are presented with the Query Editor (which can be accessed in the Design mode using the Query command in the Records menu. Pode dizer ao comando para pesquisar por dados descritos explicitamente. For example, ([People];[People]Last Name="Smith") will find all the people named Smith in your database.
