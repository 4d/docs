...
Os ajustes de impressão são aplicadas [ às configurações de impressão 4D](../settings/compatibility.md), sempre e cuando nenhum comando como *\_o\_PAGE SETUP*, [SET PRINT OPTION](set-print-option.md) ou [PRINT SELECTION](print-selection.md) sem o parâmetro > os modifique. O conjunto de parâmetros se utiliza particularmente para os comandos [PRINT SELECTION](print-selection.md), [PRINT LABEL](print-label.md), [PRINT RECORD](print-record.md), [Print form](../commands/print-form.md) e [QR REPORT](qr-report.md), assim como para os comandos de menu de 4D, incluindo as do ambiente Desenho.
