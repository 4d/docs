...
- Na pasta 4D ativa da máquina. As macros são então partilhadas para todas as bases de dados.
  \*\*Nota: O local da pasta 4D ativa varia de acordo com o sistema operacional utilizado. For more information, refer to the description of the [Get 4D folder](../commands/get-4d-folder) command in the 4D *Language Reference* manual.
...
| `<method_path/>`            | Tag substituída pela sintaxe do caminho (conforme retornado por [`METHOD Get path`](../commands/method-get-path) do método atual.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
É recomendado gerenciar seleções de texto usando o [GET MACRO PARAMETER](../commands/get-macro-parameter) e os comandos [SET MACRO PARAMETER](../commands/set-macro-parameter). Estes comandos podem ser utilizados para ultrapassar o particionamento dos espaços de execução do projeto/componente anfitrião, permitindo assim a criação de componentes dedicados à gestão de macros. Para ativar este modo para uma macro, é necessário declarar o atributo Versão com o valor 2 no elemento Macro. Neste caso, o 4D não gerencia mais as variáveis predefinidas _textSel,_textReplace, etc. e são utilizados os comandos [GET MACRO PARAMETER](../commands/get-macro-parameter) e [SET MACRO PARAMETER](../commands/set-macro-parameter). Este atributo deve ser declarado da seguinte forma:
