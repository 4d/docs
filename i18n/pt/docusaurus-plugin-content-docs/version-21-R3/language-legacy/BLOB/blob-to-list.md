...
Os dados do BLOB devem ser coerentes com o comando. Normalmente, você irá usar BLOBs que você previamente preenchou utilizando o comando [LIST TO BLOB](../commands/list-to-blob).
...
**Nota sobre a independência de plataforma**: BLOB to list e [LIST TO BLOB](../commands/list-to-blob) usam um formato interno de 4D para o processamento de listas armazenadas em BLOBs. Como benefício, você não precisa se preocupar com a troca de bytes entre as plataformas ao usar esses dois comandos. Em outras palavras, um BLOB criado no Windows usando os dois comandos podem ser reutilizados em Macintosh e vice-versa.
