...
<!--REF #_command_.WP Table range.Syntax-->**WP Table range** ( *objFaixa* : Object ) -> Resultado<!-- END REF-->
...
O comando **WP Table range** devolve um objeto de range que trata só as tabelas contidas em *objRange* passado como parâmetro. Devolverá uma range de wk type table, mesmo se *objRange* passado como parâmetro não contiver uma tabela. O objeto de range de tabela devolvido pode ser utilizado por [WP GET ATTRIBUTES](../commands/wp-get-attributes) e [WP SET ATTRIBUTES](../commands/wp-set-attributes) para manejar os atributos que são aplicados às tabelas e aos conteúdos da tabela.
