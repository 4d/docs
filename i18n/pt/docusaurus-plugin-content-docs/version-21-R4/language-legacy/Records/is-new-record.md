...
<!--REF #_command_.Is new record.Syntax-->**Is new record** ( *tabela* : Table ) : Boolean<!-- END REF-->
...
<!--REF #_command_.Is new record.Summary-->O comando Is new record devolve [True](../commands/true) quando o registro atual de *tabela* estiver sendo criado e ainda não foi guardado no processo atual.<!-- END REF-->  
...
**4D Server**: Este comando devolve um resultado diferente no contexto do evento de formulário On Validate dependendo se foi executado em 4D (mono-posto) ou 4D Client. Em versão mono-posto, o comando devolve [False](../commands/false) (o registro se considera como criado). Em versão cliente/servidor, o comando devolve *True* porque nesse caso, o registro já foi criado no servidor mas a informação não foi enviada ainda ao cliente.
