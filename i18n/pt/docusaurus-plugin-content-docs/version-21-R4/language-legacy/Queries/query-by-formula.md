...
<!--REF #_command_.QUERY BY FORMULA.Syntax-->**QUERY BY FORMULA** ( *tabela* : Table {; *formula* : Expression} )<!-- END REF-->
...
QUERY BY FORMULA e [QUERY SELECTION BY FORMULA](../commands/query-selection-by-formula) funcionam exatamente da mesma maneira, exceto que QUERY BY FORMULA busca em todos os registros da tabela e [QUERY SELECTION BY FORMULA](../commands/query-selection-by-formula) busca unicamente nos registros da seleção atual.  
...
**Nota de compatibilidade**: por razões de compatibilidade, é possível desativar o mecanismo de uniões, seja globalmente através das Preferências do banco (apenas bancos de dados convertidos) ou por processos utilizando o comando [SET DATABASE PARAMETER](../commands/set-database-parameter).  
...
**Nota de compatibilidade**: até 4D Server v11, este comando se executava na máquina cliente Por compatibilidade, este funcionamento é conservado nos bancos de dados convertidos a versão 11\. Uma preferência de compatibilidade e um seletor do comando [SET DATABASE PARAMETER](../commands/set-database-parameter) permitem adotar o funcionamento da versão 11 (execução no servidor) nestes bancos de dados.
