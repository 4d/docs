...
<!--REF #_command_.FORM GET ENTRY ORDER.Syntax-->**FORM GET ENTRY ORDER** ( *objectNames* : Text array {; numPagina | * } )<!-- END REF-->
...
* Se não passar o parâmetro *\**, **FORM GET ENTRY ORDER** devolve a ordem de entrada como foi declarado anteriormente com o comando [FORM SET ENTRY ORDER](../commands/form-set-entry-order). Pode omitir ou passar o parâmetro *numPag*:  
...
Em ambos casos, se o comando [FORM SET ENTRY ORDER](../commands/form-set-entry-order) não foi chamado previamente para o formulário atual, o array *nomObjetos* é devolvido vazio.
* Se passar o parâmetro *\**, **FORM GET ENTRY ORDER** devolve a ordem de entrada atual da página atual, ou seja, o array *nomObjetos* só contém nomes de objeto **válidos** ( (para mais informação sobre objetos válidos, consulte a descrição do comando [FORM SET ENTRY ORDER](../commands/form-set-entry-order)). A ordem de entrada de formulário real pode ser:  
   ...
   * Ou a ordem de entrada definido por uma chamada ao comando [FORM SET ENTRY ORDER](../commands/form-set-entry-order) no processo atual, se tiver sido utilizado.  
