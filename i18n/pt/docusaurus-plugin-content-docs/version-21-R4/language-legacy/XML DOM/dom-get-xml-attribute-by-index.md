...
**Nota**: O número de índice não corresponde à localização do atributo no arquivo XML mostrado em forma de texto. Em XML, o índice de um atributo indica sua posição entre os atributos classificados por ordem alfabética (em função de seu nome). Para ver uma ilustração deste principio, consulte o exemplo do comando [DOM Count XML attributes](../commands/dom-count-xml-attributes).  
  
Se o valor passado em *indexAtrib* é superior ao número de atributos presentes no elemento XML, se retorna um erro.

## Exemplo 

Consulte o exemplo do comando [DOM Count XML attributes](../commands/dom-count-xml-attributes). .
