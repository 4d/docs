...
<!--REF #_command_.SET INDEX.Summary-->**SET INDEX**, apesar de ser um comando herdado, ainda é compatível e pode permanecer em sua aplicação, não é necessário voltar a escrever o código existente.<!-- END REF--> Entretanto, é melhor usar [CREATE INDEX](../commands/create-index) y [DELETE INDEX](../commands/delete-index) para administrar os índices porque oferecem mais funcionalidades.
...
* Os índices criados por este comando não têm nomes. Não podem ser apagados pelo comando [DELETE INDEX](../commands/delete-index) utilizando a sintaxe baseada no nome.
* Este comando não permite criar ou apagar índices compostos.
* Este comando não permite apagar um índice de palavras chaves criado pelo comando [CREATE INDEX](../commands/create-index)
