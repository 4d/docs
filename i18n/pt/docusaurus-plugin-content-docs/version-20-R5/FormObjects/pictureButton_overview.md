...
- Como botão de imagem que permite ao usuário escolher entre várias opções. Neste caso, pode ser utilizado um botão de imagem em vez de um menu pop-up de imagem. Com [Menu pop-up de imagem](picturePopupMenu_overview.md), todas as escolhas são exibidas simultaneamente (como os itens no menu pop-up), enquanto o botão imagem mostra as opções consecutivas (como o usuário clica no botão).\
  Aqui está um exemplo de um botão imagem. Suponha que você queira dar aos usuários de um aplicativo personalizado a oportunidade de escolher o idioma da interface do aplicativo. Você implementa a opção como um botão de imagem em uma caixa de diálogo de propriedades personalizadas:
...
1. Primeiro, prepare um único gráfico no qual a série de imagens esteja organizada em uma linha, uma coluna ou uma grade de linha por coluna.
