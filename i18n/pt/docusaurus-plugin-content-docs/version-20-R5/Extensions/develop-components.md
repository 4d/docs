...
- **Componente**: projeto matriz que pode ser compilado ou [construído](Desktop/building.md#build-component), copiado para a pasta [`Components`](Project/architecture.md) do aplicativo host e cujo conteúdo é usado no aplicativo host.

## Básicos

Criar e instalar componentes 4D é realizado diretamente a partir de 4D:

- Para instalar um componente, basta copiar os arquivos do componente para a pasta [`Components` do projeto] (Project/architecture.md). Pode usar pseudónimos ou atalhos.
...
Por outro lado, por padrão, esses métodos de projeto não estarão visíveis e não poderão ser chamados no projeto host. No projeto matriz, você deve designar explicitamente os métodos que você deseja compartilhar com o projeto host, marcando a caixa **Compartilhada por componentes e projetos do host** na caixa de diálogo de propriedades do método:
...
Para permitir que as classes e funções do seu componente sejam expostas nos projetos host, insira um valor na opção [**Namespace do componente na class store** da página General](../settings/general.md#component-namespace-in-the-class-store) das Configurações do projeto matriz. Por padrão, a área está vazia: as classes de componentes não estão disponíveis fora do contexto do componente.
...
Quando você insere um valor, você declara que classes de componente e funções estarão disponíveis na [loja da classe de usuário (**cs**)](. /Concepts/classes.md#cs) do código do projeto host, através do namespace `cs.<value>`. Por exemplo, se você digitar "eGeometry" como namespace do componente, Supondo que você criou uma classe `Rectangle` contendo uma função `getArea()`, assim que seu projeto for instalado como componente, o desenvolvedor do projeto host pode escrever:
...
O namespace de um componente [compilado](#protection-of-components-compilation) será adicionado entre parênteses após o nome do componente na página [Component Methods](../Concepts/components.md#using-components) dos projetos do host:
