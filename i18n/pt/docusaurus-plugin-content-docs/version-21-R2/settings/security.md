...
- **Execute "On Host Database Event" method of the components**: The [On Host Database Event database method](./commands/on-host-database-event-database-method) facilitates the initialization and backup phases for 4D components. Por razões de segurança, você deve autorizar explicitamente a execução deste método em cada banco de dados de host. Para o efeito, é necessário marcar esta opção. Por padrão, não está marcada.
  ...
  - cada [método banco de dados On Host Database Event](./commands/on-host-database-event-database-method) do componente (se houver) é chamado pelo banco de dados host,
