...
4D View Pro permite que você faça referência a campos 4D usando a estrutura virtual do banco de dados, ou seja, declarada por meio dos comandos [`SET TABLE TITLES`](../commands-legacy/set-table-titles.md) e/ou [`SET FIELD TITLES`](../commands-legacy/set-field-titles.md) com o parâmetro \*. **ATENÇÃO:** Você não pode usar a estrutura virtual e `VP SET CUSTOM FUNCTIONS` simultaneamente.
