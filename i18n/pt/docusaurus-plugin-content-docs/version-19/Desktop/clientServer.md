...
- **Network address**: The IP address of the machine where the 4D Server was launched. If two servers are executed simultaneously on the same machine, the IP address must be followed by a colon and port number, for example: `192.168.92.104:19814`. By default, the publishing port of a 4D Server is 19813. Este número pode ser modificado nas definições do Projecto.
