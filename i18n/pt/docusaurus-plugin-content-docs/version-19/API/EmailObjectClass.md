...
[`bodyStructure`](#bodystructure) e [`bodyValues`](#bodyvalues) são usados para [SMTP](SMTPTransporterClass.md) quando o [Email objeto](email-object) é construído a partir de um documento MIME, por exemplo, quando gerado pelo comando `MAIL Converter a partir do MIME`. Neste caso, tanto as propriedades `bodyStructure` e `bodyValues` devem ser passados juntas, e não é recomendado usar o `textBody` e `htmlBody`.
