...
Além disso, de acordo com o [princípio da recursão](../Tags/transformation-tags.md#recursive-processing), o próprio código malicioso pode incluir tags de transformação. En este caso, es imprescindible utilizar la etiqueta `4DTEXT`. Imagine, por exemplo, um campo de formulário da Web chamado "Nome", no qual os usuários devem digitar seu nome. Este nombre se muestra mediante una etiqueta `<!--#4DHTML vName-->` en la página. Se o texto do tipo "\<!--#4DEVAL QUIT 4D-->" for inserido ao invés do nome, interpretar esta etiqueta fará com que a aplicação seja encerrada. Para evitar este riesgo, basta con utilizar sistemáticamente la etiqueta `4DTEXT` en este caso. Para fazer referência ao exemplo anterior, o campo "Name" conterá, nesse caso, "<code>&amp;lt;!--#4DEVAL QUIT 4D--&amp;gt;</code>", que não será transformado. Para se referir ao exemplo anterior, o campo "Name" conterá, neste caso, "`&lt;! -#4DEVAL QUIT 4D--&gt;`" que não será transformado.
