...
| Backup.4DSettings   | Parâmetros de backup do banco de dados, usadas para definir as [opções de backup](Backup/settings.md) quando cada backup é iniciado. This file can also be used to read or set additional options, such as the amount of information stored in the *backup journal*. As teclas relativas à configuração da cópia de segurança são descritas no manual *4D XML Keys Backup*. | XML     |
