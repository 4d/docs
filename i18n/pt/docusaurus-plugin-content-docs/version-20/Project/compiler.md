...
Usado para gerar o arquivo de erro (consulte \[arquivo de erro\](#error-file)) no momento da verificação de sintaxe. O arquivo de erro é criado na [pasta Logs](../Project/architecture.md#logs) do projeto com o nome `ProjectName_errors.xml`.
...
- **Métodos**: agrupa as declarações de parâmetro do método (por exemplo, `C_LONGINT(mymethod;$1;$2)`). Para obter mais informações, consulte método [`Compiler_Methods`](../Concepts/parameters.md#compiler-method).
...
Se você marcar a opção [**Gere o arquivo de símbolos**](#symbol-file) nas configurações do compilador, um arquivo de símbolo chamado `ProjectName_symbols.xt` é criado na [pasta Logs](../Project/architecture.md#logs) do projeto durante a compilação. Está dividido em várias partes:
