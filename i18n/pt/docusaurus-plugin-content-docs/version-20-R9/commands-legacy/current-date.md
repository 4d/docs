...
    End if
 Until(vOK=1)
 $format:="" // Inicialização do resultado da função
...
       $format:="YYYY"
 End case
 $format:=$format+"/" //Iniciar a construção do resultado da função
...
       $format:=$format+"YYYY"
 End case
  //Neste momento $format é igual a MM/DD/YYYY ou DD/MM/YYYY ou...
```
