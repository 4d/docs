...
Em um list box do tipo seleção, para produzir o mesmo efeito, você pode usar um método para atualizar a [Expressão de cor de fundo](properties_BackgroundAndBorder.md#background-color-expression) com base no conjunto especificado na propriedade [Conjunto destacado](properties_ListBox.md#highlight-set).
