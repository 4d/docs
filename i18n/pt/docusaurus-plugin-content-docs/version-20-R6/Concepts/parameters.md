...
> Internamente, o `return x` executa `$0:=x` ou (se declarado) `myReturnValue:=x`, e retorna ao remetente. Se `return` for usado sem uma expressão, a função ou o método retornará um valor nulo do tipo de retorno declarado (se houver), caso contrário, *indefinido*.
