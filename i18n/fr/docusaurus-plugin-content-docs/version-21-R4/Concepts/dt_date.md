...
Utiliser la commande [`JSON Parse`](../commands/json-parse) :
...
Utiliser la commande [`Date`](../commands/date) :
...
Notez la différence entre ces deux solutions : [`JSON Parse`](../commands/json-parse) respecte le [mode de conversion défini à l'aide de `SET DATABASE PARAMETER`](../commands/set-database-parameter#dates-inside-objects-85) (s'il existe), alors que [`Date`](../commands/date) n'y est pas soumis. La conversion à l'aide de la commande [`Date`](../commands/date) tient toujours compte du fuseau horaire local.

:::note

Lorsque le paramètre courant de stockage de la date est [`date type`](../commands/set-database-parameter#dates-inside-objects-85) (par défaut), les chaînes de date JSON au format "YYYY-MM-DD" sont automatiquement traitées comme des valeurs de date par les commandes [`JSON Parse`](../commands/json-parse) et [`Date`](../commands/date).
