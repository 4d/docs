...
<!--REF #_command_.LISTBOX Get auto row height.Syntax-->**LISTBOX Get auto row height** ( * ; *objet* : Text ; *sélecteur* : Integer {; *unité* : Integer} ) : Integer<br/>**LISTBOX Get auto row height** ( *objet* : Variable ; *sélecteur* : Integer {; *unité* : Integer} ) : Integer<!-- END REF-->
...
| objet | Text, Variable | &#8594;  | Nom de l'objet (si * est spécifié) ou variable (si * est omis) |
...
La valeur courante de hauteur minimum ou maximum peut être définie soit dans la Liste des propriétés (voir *Hauteur de ligne automatique*) soit, pour le process courant, à l'aide de la commande [LISTBOX SET AUTO ROW HEIGHT](../commands/listbox-set-auto-row-height).
