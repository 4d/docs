...
<!--REF #_command_.OBJECT GET EVENTS.Syntax-->**OBJECT GET EVENTS** ( * ; *objet* : Text ; *tabEvénements* : Integer array )<br/>**OBJECT GET EVENTS** ( *objet* : Variable, Field ; *tabEvénements* : Integer array )<!-- END REF-->
...
| objet | Text, Variable, Field | &#8594;  | Nom d'objet ou "" pour désigner le formulaire (si * est spécifié) ou <br/>Champ ou variable (si * est omis) |
...
Les événements formulaire peuvent avoir été activés/désactivés soit via la Liste des propriétés, soit via la commande [OBJECT SET EVENTS](../commands/object-set-events) si elle a été appelée dans le process courant.
