...
Si vous souhaitez afficher une sélection et pouvoir également modifier un enregistrement dans le formulaire entrée courant une fois que vous avez double-cliqué dessus (comme vous le faites dans la fenêtre du mode Développement) ou via le mode “Saisie en liste”, utilisez [MODIFY SELECTION](../commands/modify-selection) au lieu de **DISPLAY SELECTION**. Toutes les explications suivantes s'appliquent à ces deux commandes, hormis la possibilité de modifier des enregistrements. 

Après qu'un **DISPLAY SELECTION** ait été exécuté, il n'y a plus d'enregistrement courant. Vous devez utiliser une commande telle que [FIRST RECORD](../commands/first-record) ou [LAST RECORD](../commands/last-record) pour en récupérer un.
...
Reportez-vous à l'exemple n°6 de la commande [Form event code](../commands/form-event-code) ; il indique tous les tests que vous pourrez avoir besoin d'effectuer pour surveiller la totalité des événements intervenant pendant l'exécution de la commande **DISPLAY SELECTION**. 

## Exemple 3 

Pour reproduire, par exemple, les fonctionnalités apportées par le menu **Enregistrements** du mode Développement lorsque vous utilisez [MODIFY SELECTION](../commands/modify-selection) ou **DISPLAY SELECTION** en mode Application, procédez de la manière suivante :
...
II. Associez cette barre de menus (à l'aide du menu “Barre de menus associée” dans la boîte de dialogue des propriétés du formulaire) au formulaire sortie utilisé avec les commandes **DISPLAY SELECTION** ou [MODIFY SELECTION](../commands/modify-selection).  
...
Vous pouvez aussi utiliser d'autres commandes telles que [PRINT SELECTION](../commands/print-selection), [QR REPORT](../commands/qr-report), etc., afin de reproduire les commandes de menu "standard" à chaque fois que vous affichez ou modifiez une sélection en mode Application. Grâce à la commande [Current form table](../commands/current-form-table), ces méthodes sont génériques et les barres de menus auxquelles elles sont associées peuvent être rattachées à tout formulaire de sortie ou à toute table.
