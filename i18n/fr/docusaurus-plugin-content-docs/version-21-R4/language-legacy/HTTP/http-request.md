...
| HTTP GET method     | Chaîne | GET     | Voir la *RFC 2616*. Equivaut à utiliser la commande [HTTP Get](../commands/http-get) |
...
Si la connexion au serveur est impossible pour une raison liée au réseau (*DNS Failed*, *Server not reachable*...) la commande retourne 0 et une erreur est générée. Vous pouvez intercepter les erreurs à l’aide d’une méthode installée par la commande [ON ERR CALL](../commands/on-err-call).
