...
<!--REF #_command_.Subtotal.Syntax-->**Subtotal** ( *valeurs* : Field, Variable {; *sautPage* : Integer} ) : Real<!-- END REF-->
...
Pour pouvoir générer des états avec ruptures, vous devez déclencher le traitement des ruptures en appelant les commandes [BREAK LEVEL](../commands/break-level) et [ACCUMULATE](../commands/accumulate). Il faut que ces deux commandes soient appelées avant l'impression du formulaire. L'appel à la fonction **Subtotal** est nécessaire pour afficher les calculs de niveaux intermédiaires. Il est obligatoire de trier sur au moins le nombre de niveaux de ruptures désiré.

Dans le cadre de l'utilisation des commandes [BREAK LEVEL](../commands/break-level) et [ACCUMULATE](../commands/accumulate), les étapes à suivre sont :  
...
3\. Appeler les commandes [BREAK LEVEL](../commands/break-level) et [ACCUMULATE](../commands/accumulate),  
4\. Imprimer l'état avec la commande [PRINT SELECTION](../commands/print-selection).
