...
L'**objet de langage** est référencé de manière unique par un identifiant interne, de type Entier long, désigné par *RefListe* dans ce manuel. Cet ID est retourné par les commandes qui peuvent être utilisées pour créer des listes : [`New list`](../commands/new-list), [`Copy list`](../commands/copy-list), [`Load list`](../commands/load-list), [`BLOB to list`](../commands/blob-to-list). Il n’existe qu’une seule instance en mémoire de l’objet de langage et toute modification effectuée sur cet objet est immédiatement répercutée dans tous les endroits où il est utilisé.
...
Par conséquent, lorsque vous utilisez des commandes basées sur la configuration déployé/contracté ou l'élément courant, par exemple [`Count list items`](../commands/count-list-items) (lorsque le paramètre `*` final n’est pas passé), il importe de pouvoir désigner sans ambiguïté la représentation à utiliser.
...
Comme pour [les autres commandes de gestion des propriétés d'objets](../FormObjects/formObjects_overview.md#accessing-form-objects-using-their-name-or-their-data-source-in-the-4d-language), il est possible d'utiliser le caractère "@" dans le paramètre `ListName`. En principe, cette syntaxe permet de désigner un ensemble d’objets dans le formulaire. Toutefois, dans le contexte des commandes de liste hiérarchique, ce principe n’est pas applicable dans tous les cas. Cette syntaxe aura deux effets différents en fonction du type de commande :
...
> Rappel : A l'exception de la commande [`OBJECT SET SCROLL POSITION`](../commands/object-set-scroll-position), ces commandes modifient toutes les représentations d'une même liste, même si vous désignez une liste via son nom d'objet.
...
Dans la plupart des cas, lorsque vous utilisez des listes hiérarchiques pour des besoins d'interface utilisateur, pour lesquels seul l'élément sélectionné (par un clic ou par glisser-déposer) est important, vous n'avez pas besoin d'utiliser les numéros de référence des éléments. En utilisant [`Selected list items`](../commands/selected-list-items) et [`GET LIST ITEM`](../commands/get-list-item) vous avez tout ce dont vous avez besoin pour traiter l'élément actuellement sélectionné. De plus, des commandes telles que [`INSERT IN LIST`](../commands/insert-in-list) et [`DELETE FROM LIST`](../commands/delete-from-list) vous permettent de manipuler la liste de manière "relative" à l'élément sélectionné.
