...
| entitySetCount | Number     | Nombre d'ensembles d'entités.                                                                                                                                                                                                   |
...
Pour chaque entity set stocké dans le cache de 4D Server, les informations retournées sont les suivantes :
...
| expires       | Date    | Date d'expiration de l'entity set (cette date/heure change chaque fois que l'entity set est actualisé). La différence entre refreshed et expires est le timeout pour un entity set. Cette valeur correspond soit à deux heures par défaut, soit à la valeur que vous avez définie à l'aide de `$timeout`. Il est également possible de la modifier pour la session via la propriété [`Session.quotas`](../API/SessionClass.md#quotas). |

Pour plus d'informations sur la création d'un entity set, consultez [`$method=entityset`](./$method.md#methodentityset). Si vous souhaitez supprimer un entity set du cache de 4D Server, utilisez [`$entityset/$release`](./$entityset.md#entitysetrelease).
