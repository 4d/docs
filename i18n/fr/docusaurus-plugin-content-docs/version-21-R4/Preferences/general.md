...
Lorsque cette option est cochée, les [fichiers sources des méthodes](../Project/architecture.md#sources) enregistrées dans les nouveaux projets 4D contiendront des [**tokens** pour les objets du langage classic et de la base (constantes, commandes, tables et champs)](../Concepts/methods.md#language-tokens). Les tokens sont des caractères supplémentaires tels que `:C10` ou `:5` insérés dans les fichiers de code source, qui permettent de renommer les tables et les champs et d'identifier les éléments quelle que soit la version 4D.
...
Vous pouvez définir le contenu par défaut du fichier `.gitignore` en cliquant sur l'icône du crayon. Cela ouvrira le fichier de configuration .gitignore dans votre éditeur de texte. Cela ouvrira le fichier de configuration .gitignore dans votre éditeur de texte.
