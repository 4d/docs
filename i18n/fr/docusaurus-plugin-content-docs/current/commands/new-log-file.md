...
**Note préliminaire:** Cette commande ne fonctionne qu'avec 4D Server. Elle ne peut être exécutée que via la commande [Execute on server](./commands/execute-on-server) ou via une procédure stockée.
...
En cas d'erreur, la commande génère un code qui peut être intercepté à l'aide de la commande [ON ERR CALL](./commands/on-err-call).
