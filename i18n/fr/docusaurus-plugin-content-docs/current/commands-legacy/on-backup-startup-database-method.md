...
<!--REF #_command_.On Backup Startup database method.Summary-->La **On Backup Startup database method** est appelée à chaque fois qu’une sauvegarde de la base est sur le point d’avoir lieu (sauvegarde manuelle, sauvegarde automatique périodique ou via la commande [BACKUP](../commands/backup)).<!-- END REF-->   
...
* si $0 # 0, vous n'autorisez pas la sauvegarde. L’opération est annulée et une erreur est retournée. Vous pouvez récupérer l’erreur à l’aide de la commande [BACKUP INFO](../commands/backup-info).
