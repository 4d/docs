...
Pour une liste des propriétés prises en charge par les objets colonnes, veuillez vous référer à la section [Propriétés spécifiques des colonnes](listbox-column.md#column-specific-properties).
...
L'événement `On Close Detail` est alors généré lorsqu'un enregistrement affiché dans le formulaire détaillé associé à la listbox est sur le point d'être fermé (que l'enregistrement ait été modifié ou non). Pour les deux dernières actions, l'événement formulaire `On Open Detail` est également généré.
...
> Vous pouvez ajouter ou supprimer des colonnes dynamiquement par programmation, en utilisant des commandes telles que [`LISTBOX INSERT COLUMN`](../commands/listbox-insert-column) ou [`LISTBOX DELETE COLUMN`](../commands/listbox-delete-column).
