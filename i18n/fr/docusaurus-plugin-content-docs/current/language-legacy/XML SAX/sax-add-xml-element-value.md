...
<!--REF #_command_.SAX ADD XML ELEMENT VALUE.Syntax-->**SAX ADD XML ELEMENT VALUE** ( *document* : Time ; *données* : Text, Variable, Field {; *} )<!-- END REF-->
...
Si vous souhaitez encoder le contenu de *données*, vous devez utiliser la commande [BASE64 ENCODE](../commands/base64-encode). Dans ce cas bien entendu, vous devez passer un BLOB dans *données*. 

Par défaut, la commande encode les caractères spéciaux (< > ” ’...) contenus dans le paramètre *données*, sauf si vous avez désactivé ce mécanisme pour le process courant à l'aide de la commande [XML SET OPTIONS](../commands/xml-set-options) en passant la valeur XML raw data à l'option XML string encoding. Par exemple :  
