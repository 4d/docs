...
| --- | --- | --- | --- |
| cpuUsage(\*) | numérique | Pourcentage du processeur utilisé par 4D. Pour les Macs, le nombre affiché correspond à 1 cœur et représente une moyenne pour le processeur. | 10.03 |
...
| newConnectionsAllowed | booléen | 4D Server uniquement. Vrai si de nouvelles connexions sont autorisées. Valeurs possibles : vrai, faux. | vrai |
...
| TLSEnabled | booléen | Vrai si TLS est activé pour les communications client/serveur. Non retourné en mode local. Valeurs possibles : vrai, faux. Pour plus d'informations, consultez *Qu’est-ce que le protocole TLS dans le cadre des connexions client/serveur ?*. | vrai |
...
| useQUICNetworkLayer | booléen | Vrai si la [couche réseau QUIC](../../settings/client-server.md#network-layer) est utilisée pour le serveur d'application et le serveur db4d. Non retourné en mode local. Valeurs possibles : vrai, faux.| faux |
