...
La commande **RECEIVE BUFFER** est différente de [RECEIVE PACKET](../commands/receive-packet) dans la mesure où elle récupère tout ce qui se trouve dans le buffer et le retourne immédiatement. [RECEIVE PACKET](../commands/receive-packet), pour sa part, attend de récupérer un caractère spécifique ou un certain nombre de caractères.
...
Notez que l'accès à la variable interprocess *◊vtBuffer* doit être protégé par un sémaphore, de manière à ce que les process n'entrent pas en conflit (reportez-vous à la description de la fonction [Semaphore](../commands/semaphore) pour plus d'informations).
