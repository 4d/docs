...
<!--REF #_command_.RECEIVE PACKET.Syntax-->**RECEIVE PACKET** ( {*docRef* : Time ;} *réceptVar* : Text, Blob ; *stopChar* : Text )<br/>**RECEIVE PACKET** ( {*docRef* : Time ;} *réceptVar* : Text, Blob ; *numBytes* : Integer )<!-- END REF-->
...
Si *docRef* est spécifié, la commande récupère des données depuis un document ouvert par la fonction [Open document](../commands/open-document), [Create document](../commands/create-document) ou [Append document](../commands/append-document). Si *docRef* est omis, la commande récupère des données depuis un port série ou un document ouvert par la commande [SET CHANNEL](../commands/set-channel). 

Dans tous les cas, les caractères lus sont retournés dans la variable *réceptVar*, qui doit être une variable de type Texte, Alpha ou BLOB. Si les données ont été envoyées par la commande [SEND PACKET](../commands/send-packet), le type doit correspondre à celui du paquet envoyé.

**Notes**

* Si le paquet reçu est de type BLOB, la commande ne tient pas compte du jeu de caractères éventuellement défini par la commande [USE CHARACTER SET](../commands/use-character-set). Le BLOB est retourné sans aucune modification.
...
* lorsque **RECEIVE PACKET** lit des données en provenance du port série, la commande s'exécutera indéfiniment jusqu'à ce que le délai d'attente (s'il est fixé) soit écoulé (cf. la commande [SET TIMEOUT](../commands/set-timeout)) ou que l'utilisateur interrompe la réception (voir ci-dessous).
...
**Note :** Ce fonctionnement est valide avec un document ouvert par [SET CHANNEL](../commands/set-channel). Cependant, pour un document ouvert par [Open document](../commands/open-document), [Create document](../commands/create-document) ou [Append document](../commands/append-document), vous pouvez aussi utiliser les commandes [Get document position](../commands/get-document-position) et [SET DOCUMENT POSITION](../commands/set-document-position)pour connaître et modifier la position à laquelle, dans le document, la prochaine écriture ([SEND PACKET](../commands/send-packet)) ou lecture (**RECEIVE PACKET**) aura lieu.
