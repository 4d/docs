...
**QUERY BY FORMULA** et la commande [QUERY SELECTION BY FORMULA](../commands/query-selection-by-formula) fonctionnent exactement de la même manière, à la différence près que **QUERY BY FORMULA** effectue sa recherche parmi la totalité des enregistrements de la table alors que [QUERY SELECTION BY FORMULA](../commands/query-selection-by-formula) se cantonne aux enregistrements de la sélection courante.
...
**Note de compatibilité :** Pour des raisons de compatibilité avec des bases converties à partir de versions antérieures à la v11, il est possible de désactiver le mécanisme de jointures via la commande [SET DATABASE PARAMETER](../commands/set-database-parameter).
