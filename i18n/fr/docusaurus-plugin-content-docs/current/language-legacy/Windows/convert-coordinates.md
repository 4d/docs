...
| XY Screen         | Entier long | 3      | L'origine est le coin supérieur de l'écran principal (comme pour la commande [SCREEN COORDINATES](../commands/screen-coordinates)) |
...
Lorsque vous effectuez une conversion depuis/vers la position d'une fenêtre de formulaire (par exemple une conversion depuis les résultats de [GET WINDOW RECT](../commands/get-window-rect), ou vers des valeurs passées à [Open form window](../commands/open-form-window)), le sélecteur XY Main window doit être utilisé car il s'agit du système de coordonnées utilisé par les commandes de gestion des fenêtres sous Windows. Ce sélecteur peut également être utilisé dans ce but sous macOS, où il est équivalent à XY Screen.
...
Vous souhaitez créer une fenêtre pop up à l'emplacement du curseur de la souris. Sous Windows, vous devez convertir les coordonnées car [MOUSE POSITION](../commands/mouse-position) (avec le paramètre \*) retourne des valeurs basées sur la position de la fenêtre MDI :
