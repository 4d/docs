...
- Les commandes classiques du langage 4D ([`START TRANSACTION`](../commands-legacy/start-transaction), [`VALIDATE TRANSACTION`](../commands-legacy/validate-transaction), [`CANCEL TRANSACTION`](../commands-legacy/cancel-transaction)) ne s'appliquent qu'au datastore principal (renvoyé par `ds`).
