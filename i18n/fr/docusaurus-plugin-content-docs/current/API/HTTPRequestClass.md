...
La classe `HTTPRequest` est disponible dans le class store `4D`. You create and send HTTP requests using the [4D.HTTPRequest.new()](#4dhttprequestnew) function, that returns a [`HTTPRequest object`](#httprequest-object).
...
| returnResponseBody     | Boolean                                         | If false, the response body is not returned in the [`response` object](#response). Renvoie une erreur si faux et `onData` est undefined                                                                                                                                                                                                 | True        |
...
The `.terminate()` function <!-- REF #HTTPRequestClass.terminate().Summary -->aborts the HTTP request<!-- END REF -->. Elle déclenche l'événement `onTerminate` .
