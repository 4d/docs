...
| Release | Modifications                                                          |
| ------- | ---------------------------------------------------------------------- |
| 21 R3   | Prise en charge de la propriété *storeCertificateName* sur macOS       |
| 21      | Prise en charge de la propriété *storeCertificateName* sous Windows    |
...
| certificatesFolder     | [Folder](FolderClass.md)                           | Définit le dossier actif des certificats du client. Peut être remplacé par "storeCertificateName" (voir ci-dessous).                                                                                                                                                                                                                                                                                                                                                                                                              | undefined          |
...
| storeCertificateName   | Text                                               | Nom d'un certificat stocké dans le Certificate Store (Windows) ou dans le *keychain* (macOS) à utiliser à la place d'un certificat enregistré dans le dossier des certificats. Si le certificat n'est pas trouvé dans le store, une erreur est renvoyée. Pour plus d'informations, voir [ce blog post pour Windows](https://blog.4d.com/https-requests-now-support-windows-certificate-store) et [ce blog post pour macOS](https://blog.4d.com/https-requests-macos-keychain-support-is-here). | undefined          |
...
Sur macOS, lorsqu'une nouvelle application (nouvel [UUID](./FileClass.md#setappinfo)) demande l'accès à la keychain (*trousseau*) pour la première fois, un mot de passe peut être demandé à l'utilisateur, en fonction de la configuration locale du trousseau.
...
Lors d'une exécution `.wait()`, les fonctions de rappel sont exécutées, que ce soit à partir d'autres instances `HTTPRequest` ou [`SystemWorker`](SystemWorkerClass.md), ou d'autres appels [`CALL WORKER`](../commands/call-worker) .  Vous pouvez sortir d'un `.wait()` en appelant [`terminate()`](#terminate) à partir d'une callback.
