...
| storeCertificateName   | Text                                               | (Windows uniquement) Nom du magasin de certificats de l'OS (par exemple "LocalMachine") à partir duquel les certificats doivent être utilisés au lieu de ceux qui se trouvent dans le dossier des certificats. Si le certificate store n'est pas trouvé, une erreur est renvoyée. Pour plus d'informations, voir [cet article de blog](https://blog.4d.com/https-requests-now-support-windows-certificate-store). | undefined          |
