...
Les commandes du langage de 4D vous donnent la possibilité d’accéder aux éditeurs standard de gestion des enregistrements. For example, when you use the `QUERY` command, you are presented with the Query Editor (which can be accessed in the Design mode using the Query command in the Records menu. Vous pouvez, si vous le voulez, indiquer explicitement à la commande Query ce qu’elle doit rechercher. For example, `QUERY([People];[People]Last Name="Smith")` will find all the people named Smith in your database.

Le langage 4D est très puissant — une commande remplace souvent des centaines, voire des milliers de lignes de code écrites dans les langages informatiques traditionnels. Et cette puissance s’accompagne d'une réelle simplicité : les commandes sont écrites en français. For example, to perform a query, you use the `QUERY` command; to add a new record, you use the `ADD RECORD` command.
