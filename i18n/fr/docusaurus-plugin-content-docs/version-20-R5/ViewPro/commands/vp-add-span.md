...
La commande `VP ADD SPAN` combine les cellules dans _rangeObj_ en une seule cellule fusionnée.

Dans _rangeObj_, passez un objet de plage de cellules. Les cellules de la plage sont jointes, afin de créer une cellule plus large qui s'étend sur plusieurs colonnes et/ou lignes. Vous pouvez passer plusieurs plages de cellules pour créer plusieurs fusions de cellules en même temps. A noter que si les plages de cellules se chevauchent, seule la première plage est utilisée.
