...
La commande `VP IMPORT DOCUMENT` <!-- REF #_method_.VP IMPORT DOCUMENT. ummary -->importe et affiche le document désigné par _filePath_ dans la zone 4D View Pro _vpAreaName_<!-- END REF -->. Le document importé remplace toutes les données déjà présentes dans la zone.

Dans _vpAreaName_, passez le nom de la zone 4D View Pro. Si vous passez un nom inexistant, une erreur est retournée.

Dans _filePath_, passez le chemin et le nom du document à importer. Les formats suivants sont pris en charge :
...
Le paramètre optionnel _paramObj_ vous permet de définir les propriétés du document importé:
