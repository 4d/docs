...
| Validation d'une suggestion         | Définit le contexte de saisie qui permet à l'éditeur de code de valider automatiquement la suggestion courante affichée dans la fenêtre d'autocomplétion.<ul><li>**Tab and delimiters**<br/>Lorsque cette option est sélectionnée, vous pouvez valider la sélection courante avec la touche Tab ou tout délimiteur qui est pertinent pour le contexte. Par exemple, si vous entrez "ALE" et ensuite "(", 4D écrit automatiquement "ALERT(" dans l'éditeur. Voici la liste des délimiteurs pris en compte :<br/> ( ; : = < [ {</li><li>**Tab uniquement**<br/>Lorsque cette option est sélectionnée, vous ne pouvez utiliser que la touche Tab pour insérer la suggestion en cours. Ceci peut être utilisé plus particulièrement pour faciliter la saisie de caractères délimiteurs dans les noms d'éléments, tels que ${1}.**Note** : Vous pouvez également double-cliquer dans la fenêtre ou appuyer sur la touche Retour chariot pour valider une suggestion.</li></ul> |
