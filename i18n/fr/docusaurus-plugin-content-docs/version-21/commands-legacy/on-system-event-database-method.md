...
* sélection via le raccourci clavier **Alt+Tab** (Windows) ou **Commande+Tab** (macOS),
* sélection de la commande **Masquer** dans le dock (macOS),
...
Vous devez impérativement déclarer le paramètre $event (entier long) dans la méthode base. La structure du code de la méthode base sera donc :
