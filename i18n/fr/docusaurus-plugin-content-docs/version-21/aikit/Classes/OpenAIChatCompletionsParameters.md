...
La classe `OpenAIChatCompletionsParameters` permet de gérer les paramètres requis pour les générations de réponses conversationnelles en utilisant l'API OpenAI.
...
| `top_p`                 | Real       | `-1`              | Une alternative à l'échantillonnage en fonction de la température, appelée « échantillonnage par noyau », dans laquelle le modèle prend en compte les résultats des tokens présentant la masse de probabilité top_p. Ainsi, 0,1 signifie que seuls les tokens constituant la masse de probabilité la plus élevée de 10% sont pris en compte. Envoyé uniquement lorsque la valeur est supérieure à 0 (omis lorsque `<= 0`, avec la valeur par défaut `-1`). |
...
| `service_tier`          | Text       | `Null`            | Spécifie le type de traitement utilisé pour répondre à la requête. `"auto"`, `"auto"`, `"default"`, et `"priority"`.                                                                                                                                                                                                                                                                                                                                                                                               |
