...
| encryption  | Integer                     | The encryption to use if a password is set:<ul><li>`ZIP Encryption AES128`: AES encryption using 128-bit key.</li><li>`ZIP Encryption AES192`: Cryptage AES avec une clé de 192 bits.</li><li>`ZIP Encryption AES256`: Cryptage AES à l'aide d'une clé de 256 bits (par défaut si un mot de passe est défini).</li><li>`ZIP Encryption none`: Data is not encrypted (default if no password is set)</li></ul> |
