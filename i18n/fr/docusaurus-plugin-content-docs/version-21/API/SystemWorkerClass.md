...
| variables        | Object  |            | Définit des variables d'environnement personnalisées pour le system worker. Définit des variables d'environnement personnalisées pour le system worker. Les valeurs sont converties en chaînes de caractères lorsque cela est possible. La valeur ne peut pas contenir un '='. S'il n'est pas défini, le system worker hérite de l'environnement 4D. |
...
La propriété `.dataType` <!-- REF #SystemWorkerClass.dataType.Summary -->contient le type du contenu du body de la réponse<!-- END REF -->. .
