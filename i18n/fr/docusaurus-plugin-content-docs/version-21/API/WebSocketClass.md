...
| onError     | [Function](FunctionClass.md) | Fonction de callback pour les erreurs d'exécution. Le callback reçoit les paramètres suivants :<li>`$1`: objet WebSocket</li><li>`$2`: Objet</li><ul><li>`$2.type` (texte) : toujours "error"</li><li>`$2.errors`: collection de piles d'erreurs 4D en cas d'erreur d'exécution.<ul><li>`[].errCode` (nombre) : Code d'erreur 4D</li><li>`[].message` (texte) : Description de l'erreur 4D</li><li>`[].componentSignature` (texte) : Signature du composant interne qui a renvoyé l'erreur</li></ul></li></ul> |
| onTerminate | [Function](FunctionClass.md) | Fonction de callback lorsque la WebSocket est terminée. La callback reçoit les paramètres suivants :<li>`$1` : Objet WebSocket</li><li>`$2` : Objet</li><ul> <li>`$2.code` (numérique, lecture seulement) : unsigned short contenant le code de fermeture envoyé par le serveur.</li><li>`$2.reason` (texte, lecture seulement) : Raison pour laquelle le serveur a fermé la connexion. Elle est spécifique au serveur et au sous-protocole particulier.</li></ul>                                                                                                   |
...
| headers     | Object                       | En-têtes du WebSocket.<li>Syntaxe pour l'affectation d'une clé standard : `headers.*key*:=*value*` (*value* peut être une collection si la même clé apparaît plusieurs fois)</li><li>Syntaxe pour l'affectation d'un Cookie (cas particulier) : `headers.Cookie:="*nom*=*valeur* { ; *nom2*=*valeur2*{ ; ... } }"`</li>                                                                                                                                                                                                                                                                                                                                              |
