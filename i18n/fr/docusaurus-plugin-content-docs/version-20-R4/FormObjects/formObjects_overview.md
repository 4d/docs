...
*   **Code JSON dans les formulaires dynamiques :** Définissez les propriétés à l'aide du JSON. Use the [type](properties_Object.md#type) property to define the object type, then set its [available properties](properties_Reference.md). Exemple d'un objet bouton :
