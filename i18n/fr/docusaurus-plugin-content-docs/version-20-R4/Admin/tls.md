...
Pour que les connexions TLS soient acceptées par le serveur HTTP, vous devez activer HTTPS. Veuillez consulter le paragraphe [**Activer HTTPS**](WebServer/webServerConfig.md#enable-https) pour découvrir les différentes façons d'activer TLS pour le serveur HTTP.
