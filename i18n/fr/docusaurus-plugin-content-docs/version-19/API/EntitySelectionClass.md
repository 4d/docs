...
* Si vous passez *entity* comme paramètre, vous combinez cette entité avec l'entity selection. Si l'entité appartient à l'entity selection, une nouvelle référence à l'entity selection est retournée. S'il n'y a pas d'intersection d'entité, une entity selection vide est retournée.
...
La fonction `.average()` <!-- REF #EntitySelectionClass.average().Summary -->La fonction `.average()`<!-- END REF -->.
...
La fonction `.count()` <!-- REF #EntitySelectionClass.count().Summary -->La fonction `.count()`<!-- END REF -->.
...
La fonction `.distinct()` <!-- REF #EntitySelectionClass.distinct().Summary -->La fonction `.distinct()`<!-- END REF -->.
...
La fonction `.drop()` <!-- REF #EntitySelectionClass.drop().Summary -->La fonction `.drop()`<!-- END REF -->. L'entity selection reste en mémoire.
...
La fonction `.extract()` <!-- REF #EntitySelectionClass.extract().Summary -->La fonction `.extract()`<!-- END REF -->.
...
La fonction `.getDataClass()` <!-- REF #EntitySelectionClass.getDataClass().Summary -->La fonction `.getDataClass()`<!-- END REF -->.
...
La propriété `.length` <!-- REF #EntitySelectionClass.length.Summary -->La propriété `.length`<!-- END REF -->. Si l'entity selection est vide, elle contient 0.
...
La fonction `.max()` <!-- REF #EntitySelectionClass.max().Summary -->La fonction `.max()`<!-- END REF -->. Autrement dit, elle retourne la valeur de la dernière entité de la sélection si elle était triée par ordre croissant avec la fonction [`.orderBy()`](#orderby).
...
La fonction `min()` <!-- REF #EntitySelectionClass.min().Summary --> La fonction `min()`<!-- END REF -->.  Autrement dit, elle retourne la valeur de la première entité de la sélection si elle était triée par ordre croissant avec la fonction [`.orderBy()`](#orderby) (hors valeurs **Null**).
...
La fonction `.query()` <!-- REF #EntitySelectionClass.query().Summary -->La fonction `.query()`<!-- END REF -->, et renvoie un nouvel objet de type `EntitySelection` contenant toutes les entités trouvées. Le mode lazy loading est appliqué.
> Cette fonction ne modifie pas l'entity selection d'origine.
...
La propriété `.queryPlan` <!-- REF #EntitySelectionClass.queryPlan.Summary --> La propriété `.queryPlan`<!-- END REF -->. Cette propriété est disponible pour les objets de type `EntitySelection` générés via des recherches si la propriété `"queryPlan":true` a été passée dans le paramètre *querySettings* de la fonction [`.query()`](#query).
...
La fonction `.sum()` <!-- REF #EntitySelectionClass.sum().Summary -->La fonction `.sum()`<!-- END REF -->.
