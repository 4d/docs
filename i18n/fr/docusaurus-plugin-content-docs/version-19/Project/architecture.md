...
A 4D project is made of several folders and files, stored within a single project root folder (package folder). Par exemple :
...
| settings.4DSettings     | Propriétés de la base *Structure*. Propriétés de la base *Structure*. Pour le déploiement, il est nécessaire d'utiliser les *paramètres utilisateur* ou les *paramètres utilisateur pour les données* afin de définir des paramètres personnalisés. | XML       |
