...
Une zone de plug-in est une zone du formulaire entièrement contrôlée par un plug-in. La capacité d’intégrer des plug-ins dans les formulaires permet d’accéder à des possibilités illimitées lorsque vous créez des applications personnalisées.

Lorsque vous ouvrez une application, 4D crée une liste interne des plug-ins [installés dans votre application](../Concepts/plug-ins.md#how-to-install-a-plug-in). Une fois que vous avez inséré une Zone de plug-in dans un formulaire, vous pouvez sélectionner le plug-in à lui affecter via la liste **Type** dans la fenêtre de propriétés de l’objet :
