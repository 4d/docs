...
- Les **applications client/serveur fusionnées** sont construites avec le [Générateur d'applications](building.md#clientserver-page). Elles sont utilisées pour les déploiements d'applications.
...
Ces principes sont détaillés dans la page [Générateur d'application](building.md#what-is-a-clientserver-application).
...
1. Sélectionnez **Connectez-vous à 4D Server** dans l'assistant de bienvenue, OU Sélectionnez **Ouvrir/Projet distant...** dans le menu **Fichier** ou le bouton **Ouvrir** de la barre d'outils.
...
- **Adresse réseau** : L'adresse IP de la machine sur laquelle le 4D Server a été lancé. Si deux serveurs sont exécutés simultanément sur la même machine, l'adresse IP doit être suivie de deux points et d'un numéro de port, par exemple : `192.168.92.104:19814`. Par défaut, le port de publication d'un 4D Server est 19813. Ce numéro peut être modifié dans les paramètres du projet.
