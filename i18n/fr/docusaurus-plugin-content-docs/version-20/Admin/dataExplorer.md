...
- La partie centrale contient la **Zone de recherche** et la **Grille de données** (liste des entités de la dataclass sélectionnée). Chaque colonne de la grille représente un attribut du datastore.
  - Par défaut, toutes les entités sont affichées. Vous pouvez filtrer les entités affichées en utilisant la zone de recherche. Deux modes de requête sont disponibles : [Recherche sur les attributs](#query-on-attributes) (sélectionné par défaut), et la [Recherche avancée avec expression](#advanced-queries-with-expression). Deux modes de requête sont disponibles : [Requête sur les attributs](#query-on-attributes) (sélectionné par défaut), et [Requête avancée avec expression](#advanced-queries-with-expression). ![alt-text](../assets/en/Admin/dataExplorer4b.png)
