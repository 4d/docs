...
Il est possible d’appeler des méthodes 4D depuis le code JavaScript exécuté dans une zone Web et de recevoir des valeurs en retour. Il est possible d’appeler des méthodes 4D depuis le code JavaScript exécuté dans une zone Web et de recevoir des valeurs en retour.
...
*   **coché** - `valeur JSON : embedded` : Dans ce cas, 4D utilise Chromium Embedded Framework (CEF). L’utilisation d'un moteur Web intégré vous permet d’avoir l’assurance que le rendu et le fonctionnement des zones Web de votre application seront quasiment identiques, quelle que soit la plate-forme d’exécution de 4D (de légères variations de pixels ou des différences liées à l’implémentation réseau pourront toutefois être constatées). Lorsque cette option est choisie, vous ne bénéficiez plus des mises à jour automatiques du moteur Web effectuées par le système d'exploitation ; cependant, de [nouvelles versions des moteurs sont régulièrement fournies par l'application 4D](../Notes/updates.md#library-table-4d-20-lts).

Le moteur CEF a les limitations suivantes :

- [WA SET PAGE CONTENT](https://doc.4d.com/4dv20/help/command/en/page1037.html): l'utilisation de cette commande nécessite qu'au moins une page soit déjà chargée dans la zone (par le biais d'un appel à la commande [`WA OPEN URL`](https://doc.4d.com/4dv20/help/command/en/page1020.html) ou d'une affectation à la variable URL associée à la zone).
- Lorsque les déposer d'URL sont autorisés via le sélecteur `WA enable URL drop` de la commande [WA SET PREFERENCE](https://doc.4d.com/4dv20/help/command/en/page1041.html), le premier déposer doit être précédé d'au moins un appel à la commande [WA OPEN URL](https://doc.4d.com/4dv20/help/command/en/page1020.html) ou d'une affectation à la variable d'URL associée à la zone.
