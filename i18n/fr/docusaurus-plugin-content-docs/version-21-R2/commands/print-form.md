...
La valeur retournée par **Print form** indique la hauteur de la zone d’impression. Cette valeur sera automatiquement prise en compte par la commande [Get printed height](./commands/get-printed-height).
...
- Appeler [PRINT SETTINGS](./commands/print-settings). Dans ce cas, vous laissez l'utilisateur définir ses paramètres dans les boîtes de dialogue d'impression.
- Appeler [SET PRINT OPTION](./commands/set-print-option) et [GET PRINT OPTION](./commands/get-print-option). Dans ce cas, les paramètres sont définis par programmation.

**Print form** construit chaque page à imprimer en mémoire. Chaque page est imprimée lorsque la page en mémoire est remplie ou lorsque vous appelez [PAGE BREAK](./commands/page-break). Pour vous assurer que la dernière page d'une impression exécutée par l'intermédiaire de **Print form** est effectivement imprimée, il faut terminer par la commande [PAGE BREAK](./commands/page-break) (sauf dans le cadre d'un [OPEN PRINTING JOB](./commands/open-printing-job), voir note). Sinon, la dernière page, si elle n'est pas remplie, reste en mémoire et n'est pas imprimée.

**Attention :** Si la commande est appelée dans le contexte d'une tâche d'impression ouverte avec [OPEN PRINTING JOB](./commands/open-printing-job), vous ne devez PAS appeler [PAGE BREAK](./commands/page-break) pour la dernière page car celle-ci est automatiquement imprimée par la commande [CLOSE PRINTING JOB](./commands/close-printing-job). Si vous appelez [PAGE BREAK](./commands/page-break) dans ce cas, une page vide est imprimée.
...
**Attention :** **Print form** n'imprime pas les sous-formulaires. Si vous voulez imprimer uniquement un formulaire comportant de tels objets, utilisez plutôt [PRINT RECORD](./commands/print-record).
...
L'exemple suivant effectue la même chose que ce que ferait la commande [PRINT SELECTION](./commands/print-selection). Cependant, l'état utilise deux formulaires différents suivant le type d'enregistrement (chèque émis ou dépôt) :
...
Voir l'exemple de la commande [SET PRINT MARKER](./commands/set-print-marker).
