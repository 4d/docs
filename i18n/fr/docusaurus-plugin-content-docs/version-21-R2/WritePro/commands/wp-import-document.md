...
- anciens documents 4D Write (.4w7 ou .4wt). Pour une liste détaillée des fonctionnalités de 4D Write qui sont actuellement prises en charge dans les objets de 4D Write Pro, veuillez vous référer à la section *Importer des documents 4D Write*.
- Documents au format 4D Write Pro (.4wp). Pour plus d'informations sur le format de document 4D Write Pro, voir *.4wp document format*.
