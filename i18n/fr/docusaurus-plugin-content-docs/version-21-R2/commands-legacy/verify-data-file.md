...
*dataPath* désigne un fichier de données 4D (.4DD). Il doit correspondre au fichier de structure défini par le paramètre *structurePath*. Pour vérifier le fichier de données actuellement ouvert, utilisez la commande [VERIFY CURRENT DATA FILE](verify-current-data-file.md).
