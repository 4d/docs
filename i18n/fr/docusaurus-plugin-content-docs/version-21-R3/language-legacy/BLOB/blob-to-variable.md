...
Les données dans le BLOB doivent être compatibles avec la variable de destination : vous utiliserez généralement des BLOBs que vous avez précédemment remplis à l'aide de [VARIABLE TO BLOB](../commands/variable-to-blob).
...
**Note:** **BLOB TO VARIABLE** prend en charge les variables objet de type *C\_OBJECT* et les variables collection de type *C\_COLLECTION*. Pour plus d'informations, reportez-vous à la commande [VARIABLE TO BLOB](../commands/variable-to-blob).
...
**Note sur l'indépendance de plate-forme :** **BLOB TO VARIABLE** et [VARIABLE TO BLOB](../commands/variable-to-blob) utilisent un format interne à 4D pour gérer les variables stockées dans les BLOBs. Vous n'avez donc pas besoin de vous préoccuper de la conversion des octets ("byte swapping") entre les différentes plates-formes lors de l'utilisation de ces deux commandes. Un BLOB créé sous Windows à l'aide de ces deux commandes peut être réutilisé sans la moindre manipulation sous Mac OS et vice-versa.

## Exemple 

Référez-vous aux exemples de [VARIABLE TO BLOB](../commands/variable-to-blob).
