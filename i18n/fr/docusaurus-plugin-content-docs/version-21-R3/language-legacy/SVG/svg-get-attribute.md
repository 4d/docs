...
Si vous passez le paramètre optionnel *\**, vous indiquez que le paramètre *objetImage* est un nom d’objet (une chaîne). Dans ce cas, la commande retourne la valeur de l’attribut pour l’image de rendu attachée à l’objet. Cette valeur peut avoir été modifiée par [SVG SET ATTRIBUTE](../commands/svg-set-attribute) par exemple.   
...
**Note :** Ce principe s’applique également à la commande [SVG Find element ID by coordinates](../commands/svg-find-element-id-by-coordinates). 
...
Pour plus d’informations sur les attributs SVG, reportez-vous à la description de la commande [SVG SET ATTRIBUTE](../commands/svg-set-attribute). Voici la liste des attributs 4D réservés et dédiés à l’animation :
...
| 4D-bringToFront                               | écriture         | Si 'true', déplacer le noeud devant les noeuds frères. Utilisable uniquement avec la commande [SVG SET ATTRIBUTE](../commands/svg-set-attribute)                                                                                                                                                                                                                                                                                                                                                                                                                    |
