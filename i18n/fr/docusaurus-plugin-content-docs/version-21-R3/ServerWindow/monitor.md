...
- **Cache** : Affiche dans la zone graphique la quantité de mémoire cache utilisée par l’application 4D Server. La valeur située en bas à droite de la zone indique la quantité de mémoire en cours d’utilisation. La valeur située en haut à gauche indique la taille totale de la mémoire cache, telle qu'elle a été définie dans les propriétés.
...
En règle générale, vous devrez consulter le [**Gestionnaire de licences**](Admin/licenses.md).
