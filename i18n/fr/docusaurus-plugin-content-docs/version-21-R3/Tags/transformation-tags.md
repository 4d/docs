...
- En utilisant la commande [`PROCESS 4D TAGS`](../commands/process-4d-tags) ; cette commande accepte un *template* en entrée, ainsi que des paramètres optionnels et renvoie un texte résultant du traitement.
...
- La commande [`TRACE`](../commands/trace) est prise en charge et active le [débogueur 4D](../Debugging/debugger.md), ce qui vous permet de déboguer le code de votre template.
...
#### Exemple avec [`PROCESS 4D TAGS`](../commands/process-4d-tags)
...
Cette syntaxe est utile lorsque vous passez un pointeur de tableau en tant que paramètre à la commande [`PROCESS 4D TAGS`](../commands/process-4d-tags).
...
> Si la balise est appelée dans le contexte d'un process Web, lorsque la page est chargée, 4D appelle la méthode base [`On Web Authentication`](../commands/on-web-authentication-database-method) (si elle existe). Si elle retourne True, 4D exécute la méthode.
