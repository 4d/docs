...
<!--REF #_command_.FILTER EVENT.Summary-->Llame al comando **FILTER EVENT** desde el interior del método de gestión de eventos instalado utilizando el comando [ON EVENT CALL](../commands/on-event-call).<!-- END REF-->
...
**Caso especial:** el comando **FILTER EVENT** puede igualmente utilizarse en un método de formulario estándar cuando el formulario se visualiza utilizando los comandos [DISPLAY SELECTION](../commands/display-selection) o [MODIFY SELECTION](../commands/modify-selection). En este caso específico, el comando **FILTER EVENT** le permite filtrar los doble clics en los registros (y de esta manera ejecutar acciones diferentes a las de apertura de los registros en modo página).
