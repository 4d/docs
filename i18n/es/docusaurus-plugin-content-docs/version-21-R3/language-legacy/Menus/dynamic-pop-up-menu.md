...
El menú jerárquico utilizado debe haber sido creado con el comando [Create menu](../commands/create-menu). La referencia devuelta por [Create menu](../commands/create-menu) debe pasarse en el parámetro *menu*.

**Nota:** el comando [Pop up menu](../commands/pop-up-menu) (tema “Interfaz de usuario”) puede utilizarse parar crear menús pop-up menus basados en texto.
...
El parámetro opcional *pordefecto* puede utilizarse para definir un elemento del menú desplegable seleccionado por defecto cuando aparece el menú. En este parámetro, pase una cadena personalizada asociada a la referencia del elemento de menú. Esta cadena debe haber sido definida de antemano con la ayuda del comando [SET MENU ITEM PARAMETER](../commands/set-menu-item-parameter). Si no pasa este parámetro, el primer elemento del menú será seleccionado por defecto.
...
Si un elemento de menú ha sido seleccionado, el comando devuelve su cadena de caracteres personalizada asociada (tal como se ha definido utilizando el comando [SET MENU ITEM PARAMETER](../commands/set-menu-item-parameter)). De lo contrario, el comando devuelve una cadena vacía.
...
* Si una acción estándar asociada no está activada (es decir, no se puede invocar) en el contexto del menú emergente, el elemento se oculta automáticamente. Puede saber si una acción se activa utilizando el comando [Action info](../commands/action-info).
