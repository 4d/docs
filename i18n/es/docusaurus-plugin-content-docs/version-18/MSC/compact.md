...
> La compactación sólo está disponible en el modo mantenimiento. Si intenta realizar esta operación en modo estándar, una caja de diálogo de alerta le informará de que la base se cerrará y se reiniciará en modo de mantenimiento. Puede compactar un archivo de datos que no esté abierto por la base de datos (ver [Compactar los registros y los índices](#compact-records-and-indexes) abajo).
...
Una vez finalizada la operación, los archivos compactados sustituyen automáticamente a los originales. La base de datos es inmediatamente operacional sin ninguna otra manipulación.
...
Una vez finalizada la compactación, 4D genera un archivo de historial en la carpeta Logs de la base. Este archivo permite ver todas las operaciones realizadas. Se crea en formato XML y se denomina  *DatabaseName**_Compact_Log_yyyy-mm-dd hh-mm-ss.xml*" donde:
...
> - El uso de esta opción hace que la tabla de direcciones, y por tanto la base de datos, sea incompatible con el archivo de diario actual (si existe). Se guardará automáticamente y habrá que crear un nuevo archivo de historial la próxima vez que se inicie la base.
