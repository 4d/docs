...
A partir de la nightly build **101734**, el diálogo Build application tiene una nueva opción que permite crear aplicaciones de evaluación. Ver [la descripción en la documentación de 4D Rx](../../../docs/Desktop/building#build-an-evaluation-application).
...
- 4D Server integra automáticamente varios historiales: [Restauración automática](../Backup/settings.md#automatic-restore-and-log-integration).
...
- Nueva opción `validateTLSCertificate` para [`4D.HTTPRequest.new()`](../API/HTTPRequestClass.md#4dhttprequestnew) que permite controlar la validación automática del certificado.
...
- Los certificados TLS son ahora validados automáticamente por 4D al enviar peticiones HTTP con [`4D.HTTPRequest.new()`](../API/HTTPRequestClass.md#4dhttprequestnew) y rechazados con un error si no son válidos. Una nueva propiedad *option* le permite controlar esta validación.
...
- Para beneficiarse de las funcionalidades de desarrollo de 4D for Mobile en 4D, necesita [instalar el componente 4D Mobile App](https://developer.4d.com/go-mobile/docs/getting-started/installation) en la carpeta ["Components"](../Project/architecture.md#components) de sus proyectos. Si un proyecto convertido utiliza funciones del [componente 4D Mobile App Server](https://github.com/4d/4D-Mobile-App-Server#4d-mobile-app-server), asegúrese de instalarlo también en la carpeta "Componentes" del proyecto. A partir de 4D 20, [4D for Mobile](https://developer.4d.com/go-mobile/) ya no se instala por defecto en el entorno 4D.
...
- Nuevo archivo de registro [4DHTTPClientLog.txt](../Debugging/debugLogFiles.md#4dhttpclientlogtxt).
...
```4d
$value:=($size>1000)? a:b // Aquí 'a:b' se considera un operador ternario. a:b // Aquí 'a:b' se considera un operador ternario.
```
