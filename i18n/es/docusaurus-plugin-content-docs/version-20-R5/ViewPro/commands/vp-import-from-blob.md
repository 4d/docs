...
El comando `VP IMPORT FROM BLOB` <!-- REF #_method_.VP IMPORT FROM BLOB.Summary -->importa el _vpBlob_ del área _vpAreaName_ de 4D View Pro y reemplaza su contenido<!-- END REF -->. _vpBlob_ debe contener un documento 4D View Pro previamente guardado como Blob ya sea utilizando el comando [VP EXPORT TO BLOB](vp-export-to-blob.md) o a través de la interfaz de 4D View Pro.
