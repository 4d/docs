...
| encryption   | Integer                     | The encryption to use if a password is set:<ul><li>`ZIP Encryption AES128`: AES encryption using 128-bit key.</li><li>`ZIP Encryption AES192`: cifrado AES con una clave de 192 bits.</li><li>`ZIP Encryption AES256`: encriptación AES con una llave de 256 bits (por defecto si se define la contraseña).</li><li>`ZIP Encryption none`: Data is not encrypted (default if no password is set)</li></ul>  |
