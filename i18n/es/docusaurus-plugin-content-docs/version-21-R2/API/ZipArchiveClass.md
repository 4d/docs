...
- Puede crear un archivo ZIP 4D con el comando [`ZIP Create archive`](./commands/zip-create-archive).
- Las instancias 4D [`ZIPFile`](ZipFileClass.md) y [`ZIPFolder`](ZipFolderClass.md) están disponibles a través de la propiedad [`root`](#root) (`ZIPFolder`) del objeto devuelto por el comando [`ZIP Read archive`](./commands/zip-read-archive).
