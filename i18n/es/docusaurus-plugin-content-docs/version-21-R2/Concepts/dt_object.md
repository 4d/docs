...
- objetos regulares (no compartidos), utilizando el comando [`New object`](./commands/new-object) o la sintaxis literal de objeto (`{}`). Estos objetos pueden ser editados sin ningún control de acceso específico, pero no pueden ser compartidos entre procesos.
