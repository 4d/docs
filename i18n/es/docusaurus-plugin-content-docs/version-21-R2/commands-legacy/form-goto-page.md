...
Si ningún formulario es mostrado o cargado por el comando [FORM LOAD](./commands/form-load) o si *numPag* corresponde a la página actual del formulario, **FORM GOTO PAGE** no hace nada. Si *numPag* es superior que el número de páginas del formulario, se muestra la última página. Si *numPag* es menor que uno, se muestra la primera página.
...
* Utilice [Form event code](./commands/form-event-code) On Page Change que se genera cada vez que la página actual del formulario cambia.
