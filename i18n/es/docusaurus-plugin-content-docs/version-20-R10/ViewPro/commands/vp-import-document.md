...
|              | openMode                  | integer                     | <li>0 (normal): modo abierto normal, sin perezoso (lazy) e incremental. When opening file, UI and UI event could be refreshed and responsive at specific time points.</li><li>1 (lazy): lazy open mode. Al abrir el archivo, sólo se cargará directamente la hoja activa. Other sheets will be loaded only when they are be used.</li><li>2 (incremental): incremental open mode. Al abrir un archivo, la interfaz de usuario y el evento de interfaz de usuario podrían actualizarse y responder directamente.</li> |
