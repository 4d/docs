...
> Ciertos objetos de formulario pueden activarse con el teclado. Por ejemplo, una vez que una casilla de selección recibe el foco, se puede introducir con la barra espaciadora. In such a case, the `On Clicked` event is still generated.
...
Este evento se genera cuando el usuario hace clic en cualquier parte en un documento 4D View Pro. On this context, the [event object](overview.md#event-object) returned by the `FORM Event` command contains:
