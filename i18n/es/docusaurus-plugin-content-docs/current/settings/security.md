...
Da al grupo especificado la capacidad de entrar en el entorno Diseño de la base de datos y mostrar el Explorador de ejecución.
...
When a Default User has been set, every user that opens the database or logs onto it has the same access privileges and restrictions defined for this Default User. Ya no es necesario ingresar un nombre de usuario. Además, si no ha asociado una contraseña al usuario por defecto, la caja de diálogo Contraseña ya no aparece y la base se abre directamente.
...
- Si no ha asociado una contraseña al usuario por defecto, el cuadro de diálogo de identificación de usuario no aparecerá.  
  **Note:** You can "force" the display of the User Identification dialog box when the "Default User" mode is active, for instance in order to connect as Administrator or Designer. Para ello, presione la tecla **Mayús** mientras abre la base de datos o se conecta a ella.

#### Mostrar la lista de usuarios en el cuadro de diálogo Contraseña

Si se marca esta opción, los usuarios deben elegir su nombre de la lista de usuarios e introducir su contraseña en el cuadro de diálogo Identificación de usuario. Si no está marcada, los usuarios deben introducir tanto su nombre como su contraseña. Para obtener más información sobre las dos versiones de la caja de diálogo de contraseña, consulte la sección "Acceso a las bases protegidas" en [Presentación del control de acceso](https://doc.4d.com/4Dv20/4D/20.2/Access-system-overview.300-6750353.en.html

#### Lista de usuarios en orden alfabético (solo disponible si se ha marcado la opción anterior)

Cuando esta opción está marcada, la lista de usuarios del cuadro de diálogo de introducción de contraseña se ordena alfabéticamente.

#### Los usuarios pueden cambiar su contraseña

Cuando esta opción está marcada, se muestra el botón **Cambiar** en el cuadro de diálogo de inicio de sesión de usuario. Si lo desea, puede ocultar el botón **Cambiar** para que los usuarios no puedan modificar sus contraseñas. Este botón permite al usuario acceder a una caja de diálogo que puede utilizar para modificar su contraseña (para más información sobre esta caja de diálogo, consulte "Modificación de la contraseña por el usuario" en [Garantizar el mantenimiento del sistema](https://doc.4d.com/4Dv20/4D/20.2/Ensuring-system-maintenance.300-6750352.en.html)). Para ello, desmarque esta opción.

## Opciones

#### Filtrado de comandos y métodos de proyecto en el editor de fórmulas y en los documentos de 4D View Pro y 4D Write Pro

For security reasons, by default 4D restricts access to the commands, functions and project methods in the [Formula editor](https://doc.4d.com/4Dv20/4D/20.2/Formula-editor.200-6750079.en.html) in Application mode or added to multistyle areas (using [`ST INSERT EXPRESSION`](../commands/st-insert-expression)), 4D Write Pro and 4D View Pro documents: only certain 4D functions and project methods that have been explicitly declared using the [`SET ALLOWED METHODS`](../commands/set-allowed-methods) command can be used. Puede eliminar total o parcialmente este filtrado mediante las siguientes opciones.
...
- **Desactivado para todos**: esta opción desactiva el control en las fórmulas. Cuando esta opción está marcada, los usuarios tienen acceso a todos los comandos 4D, plug-ins y métodos proyecto (excepto los invisibles).
  Cuando esta opción está marcada, los usuarios tienen acceso a todos los comandos 4D, plug-ins y métodos proyecto (excepto los invisibles). Cuando se selecciona, este comando no hace nada.

#### Activar la configuración de usuario

Debe seleccionar esta opción para poder mostrar cuadros de diálogo independientes para la configuración de usuario. Cuando esta opción está marcada, hay disponibles hasta tres cuadros de diálogo: **Propiedades estructura**, **Propiedades usuario**, y **Propiedades usuario para archivo de datos**. Para más información, consulte [Parámetros usuario](../settings/overview.md#user-settings).

#### Ejecutar el método "On Host Database Event" de los componentes

The [On Host Database Event database method](../commands/on-host-database-event-database-method) facilitates the initialization and backup phases for 4D components. Por razones de seguridad, debe autorizar explícitamente la ejecución de este método en cada base de datos local. Para hacer esto, debe marcar esta opción. Por defecto, no está marcado.
