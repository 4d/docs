...
Pase el nombre de la impresora a seleccionar en el parámetro *nomImpr*. Para obtener una lista de impresoras disponibles, utilice de antemano el comando [PRINTERS LIST](../commands/printers-list). Si pasa una cadena vacía en *nomImpr*, se utilizará la impresora actual definida en el sistema.
...
| ------------------ | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Generic PDF driver | \_4d\_pdf\_printer | **Nota:** esta funcionalidad no está disponible en las versiones 32 bits de 4D.<br/>En macOS, declara el driver predeterminado como impresora actual. Este driver no es visible y no está en la lista devuelta por el comando [PRINTERS LIST](../commands/printers-list). la ruta de acceso al documento PDF se debe definir utilizando el comando [SET PRINT OPTION](../commands/set-print-option), si no, se devuelve el error 3107.En Windows, declara el driver PDF de Windows (llamado "Microsoft Print to PDF") como impresora actual. Esta constante está disponible en Windows 10 únicamente, cuando está instalada la opción PDF. Con otras versiones de Windows, o cuando no hay ningún driver PDF disponible, el comando no hace nada y la variable *OK* toma el valor 0. |

El comando **SET CURRENT PRINTER** debe llamarse antes de [SET PRINT OPTION](../commands/set-print-option), de manera que las opciones disponibles correspondan a la impresora seleccionada. Por otra parte, **SET CURRENT PRINTER** debe llamarse después de *\_o\_PAGE SETUP*, de lo contrario los parámetros de la impresora se pierden.

Este comando puede utilizarse con los comandos [PRINT SELECTION](../commands/print-selection), [PRINT RECORD](../commands/print-record), [Print form](../commands/print-form) y [QR REPORT](../commands/qr-report), y se aplica a todas las impresiones de 4D, incluyendo en el modo Diseño.  
