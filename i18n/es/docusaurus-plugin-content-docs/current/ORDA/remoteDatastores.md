...
Cuando se trabaja con un datastore remoto referenciado a través de llamadas al comando [`Open datastore`](../commands/open-datastore), la conexión con los procesos solicitantes se gestiona a través de [sesiones web](../WebServer/sessions.md) en la máquina remota.
...
- Los comandos clásicos del lenguaje 4D ([`START TRANSACTION`](../commands-legacy/start-transaction), [`VALIDATE TRANSACTION`](../commands-legacy/validate-transaction), [`CANCEL TRANSACTION`](../commands-legacy/cancel-transaction)) sólo se aplican al datastore principal (devuelto por `ds`).
