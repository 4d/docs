...
- **Usuario 4D**: nombre del usuario 4D, o alias si se ha establecido con el comando [`SET USER ALIAS`](../commands-legacy/set-user-alias) en la máquina del usuario. Si las contraseñas no están activadas y no se ha definido ningún alias, todos los usuarios se denominan "Diseñador".
...
> Una sesión remota durmiente es abandonada automáticamente por el servidor tras 48 horas de inactividad. Puede modificar este tiempo de espera por defecto utilizando el comando [`SET DATABASE PARAMETER`](../commands-legacy/set-database-parameter) con el selector `Remote connection sleep timeout`.
...
> Puede realizar la misma acción para usuarios remotos con el comando [`SEND MESSAGE TO REMOTE USER`](../commands-legacy/send-message-to-remote-user).
...
> Puede realizar la misma acción para usuarios remotos con el comando [`DROP REMOTE USER`](../commands-legacy/drop-remote-user).
