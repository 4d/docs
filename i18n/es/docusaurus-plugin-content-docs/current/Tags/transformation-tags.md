...
- Utilizando el comando [`PROCESS 4D TAGS`](./commands/process-4d-tags); este comando acepta una *plantilla* como entrada, así como parámetros opcionales y devuelve un texto resultante del procesamiento.
...
- El comando [`TRACE`](./commands/trace) está soportado y activa el [depurador 4D](../Debugging/debugger.md), permitiéndole así depurar el código de su plantilla.
...
- El texto se tokeniza en el contexto de la base de datos que llamó [`PROCESS 4D TAGS`](./commands/process-4d-tags). Esto es importante para el reconocimiento de los métodos de proyecto, por ejemplo. No se tiene en cuenta la propiedad del método [Disponible a través de etiquetas y URL 4D (4DACTION ...)](WebServer/allowProject.md).
...
#### Ejemplo con [`PROCESS 4D TAGS`](./commands/process-4d-tags)
...
Esta sintaxis es útil cuando se pasa un puntero de array como parámetro al comando [`PROCESS 4D TAGS`](./commands/process-4d-tags).
...
> Si se llama a la etiqueta en el contexto de un proceso Web, cuando se carga la página, 4D llama al método base [`On Web Authentication`](./commands/on-web-authentication-database-method) (si existe). Si devuelve True, 4D ejecuta el método.
