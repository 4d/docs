...
La clase `OpenAIChatCompletionsParameters` está diseñada para manejar los parámetros necesarios para completar el chat utilizando la API OpenAI.
...
| ----------------------- | ---------- | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `model`                 | Text       | `"gpt-4o-mini"`   | ID del modelo a utilizar. Soporta [provider:model aliases](../provider-model-aliases.md) para uso multi-proveedor (por ejemplo, `"openai:gpt-4o"`, `"anthropic:claude-3-opus"`).                                                                                                                                                                                                                                      |
...
| `top_p`                 | Real       | `-1`              | Una alternativa al muestreo con temperatura, llamado muestreo de núcleos, donde el modelo considera los resultados de los tokens con masa de probabilidad superior_p. Por lo tanto, 0,1 significa que sólo se consideran los tokens que componen la masa superior del 10% de probabilidad. Solo se envía cuando el valor es mayor que 0 (se omite cuando `<= 0`, con el valor por defecto `-1`). |
...
| `service_tier`          | Text       | `Null`            | Especifica el tipo de procesamiento utilizado para servir la petición. `"auto"`, `"auto"`, `"default"` y `"priority"`.                                                                                                                                                                                                                                                                                                                                   |

### Propiedades de retrollamada asíncrona

\| Propiedad | Tipo | Descripción |
\|---------------------------|---------|-------------------------|---------------------------------------------------------------------------------------------------|
\| `onData`<br>(o `formula`) | 4D.Function | Una función que se llamará asíncronamente al recibir un trozo de datos.<br>*Asegúrese de que el proceso actual no termina.*
