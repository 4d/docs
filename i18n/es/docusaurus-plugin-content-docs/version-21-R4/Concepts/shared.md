...
- para crear un objeto compartido, utilice el comando [`New shared object`](../commands/new-shared-object) o llama a la función [`new()`](../API/ClassClass.md#new) de una [clase compartida](./classes.md#shared-classes),
- para crear una colección compartida, utilice el comando [`New shared collection`](../commands/new-shared-collection).
...
Un catálogo único y global devuelto por el comando [`Storage`](../commands/storage) siempre está disponible en toda la aplicación y sus componentes, y puede ser utilizado para almacenar todos los objetos compartidos y colecciones.
...
Todas las instrucciones de modificación en un objeto compartido o colección requieren estar protegidas dentro de un bloque [`Use...End use`](#useend-use), de lo contrario se genera un error.
...
<<<<<<<< HEAD:i18n/es/docusaurus-plugin-content-docs/version-21-R4/Concepts/shared.md
...
========
**Storage** es un objeto compartido único, disponible automáticamente en cada aplicación y máquina. Puede utilizar este objeto para hacer referencia a todos los objetos/colecciones compartidos definidos durante la sesión que desee que estén disponibles desde cualquier proceso preventivo o estándar. Este objeto compartido es devuelto por el comando <a href="https://doc.4d.com/4dv19R/help/command/en/page1525.html"><code>Storage</code></a>.
>>>>>>>> l10n_main:i18n/es/docusaurus-plugin-content-docs/version-21-R2/Concepts/shared.md
...
Los objetos compartidos y las colecciones compartidas están diseñados para permitir la comunicación entre procesos, en particular, *[Procesos apropiativos](../Develop/preemptive.md)*. Se pueden pasar por referencia como parámetros de un proceso a otro. Es obligatorio rodear las modificaciones en los objetos o colecciones compartidas con las palabras clave `Use...End use` para evitar el acceso concurrente entre procesos.
...
- comando [`ARRAY TO COLLECTION`](../commands/array-to-collection),
- comando [`OB REMOVE`](../commands/ob-remove),
