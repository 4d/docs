...
<!--REF #_command_.QUERY SELECTION.Syntax-->**QUERY SELECTION** ( *tabla* : Table {; *criterioBusqueda* : Expression } {; *} )<br/>**QUERY SELECTION** ( *criterioBusqueda* : Expression {; *} )<!-- END REF-->
...
**QUERY SELECTION** funciona y realiza las mismas acciones que [QUERY](../commands/query). La diferencia entre los dos comandos es el alcance de la búsqueda:

* [QUERY](../commands/query) busca registros entre los registros en la tabla.
* **QUERY SELECTION** busca registros entre los registros de la selección actual de la tabla.

Para mayor información, consulte la descripción del comando [QUERY](../commands/query).

El comando **QUERY SELECTION** es útil cuando una búsqueda no se puede definir utilizando una secuencia de llamadas enlazadas [QUERY](../commands/query) con el parámetro *\**. Por lo general, es el caso cuando desea consultar una selección actual que no sea resultado de una consulta previa, pero a partir de un comando como [USE SET](../commands/use-set). 
