...
| stopCar &#124; numBytes | Text, Integer | &#8594; | Carácter(es) en el(los) cual(es) detener la recepción de datos o número de bytes a recibir |
</div>
<!-- END REF-->
...
Si se especifica *docRef*, el comando recupera los caracteres desde un documento abierto por la función [Open document](../commands/open-document), [Create document](../commands/create-document) o [Append document](../commands/append-document). Si se omite *docRef*, este comando recupera los caracteres de un puerto serial o de un documento abierto utilizando [SET CHANNEL](../commands/set-channel).

Sin importar la fuente, los caracteres leídos se devuelven en la variable *varRecep*, la cual debe ser una variable de tipo Texto, Alfa o BLOB. Si los caracteres han sido enviados por el comando [SEND PACKET](../commands/send-packet), el tipo debe corresponder al del paquete enviado. 

**Notas:**

* Cuando el paquete recibido es de tipo BLOB, el comando no tiene en cuenta ningún conjunto de caracteres definido por el comando [USE CHARACTER SET](../commands/use-character-set). Se devuelve el BLOB sin ninguna modificación.
...
* Cuando RECEIVE PACKET lee datos de un puerto serial, el comando se ejecutará indefinidamente hasta que el timeout (si hay) haya pasado (ver [SET TIMEOUT](../commands/set-timeout)) o hasta que el usuario interrumpa la recepción (ver a continuación).

Durante la ejecución de RECEIVE PACKET, el usuario puede interrumpir la recepción presionando Ctrl-Alt-Mayús (Windows) o Comando-Opción-Mayús (Macintosh). Esta interrupción genera un error -9994 que puede interceptar con un método instalado utilizando [ON ERR CALL](../commands/on-err-call). Generalmente, sólo tendrá que administrar la interrupción de una recepción en el momento de una comunicación sobre un puerto serial.
...
**Nota:** este comando es útil con un documento abierto con [SET CHANNEL](../commands/set-channel). Por el contrario, para un documento abierto con [Open document](../commands/open-document), [Create document](../commands/create-document) o [Append document](../commands/append-document) puede utilizar los comandos [Get document position](../commands/get-document-position) y [SET DOCUMENT POSITION](../commands/set-document-position) para obtener y cambiar la ubicación en el documento donde ocurrirá la próxima escritura ([SEND PACKET](../commands/send-packet)) o lectura (RECEIVE PACKET).
