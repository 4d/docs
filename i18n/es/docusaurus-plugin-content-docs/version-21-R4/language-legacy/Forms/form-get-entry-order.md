...
* Si no pasa el parámetro *\**, **FORM GET ENTRY ORDER** devuelve el orden de entrada como se declaró anteriormente con el comando [FORM SET ENTRY ORDER](../commands/form-set-entry-order). Puede omitir o pasar el parámetro *numPag*:  
...
En ambos casos, si el comando [FORM SET ENTRY ORDER](../commands/form-set-entry-order) no fue llamado previamente para el formulario actual, el array *nomObjetos* se devuelve vacío.
* Si pasa el parámetro *\**, **FORM GET ENTRY ORDER** devuelve el orden de entrada actual de la página actual, es decir, el array *nomObjetos* sólo contiene nombres de objeto **válidos** ( (para más información sobre objetos válidos, consulte la descripción del comando [FORM SET ENTRY ORDER](../commands/form-set-entry-order)). El orden de entrada de formulario real puede ser:  
   ...
   * O el orden de entrada definido por una llamada al comando [FORM SET ENTRY ORDER](../commands/form-set-entry-order) en el proceso actual, si se ha utilizado.  
