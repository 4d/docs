...
El **objeto de lenguaje** está referenciado por un ID interno único de tipo Entero largo, designado por *ListRef* en el manual de Lenguaje 4D. Este ID lo devuelven los comandos que se pueden utilizar para crear listas: [`New list`](../commands/new-list), [`Copy list`](../commands/copy-list), [`Load list`](../commands/load-list), [`BLOB to list`](../commands/blob-to-list). Sólo hay una instancia del objeto lenguaje en la memoria y cualquier modificación realizada en este objeto se traslada inmediatamente a todos los lugares donde se utiliza.
...
Por consiguiente, cuando se utilizan comandos basados en la configuración expandida/colapsada o en el elemento actual, por ejemplo [`Count list items`](../commands/count-list-items) (cuando no se pasa el parámetro final `*`), es importante poder especificar la representación que se utilizará sin ninguna ambigüedad.
...
Al igual que con [otros comandos de gestión de propiedades de objeto](../FormObjects/formObjects_overview.md#accessing-form-objects-using-their-name-or-their-data-source-in-the-4d-language), es posible usar el carácter “@” en el parámetro `ListName`. Por regla general, esta sintaxis se utiliza para designar un conjunto de objetos del formulario. Sin embargo, en el contexto de los comandos de listas jerárquicas, esto no se aplica en todos los casos. Esta sintaxis tendrá dos efectos diferentes según el tipo de comando:
...
> Recordatorio: excepto [`OBJECT SET SCROLL POSITION`](../commands/object-set-scroll-position), estos comandos modifican todas las representaciones de la misma lista, incluso si sólo especifica una lista a través de su nombre de objeto.
...
1. No es necesario identificar cada elemento con un número único (nivel principiante).
   ...
   - Segundo ejemplo: al trabajar con una base, se construye progresivamente una lista de palabras clave. Puede guardar esta lista al final de cada sesión utilizando los comandos [`SAVE LIST`](../commands/save-list) o [`LIST TO BLOB`](../commands/list-to-blob) y recargarlo al comienzo de cada nueva sesión utilizando los comandos [`Load list`](../commands/load-list) o [`BLOB to list`](../commands/blob-to-list). Puede mostrar esta lista en una paleta flotante; cuando cada usuario hace clic en una palabra clave de la lista, el elemento elegido se inserta en el área introducible que está seleccionada en el proceso en primer plano. Lo importante es que solo procese el elemento seleccionado, ya que el comando [`Selected list items`](../commands/selected-list-items) devuelve la posición del elemento que debe procesar. Al utilizar este valor de posición, se obtiene el título del elemento por el comando [`GET LIST ITEM`](../commands/get-list-item). También en este caso, no es necesario identificar cada elemento individualmente; puede pasar cualquier valor (excepto 0) en el parámetro *itemRef*.

2. Necesita identificar parcialmente los elementos de la lista (nivel intermediario).  
   El número de referencia del elemento se utiliza para almacenar la información necesaria cuando hay que trabajar con dicho elemento; este punto se explica con detalle en el ejemplo del comando [`APPEND TO LIST`](../commands/append-to-list). En este ejemplo, utilizamos los números de referencia de los artículos para almacenar los números de registro. Sin embargo, debemos ser capaces de establecer una distinción entre los elementos que corresponden a los registros [Department] y los que corresponden a los registros [Employees].

3. Debe identificar cada los elementos de la lista individualmente (nivel avanzado).  
   Programe una gestión elaborada de listas jerárquicas en la que es absolutamente necesario poder identificar cada elemento individualmente en cada nivel de la lista. Una forma sencilla de ponerlo en práctica es mantener un contador personal. Supongamos que crea una lista *hlList* utilizando el comando [`APPEND TO LIST`](../commands/append-to-list). En esta etapa, se inicializa un contador *vhlCounter* en 1. Cada vez que llame a [`APPEND TO LIST`](../commands/append-to-list) o a [`INSERT IN LIST`](../commands/insert-in-list), incrementará este contador `(vhlCounter:=vhlCounter+1)` y pasará el número del contador como número de referencia del elemento. El truco consiste en no disminuir nunca el contador cuando se eliminan elementos: el contador sólo puede aumentar. De este modo, se garantiza la unicidad de los números de referencia de los elementos. Como estos números son de tipo Entero largo, puede añadir o insertar más de dos mil millones de elementos en una lista que ha sido reiniciada... (sin embargo, si está trabajando con un número tan grande de elementos, esto suele significar que debe utilizar una tabla en lugar de una lista.)
...
En la mayoría de los casos, cuando se utilizan listas jerárquicas con fines de interfaz de usuario y cuando sólo se trata del elemento seleccionado (por un clic o arrastrado), no será necesario utilizar los números de referencia de los elementos en absoluto. Usando [`Selected list items`](../commands/selected-list-items) y [`GET LIST ITEM`](../commands/get-list-item) tiene todo lo que necesita para tratar con el elemento seleccionado actualmente. Además, comandos como [`INSERT IN LIST`](../commands/insert-in-list) y [`DELETE FROM LIST`](../commands/delete-from-list) le permiten manipular la lista "relativamente" con respecto al elemento seleccionado.
