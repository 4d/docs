...
4D bietet einen umfangreichen Satz vordefinierter Konstanten, deren Werte per Name zugänglich sind. Das sorgt für leichter lesbaren Code. Zum Beispiel ist `Read Mode` eine Konstante (Wert 2).
...
Um eine Instanz auf ein Objekt der Klasse in einer Methode zu setzen, rufen Sie die Benutzerklasse aus dem _Class Store_ auf (`cs`) und verwenden die Member Function `new()`. Sie können Parameter übergeben.
...
In der Klassenmethode `myClass` definieren Sie mit der Anweisung `Function <methodName>` die Member Method _methodName_ der Klasse. Sie kann wie jede andere Methode Parameter empfangen und zurückgeben, und `This` als Instanz des Objekts verwenden.
