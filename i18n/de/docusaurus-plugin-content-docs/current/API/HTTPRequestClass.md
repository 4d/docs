...
| Ergebnis  | 4D.HTTPRequest | <- | New HTTPRequest object|<!-- END REF -->
...
| decodeData             | Boolean                                         | If true, the data received in the `onData` callback is uncompressed                                                                                                                                                                                               | Falsch       |
...
| proxyAuthentication    | [authentication object](#authentication-object) | Object handling proxy authentication                                                                                                                                                                                                                              | Undefiniert  |
...
| validateTLSCertificate | Boolean                                         | If false, 4D does not validate the TLS certificate and does not return an error if it is invalid (i.e. expired, self-signed...). Important: In the current implementation, the Certification Authority itself is not verified.                                    | Wahr         |
...
| .typ     | Text | Type of event. Possible values: "response", "error", "headers", "data", or "terminate |
...
| Ergebnis  | Objekt     | <- | Object, each property is a part of the multipart data|<!-- END REF -->
...
| .headers    | Objekt  | Headers of the response. Header names are returned in lowercase. `<headername>.key` = value (value can be a collection if the same key appears multiple times). Undefined if the headers have not been received yet.                         |
| .status     | Zahl    | Status code of the response                                                                                                                                                                                                                        |
| .statusText | Text    | Message explaining the status code                                                                                                                                                                                                                 |
| .rawHeaders | Objekt  | Headers of the response. Header names are returned untouched (with their original case). `<headerName>.key` = value (value can be a collection if the same key appears multiple times). Undefined if the headers have not been received yet. |
...
| Ergebnis  | 4D.HTTPRequest | <- | HTTPRequest object|<!-- END REF -->
