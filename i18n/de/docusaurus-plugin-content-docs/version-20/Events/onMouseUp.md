...
When the `On Mouse Up` event is generated, you can get the local coordinates where the mouse button was released. These coordinates are returned in the `MouseX` and `MouseY` System variables. Sie werden in Pixel angegeben, ausgehend von der oberen linken Ecke des Bildes (0,0).
